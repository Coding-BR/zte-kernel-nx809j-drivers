#!/usr/bin/env python3
"""Build and run the gpio_keys_quiesce_key host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_quiesce_key host tests (2 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "gpio_keys_quiesce_key_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-quiesce-key",
        ...
        cycles.append({
            "cycle": cycle, "compile_returncode": compiled.returncode,
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_quiesce_key_asan_ubsan",
        "driver": "gpio_keys_nubia", "target": "gpio_keys_quiesce_key",
        "covered_functions": ["gpio_keys_quiesce_key"],
        "source": str(source), "source_sha256": sha256(source),
        "expected_scenarios": 2, "repetitions": 2, "cycles": cycles,
        "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness uses stand-ins for timer_list and delayed_work offsets.",
