...
static long gpio_keys_store_disabled_switches(struct device *dev,
						      ...
						      const char *buf, unsigned long count)
{
	...
	(void)attr;
	last_count = count;
	error = gpio_keys_attr_store_helper(dev->ddata, buf, 5);
...
static bool test_returns_count_on_success(void)
{
	...
	reset_trace(0);
	REQUIRE(gpio_keys_store_disabled_switches(&dev, NULL, input, 3) == 3);
	REQUIRE(helper_calls == 1 && last_ddata == &ddata && last_buf == input);
	REQUIRE(last_type == 5 && last_count == 3);
	return true;
}
...
static bool test_preserves_helper_error(void)
{
	...
	reset_trace(-22);
	REQUIRE(gpio_keys_store_disabled_switches(&dev, NULL, input, 3) == -22);
	REQUIRE(helper_calls == 1 && last_ddata == &ddata && last_type == 5);
	return true;
}
...
static bool test_preserves_zero_count(void)
{
	...
	reset_trace(0);
	REQUIRE(gpio_keys_store_disabled_switches(&dev, NULL, NULL, 0) == 0);
	REQUIRE(helper_calls == 1 && last_ddata == &ddata && last_buf == NULL);
	REQUIRE(last_type == 5 && last_count == 0);
	return true;
}
...
int main(void)
{
	if (!test_returns_count_on_success() ||
		...
		return 1;
	puts("PASS gpio_keys_store_disabled_switches host tests (3 scenarios)");
	return 0;
}
