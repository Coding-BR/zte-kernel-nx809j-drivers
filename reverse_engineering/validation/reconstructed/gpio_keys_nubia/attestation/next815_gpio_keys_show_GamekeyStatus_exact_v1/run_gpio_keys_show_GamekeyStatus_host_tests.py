#!/usr/bin/env python3
"""Build and run the gpio_keys_show_GamekeyStatus host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_show_GamekeyStatus host tests (3 scenarios)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path, default=pathlib.Path(__file__).with_name("gpio_keys_show_GamekeyStatus_host_test.c"))
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v", f"{cycle_root}:/output", "-v", f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE, CLANG, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra", "-Werror", "-fno-omit-frame-pointer", "-fno-pie", "-no-pie", "-frandom-seed=gpio-keys-nubia-show-gamekeystatus", "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined", "-Wl,--build-id=none", "/drivers/validation/gpio_keys_nubia/host/gpio_keys_show_GamekeyStatus_host_test.c", "-o", f"/output/{binary_name}"]
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "mode": "offline_direct_source_gpio_keys_show_GamekeyStatus_asan_ubsan", "driver": "gpio_keys_nubia", "target": "gpio_keys_show_GamekeyStatus", "covered_functions": ["gpio_keys_show_GamekeyStatus"], "source": str(source), "source_sha256": sha256(source), "expected_scenarios": 3, "repetitions": 2, "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible, "passed": passed, "status": "PASS" if passed else "FAIL", "limitations": ["The harness uses stand-ins for GPIO descriptor and formatting helpers.", "The AArch64 assembly body is validated separately by strict opcode and relocation comparison."]}
