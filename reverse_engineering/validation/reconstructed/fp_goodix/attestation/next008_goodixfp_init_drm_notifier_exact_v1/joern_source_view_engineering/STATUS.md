...
- `gf_ioctl` teve a condição `power_offset` reescrita na forma de branch equivalente ao stock; a rechecagem isolada reproduziu tamanho/instruções do módulo inteiro sem regressão nos 26 símbolos fechados, mas a função ainda diverge na comparação final de opcodes/relocations.
