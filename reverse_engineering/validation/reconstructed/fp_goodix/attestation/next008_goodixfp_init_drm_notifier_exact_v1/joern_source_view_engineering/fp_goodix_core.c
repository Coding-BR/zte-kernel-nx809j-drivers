...
static int goodixfp_drm_get_pannel(struct device_node *np)
{
...
static void goodixfp_init_drm_notifier(struct work_struct *work)
{
	...
	if (list_entry_is_head(gf_dev, &device_list, device_entry)) {
		...
		if (gf_dev->ref_count == 1) {
			...
			ret = request_threaded_irq(gf_dev->irq_num, NULL, gf_irq,
						   ...
						   "gf", gf_dev);
/* joern-source-view omitted function-local directive */
