...
| complete recoverable KCFI surface | `INCOMPLETE` | kcfi_full_surface.json matches 185/322 stock functions with recoverable preambles Note: 137 signatures differ and 46 stock functions require separate preamble review |
...
| stock text and import surface | `PASS` | symbol_inventory_kmi_exact_final.json records no missing stock text symbol and no missing or unexpected import Note: 153 candidate-only text symbols are classified and non-gating (decompiler_subroutine=131, duplicate_renamed=9, other=11, signature_wrapper=2) |
...
- Full KCFI surface has 137 mismatched function signatures
