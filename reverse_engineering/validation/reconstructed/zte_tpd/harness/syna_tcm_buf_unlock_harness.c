...
#include "../../../../../kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_buf_unlock.c"
...
{ memset(fixture, 0, sizeof(fixture)); fixture[0x40] = depth; unlocked = 0; unlocks = logs = logged_depth = 0; syna_tcm_buf_unlock((__int64)(uintptr_t)fixture); check(unlocks == 1, "unlock_once", bad); check(unlocked == (uintptr_t)fixture + 0x10, "mutex_offset", bad); check(fixture[0x40] == expected, "depth_transition", bad); check(logs == expected_logs, "log_transition", bad); if (expected_logs) check(logged_depth == depth, "log_depth", bad); }
