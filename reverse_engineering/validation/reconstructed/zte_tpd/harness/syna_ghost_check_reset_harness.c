...
struct syna_tcm {
    uint8_t reserved_0000[0x270];
    struct syna_hw_interface *hw_if;
};

struct ztp_device {
    uint8_t reserved_0000[0xdb8];
    struct syna_tcm *syna_tcm;
};

_Static_assert(offsetof(struct ztp_device, syna_tcm) == 0xdb8, "cdev context offset");
_Static_assert(offsetof(struct syna_tcm, hw_if) == 0x270, "tcm hw_if offset");
...
{ calls[call_count++] = 1; seen_tcm[0] = tcm; }
void syna_spi_hw_reset(struct syna_hw_interface *hw_if)
{ calls[call_count++] = 2; seen_hw_if = hw_if; }
int syna_recovery_game_mode_after_reset(struct syna_tcm *tcm)
{ calls[call_count++] = 3; seen_tcm[1] = tcm; return 0; }
int syna_dev_set_screen_on_fp_mode(struct syna_tcm *tcm, unsigned int enable)
{ calls[call_count++] = 4; seen_tcm[2] = tcm; seen_fp_mode = enable; return 0; }
int printk(const char *format, const char *name)
{ calls[call_count++] = 5; seen_format = format; seen_name = name; return 0; }

#include "build/syna_ghost_check_reset_under_test.c"
...
int main(void)
{
    ...
    check(calls[0] == 1 && calls[1] == 2 && calls[2] == 3 && calls[3] == 4 && calls[4] == 5,
          "call_order", &failures);
    check(seen_tcm[0] == &tcm && seen_tcm[1] == &tcm && seen_tcm[2] == &tcm,
          "tcm_context", &failures);
    ...
    check(strcmp(seen_name, "syna_ghost_check_reset") == 0, "stock_log_name", &failures);
