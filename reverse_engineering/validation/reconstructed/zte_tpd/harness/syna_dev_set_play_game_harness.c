...
#include "../../../../../kernel_development/drivers/reconstructed/zte_tpd/syna_dev_set_play_game.c"
...
int main(void)
{
	...
	check(syna_dev_set_play_game(NULL, 5, 8) == -22, "null_returns_einval", &failures);
	check(config_calls == 0 && log_calls == 1, "null_logs_before_validation", &failures);
	config_result = 0; config_calls = log_calls = 0;
	check(syna_dev_set_play_game(&tcm, 5, 8) == 0, "success_preserved", &failures);
	check(config_calls == 1 && seen_device == tcm.tcm_dev && seen_id == 213 && seen_value == 15 && seen_delay == 8, "dynamic_config_arguments", &failures);
	check(log_calls == 1, "success_logs", &failures);
	config_result = -55; config_calls = 0;
	check(syna_dev_set_play_game(&tcm, 2, 3) == -55, "negative_error_preserved", &failures);
