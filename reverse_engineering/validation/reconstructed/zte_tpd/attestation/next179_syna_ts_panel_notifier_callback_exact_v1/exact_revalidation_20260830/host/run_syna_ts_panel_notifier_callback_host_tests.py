#!/usr/bin/env python3
"""Build and run the panel notifier callback host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_ts_panel_notifier_callback host tests (6 paths)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-I", "/drivers/validation/zte_tpd/host/stubs",
                   "-frandom-seed=zte-tpd-panel-notifier", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_ts_panel_notifier_callback_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_ts_panel_notifier_callback_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_ts_panel_notifier_callback",
              "covered_functions": ["syna_ts_panel_notifier_callback", "ufp_notifier_cb",
                                     "ufp_report_lcd_state", "change_tp_state"],
              ...
              "limitations": ["The harness uses deterministic stubs for panel callbacks and printk.",
                              "No smartphone, DRM notifier, IRQ, firmware transport or hardware is exercised."]}
