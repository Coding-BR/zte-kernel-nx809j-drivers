#!/usr/bin/env python3
"""Build and run the tpd_set_one_key contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_set_one_key contract test (5 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_set_one_key_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_set_one_key.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_set_one_key_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-set-one-key",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_set_one_key_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_set_one_key_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_set_one_key", "function": "tpd_set_one_key",
        ...
        "limitations": [
            "The harness validates the observed null-context error and field updates with fake memory.",
