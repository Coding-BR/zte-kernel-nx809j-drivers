#!/usr/bin/env python3
"""Build and run the tpd_get_fake_sleep contract harness twice."""
...
EXPECTED = "PASS tpd_get_fake_sleep contract test (4 checks)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        command = [
            ...
            "/drivers/validation/zte_tpd/host/tpd_get_fake_sleep_contract_test.c "
            "-o /out/tpd_get_fake_sleep_contract_test",
        ]
        compiled = subprocess.run(command, text=True, capture_output=True, check=False)
        executed = None
        binary = cycle / "tpd_get_fake_sleep_contract_test"
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "--network", "none", "-v",
                 f"{cycle.resolve()}:/out:ro", IMAGE,
                 "/out/tpd_get_fake_sleep_contract_test"],
    ...
    report = {"schema_version": 1, "driver": "zte_tpd", "target": "tpd_get_fake_sleep",
              "mode": "offline_contract_model_asan_ubsan",
              "source": str(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/tpd_get_fake_sleep.c"),
              "source_sha256": sha256(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/tpd_get_fake_sleep.c"),
              "expected_output": EXPECTED, "cycles": cycles,
              "reproducible_binary": passed, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "coverage": {"context_load_offset_0xdb8": True, "source_load_offset_0x5e8": True,
                           "destination_store_offset_0x480": True, "source_unchanged": True},
              "limitations": ["Uses a deterministic memory fixture; hardware behavior is a separate gate.",
