#!/usr/bin/env python3
"""Run the direct tp_frame_data_read source contract twice with ASan/UBSan."""
...
EXPECTED = "tp_frame_data_read host contract: PASS (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_frame_data_read_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_frame_data_read.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_frame_data_read source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-frame-data-read-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_frame_data_read_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_frame_data_read", "mode": "offline_direct_source_asan_ubsan",
