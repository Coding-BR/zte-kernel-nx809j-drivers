...
#include "../../../reconstructed/zte_tpd/syna_testing_pt0a_zte.c"
...
static void test_allocation_failure(void)
{
	...
	allocation_fails = 1;
	expect(syna_testing_pt0a_zte(&fixture.tcm) == -ENOMEM, name,
	       "return mismatch");
	expect(allocation_calls == 1 && run_calls == 0 && data_kfree_calls == 0,
	       name, "unexpected side effect");
	expect(strcmp(last_log,
	       "\0013[error] syna_testing_pt0a_zte: alloc data_buf failed") == 0,
	       name, "log mismatch");
}
...
static void test_disconnected_preserves_stock_leak(void)
{
	...
	reset_fixture(&fixture);
	result = syna_testing_pt0a_zte(&fixture.tcm);
...
static void test_missing_item_preserves_stock_leak(void)
{
	...
	selected_item = NULL;
	result = syna_testing_pt0a_zte(&fixture.tcm);
	expect(result == (int)strlen("Invalid testing item id:2560\n"), name,
	       "return mismatch");
...
static void test_run_failure(void)
{
	...
	fixture.tcm.connection_flags = 1;
	run_return = -5;
	expect(syna_testing_pt0a_zte(&fixture.tcm) > 0, name, "return mismatch");
	expect(strstr(copied_text, "PT0A (version.7): Fail") != NULL, name,
	       "failure status missing");
	expect(strcmp(last_log,
	       "\0013[error] syna_testing_pt0a_zte: Fail to run test, PT0A\n") == 0,
	       name, "log mismatch");
...
static void test_signed_matrix_and_tcm_stride(void)
{
	const char *name = "signed_matrix_stride";
	struct fixture fixture;
...
int main(void)
{
	...
	test_result_pass();
	test_result_fail();
	test_signed_matrix_and_tcm_stride();
	...
	puts("PASS syna_testing_pt0a_zte host tests (12 cases)");
	return 0;
}
