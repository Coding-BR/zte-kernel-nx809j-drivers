# `syna_dev_set_follow_hand_level` offline exact attestation

This package records the reconstruction of stock `zte_tpd` function
`syna_dev_set_follow_hand_level` at entry `0x00111718` for the NX809J.

## Decision

`OFFLINE_EXACT` at function level. The candidate matches the stock module at
the normalized AArch64 opcode and relocation surface: 98 instructions and 392
bytes, with the stock string references and five calls to
`syna_tcm_set_dynamic_config`. The KCFI type ID is `0x1eb3b73d` and the symbol
size matches at 392 bytes.
...
- Exact linked Assembly source: `syna_dev_set_follow_hand_level_exact.S`
