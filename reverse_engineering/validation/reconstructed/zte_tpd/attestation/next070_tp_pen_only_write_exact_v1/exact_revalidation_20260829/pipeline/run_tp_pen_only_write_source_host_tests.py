#!/usr/bin/env python3
"""Run the direct tp_pen_only_write source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS tp_pen_only_write: 5 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_pen_only_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_pen_only_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_pen_only_write source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "-frandom-seed=zte-tpd-pen-only-write-source",
                   "-ffile-prefix-map=/drivers=<drivers>",
                   "/drivers/validation/zte_tpd/host/tp_pen_only_write_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_pen_only_write", "mode": "offline_direct_source_asan_ubsan",
