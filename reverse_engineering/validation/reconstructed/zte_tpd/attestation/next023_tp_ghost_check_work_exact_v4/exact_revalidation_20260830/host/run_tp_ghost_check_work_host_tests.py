...
EXPECTED_STDOUT = "PASS tp_ghost_check_work host tests (5 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "tp_ghost_check_work_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target385-ghost-check-work",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tp_ghost_check_work_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tp_ghost_check_work_asan_ubsan",
              ...
              "limitations": [
                  "tp_ghost_check, ghost_check_reset, printk and the valid callback are stubbed.",
                  "The invalid KCFI callback path is verified statically and is intentionally not invoked by the host harness.",
