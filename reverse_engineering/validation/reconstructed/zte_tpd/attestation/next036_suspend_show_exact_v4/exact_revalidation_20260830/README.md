# `suspend_show` — offline exact revalidation

- Driver: `zte_tpd`
- Stock entry: `0x0010a1b4`
- Stock body: 61 AArch64 instructions / 244 bytes
- Candidate source: `candidate_source.c` (`suspend_show.c`)
...
- The Docker ASan/UBSan host contract passed twice with the direct sysfs-read cases and fake VFS/callback memory.

## Reconstructed behavior

The function only performs the read at offset zero, invokes the optional callback at `tpd_cdev + 0xe58`, reads the suspend byte at `tpd_cdev + 0x15`, logs the stock message, formats `tp suspend is: %u\n` with the stock bound, and delegates to `simple_read_from_buffer`.

## Scope and limitations

This is an offline exact binding-level attestation. The real procfs/sysfs registration, concurrent callback lifetime, smartphone/NX809J hardware behavior, and runtime integration remain unverified. No hardware PASS is claimed.
