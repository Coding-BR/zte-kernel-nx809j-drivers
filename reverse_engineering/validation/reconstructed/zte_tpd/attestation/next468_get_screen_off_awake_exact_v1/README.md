# `get_screen_off_awake` offline exact attestation

This package records the reconstruction of stock `zte_tpd` function
`get_screen_off_awake` at entry `0x0010d240` for the ZTE NX809J / RedMagic 11
Pro+.
...
The candidate object is linked from `get_screen_off_awake_exact.S`. It preserves
the stock PAC/stack-protector sequence, callback offset `0xfd8` from
`tpd_cdev`, value offset `0x484`, callback guard `0x73fc0d79`, formatting and
`simple_read_from_buffer` flow. `candidate_source.c` is the readable lift and
...
- Exact linked Assembly source: `get_screen_off_awake_exact.S`
