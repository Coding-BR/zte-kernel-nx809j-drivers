# `tp_fold_state_write` offline exact attestation

This package records the reconstruction of stock `zte_tpd` function
`tp_fold_state_write` at entry `0x0010cfa4` for the ZTE NX809J / RedMagic 11
Pro+.
...
The candidate object is linked from `tp_fold_state_write_exact.S`. It preserves
the stock PAC/stack-protector sequence, callback offset `0xfc0` from
`tpd_cdev`, callback guard `0x6af8b499`, log strings, and the KCFI preamble
immediately before the symbol. `candidate_source.c` is the readable lift and
is not the object promoted by this exact target.

The host harness also caught and corrected the lifted boolean normalization:
the stock `cset` converts any parsed nonzero value to `1` before the callback.
...
- Exact linked Assembly source: `tp_fold_state_write_exact.S`
