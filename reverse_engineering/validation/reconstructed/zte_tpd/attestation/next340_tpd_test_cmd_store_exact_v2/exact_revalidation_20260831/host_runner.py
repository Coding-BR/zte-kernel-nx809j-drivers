#!/usr/bin/env python3
"""Build and run the tpd_test_cmd_store contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_test_cmd_store contract test (15 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_test_cmd_store_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_test_cmd_store.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_test_cmd_store_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-test-cmd-store",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_test_cmd_store_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_test_cmd_store_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_test_cmd_store", "function": "tpd_test_cmd_store",
        ...
        "expected_cases": 15, "repetitions": 2, "sanitizers": ["address", "undefined"],
        "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness uses stubbed production-test routines and a fake hardware-reset callback.",
            "It does not exercise the real transport, IRQ, MMIO, flash hardware or module loading.",
        ],
    }
