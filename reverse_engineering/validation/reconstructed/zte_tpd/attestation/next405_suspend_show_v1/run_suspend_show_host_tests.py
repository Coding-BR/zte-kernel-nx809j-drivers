#!/usr/bin/env python3
"""Run the suspend_show host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS suspend_show host tests (2 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "suspend_show_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "suspend_show.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-suspend-show",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/suspend_show_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe58 callback offset, +0x15 state byte, formatting, partial read and EOF contracts with fake memory.",
