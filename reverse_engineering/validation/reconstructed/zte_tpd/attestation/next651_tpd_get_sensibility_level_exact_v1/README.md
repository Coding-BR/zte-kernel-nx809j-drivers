# `tpd_get_sensibility_level` offline exact attestation

This package records the function-level reconstruction of
`tpd_get_sensibility_level` at stock entry `0x0012c18c` for the ZTE NX809J /
...
The exact assembly source is emitted by `tpd_get_sensibility_level_exact.S`.
...
- `host_harness_report.json`: 4 direct contract checks, two cycles
