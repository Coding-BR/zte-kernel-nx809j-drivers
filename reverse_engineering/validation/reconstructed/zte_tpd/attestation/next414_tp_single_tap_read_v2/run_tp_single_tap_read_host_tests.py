#!/usr/bin/env python3
"""Run the tp_single_tap_read host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_single_tap_read host tests (3 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation/zte_tpd/host/tp_single_tap_read_host_test.c"
    source = drivers / "reconstructed/zte_tpd/tp_single_tap_read.c"
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=zte-tpd-tp-single-tap-read",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tp_single_tap_read_host_test.c",
