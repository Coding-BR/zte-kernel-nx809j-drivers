#!/usr/bin/env python3
"""Run the headset_state_show host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS headset_state_show host tests (2 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "headset_state_show_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "headset_state_show.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-headset-state-show",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/headset_state_show_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe78 callback offset, +0x17 state byte, formatting, partial read and EOF contracts with fake memory.",
