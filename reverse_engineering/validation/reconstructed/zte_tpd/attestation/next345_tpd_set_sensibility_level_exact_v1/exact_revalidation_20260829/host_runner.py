#!/usr/bin/env python3
"""Build and run the tpd_set_sensibility_level contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_set_sensibility_level contract test (12 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_set_sensibility_level_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_set_sensibility_level.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_set_sensibility_level_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-set-sensibility-level",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_set_sensibility_level_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_set_sensibility_level_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_set_sensibility_level", "function": "tpd_set_sensibility_level",
        ...
        "expected_cases": 12, "repetitions": 2, "sanitizers": ["address", "undefined"],
        "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness stubs syna_dev_set_sensibility_level and uses fake state/hardware memory.",
