#!/usr/bin/env python3
"""Build and run the syna_spi_release harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_spi_release host tests (2 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        type=pathlib.Path,
        default=pathlib.Path(__file__).with_name("syna_spi_release_host_test.c"),
    )
    ...
    driver_source = (
        source.parents[3] / "reconstructed" / "zte_tpd" / "syna_spi_release.c"
    )
    ...
    report = {
        ...
        "mode": "offline_direct_source_next21_release_asan_ubsan",
        "target": "syna_spi_release",
        "source": str(source),
        ...
        "limitations": [
            "The harness stubs printk and does not use a smartphone.",
            "It proves exact log bytes, call count and ignored-device behavior.",
