# `syna_dev_disconnect` offline exact attestation

This package records the function-level reconstruction of
`syna_dev_disconnect` at stock entry `0x00112b04` for the ZTE NX809J /
RedMagic 11 Pro+ target.

## Decision

`PROMOTED_OFFLINE_EXACT` at function level. The candidate matches the stock
function at 98 AArch64 instructions and 392 bytes, including relocation
sites and the stock `.rodata.str1.1` strings. KCFI type ID
`0xae20471c` and symbol size match exactly.

The exact assembly source is emitted by
`syna_dev_disconnect_exact.S`; the preserved sparse stock string bytes are
