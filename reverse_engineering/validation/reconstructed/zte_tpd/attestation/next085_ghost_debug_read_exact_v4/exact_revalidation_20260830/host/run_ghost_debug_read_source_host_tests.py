#!/usr/bin/env python3
"""Run the direct ghost_debug_read source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS ghost_debug_read host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "ghost_debug_read_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "ghost_debug_read.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("ghost_debug_read source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-ghost-debug-read-source",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "ghost_debug_read", "mode": "offline_direct_source_asan_ubsan",
