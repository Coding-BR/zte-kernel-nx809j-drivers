# `tpd_get_follow_hand_level` exact revalidation

This package records the Docker-backed revalidation of microtask `346_tpd_get_follow_hand_level`.
...
- AArch64 function comparison: PASS; 6 instructions, 24-byte symbol, and equivalent relocations for stock entry `0x0012c26c`.
...
The host oracle validates the follow-hand level state-to-device mirror transfer for two values. The module was built from the Docker-curated snapshot with image `nubia-sm8850-kernel-builder:latest`, toolchain `clang-r536225`, and two canonical build cycles.
