#!/usr/bin/env python3
"""Build and run the direct-source normal-sensing harness twice in Docker."""
...
EXPECTED_STDOUT = """PASS default_sleep_mode_is_used
PASS active_gesture_power_forces_zero_mode
PASS gesture_power_bit_zero_keeps_default_mode
PASS sleep_failure_returns_before_gesture
PASS low_power_gesture_success_delays
PASS low_power_gesture_receives_selected_mode
PASS gesture_failure_returns_without_delay
PASS low_power_gesture_requires_exact_one
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_enter_normal_sensing_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_dev_enter_normal_sensing.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target172-enter-normal-sensing",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_enter_normal_sensing_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_enter_normal_sensing_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_enter_normal_sensing",
        "covered_functions": ["syna_dev_enter_normal_sensing", "syna_tcm_sleep",
        ...
        "limitations": [
            "The host-only branch replaces the inline AArch64 load/branch sequence with equivalent offset reads; the original AArch64 assembly remains the non-host path.",
            "TCM sleep, low-power gesture, printk and msleep are stubs; no real transport, concurrency or hardware is exercised.",
