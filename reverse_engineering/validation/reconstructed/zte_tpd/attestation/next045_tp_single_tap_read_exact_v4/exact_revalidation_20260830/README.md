# `tp_single_tap_read` — offline exact revalidation

- Driver: `zte_tpd`
- Stock entry: `0x0010aa5c`
- Stock body: 60 AArch64 instructions / 240 bytes
- Candidate source: `candidate_source.c` (`tp_single_tap_read.c`)
...
The function performs work only at file offset zero, invokes the callback at `tpd_cdev + 0xeb8` when present, logs the single-tap value at `tpd_cdev + 0x44c`, formats it with the stock ten-byte bound, and delegates the userspace copy/offset update to `simple_read_from_buffer`.
