#!/usr/bin/env python3
"""Build and run the direct zte_touch_probe host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS zte_touch_probe host tests (4 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("zte_touch_probe_host_test.c"))
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "zte_touch_probe.c"
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm",
			...
			"-frandom-seed=zte-tpd-next176-probe",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
			"-fno-sanitize=alignment", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/zte_touch_probe_host_test.c",
	...
	report = {"schema_version": 1,
		...
		"mode": "offline_direct_source_next176_probe_asan_ubsan", "driver": "zte_tpd",
		"target": "zte_touch_probe", "covered_functions": ["zte_touch_probe"],
		...
		"sanitizers": ["address", "undefined"], "disabled_sanitizers": ["alignment"],
		"suppressed_warnings": ["unused-variable", "unused-but-set-variable"],
		...
		"limitations": [
			"The harness directly executes reconstructed source and proves its observable offsets and branch effects.",
			"Unaligned raw stores are intentional in the recovered source; alignment sanitization is disabled for this host-only ABI model.",
			"Decompiler-only unused temporaries are not errors in the host adapter; all other warnings remain errors.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone, platform bus or hardware MMIO interaction is used."]}
