#!/usr/bin/env python3
"""Build and run the tpd_get_screen_off_awake contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_get_screen_off_awake contract test (4 checks)\n"
...
def main() -> int:
    ...
    repo = runner.parents[5]
    test_source = runner.with_name("tpd_get_screen_off_awake_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_screen_off_awake.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_screen_off_awake_contract_asan_ubsan"
        compile_command = [
            "docker", "run", "--rm", "-v", f"{repo / 'kernel_development' / 'drivers'}:/drivers:ro",
            "-v", f"{cycle}:/output", "-v", f"{TOOLS}:/toolchains:ro", IMAGE, CLANG,
            "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra", "-Werror", "-fno-omit-frame-pointer",
            "-fno-pie", "-no-pie", "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "-frandom-seed=zte-tpd-tpd-get-screen-off-awake", "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_get_screen_off_awake_contract_test.c", "-o", f"/output/{binary.name}",
        ]
        compiled = run(compile_command)
        executed = run([
            "docker", "run", "--rm", "-e", "ASAN_OPTIONS=detect_leaks=1:halt_on_error=1",
            "-e", "UBSAN_OPTIONS=halt_on_error=1:print_stacktrace=1", "-v", f"{cycle}:/output:ro",
            IMAGE, f"/output/{binary.name}",
    ...
    binary_hashes = [item["binary_sha256"] for item in cycles]
    reproducible = binary_hashes[0] is not None and len(set(binary_hashes)) == 1
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_screen_off_awake_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_get_screen_off_awake", "function": "tpd_get_screen_off_awake",
        ...
        "limitations": ["The harness uses fake client/context memory.", "It does not exercise transport, IRQ, MMIO, flash hardware or module loading."],
    }
