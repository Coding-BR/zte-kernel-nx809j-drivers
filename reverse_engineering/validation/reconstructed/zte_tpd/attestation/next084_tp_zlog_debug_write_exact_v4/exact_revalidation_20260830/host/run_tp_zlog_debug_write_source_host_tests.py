#!/usr/bin/env python3
"""Run the direct tp_zlog_debug_write source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_zlog_debug_write: 4 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_zlog_debug_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_zlog_debug_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_zlog_debug_write source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-zlog-debug-write-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_zlog_debug_write_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_zlog_debug_write", "mode": "offline_direct_source_asan_ubsan",
