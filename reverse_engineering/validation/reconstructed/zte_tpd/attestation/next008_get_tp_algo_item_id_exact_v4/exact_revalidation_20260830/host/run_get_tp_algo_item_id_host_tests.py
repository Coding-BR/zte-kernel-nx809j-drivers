#!/usr/bin/env python3
"""Build and run the TP algorithm-item selector harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS get_tp_algo_item_id host tests (9 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("get_tp_algo_item_id_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()

	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "get_tp_algo_item_id.c"
	...
	for cycle in (1, 2):
		...
		command = [args.compiler, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra",
			...
			"-frandom-seed=zte-tpd-next59-tp-algo-item-id",
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next59_tp_algo_item_id_asan_ubsan", "driver": "zte_tpd",
		"target": "get_tp_algo_item_id", "covered_functions": ["get_tp_algo_item_id"],
		...
		"limitations": ["The harness proves the seven-entry table, match priority, error and logging branches.",
