#!/usr/bin/env python3
"""Run the tp_smart_cover_write host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_smart_cover_write host tests (3 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "tp_smart_cover_write_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "tp_smart_cover_write.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-tp-smart-cover-write",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/tp_smart_cover_write_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe40 callback offset, bool normalization, -EINVAL and count-return contracts with fake memory.",
