# `tp_smart_cover_write` — offline exact revalidation

- Driver: `zte_tpd`
- Stock entry: `0x00109ce4`
- Stock body: 52 AArch64 instructions / 208 bytes
- Candidate source: `candidate_source.c` (`tp_smart_cover_write.c`)
...
- The Docker ASan/UBSan host contract passed twice with 3 direct cases covering the callback slot at `tpd_cdev + 0xe40`, boolean normalization, parse failure, and count/-EINVAL returns.

## Reconstructed behavior

The function parses an unsigned decimal value from userspace, returns `-EINVAL` on parse failure, normalizes successful values to boolean `0`/`1`, logs the stock format, and invokes the callback at `tpd_cdev + 0xe40` when present. The successful path returns the original byte count.
