# Exact revalidation: 031_tp_smart_cover_read

This package records the Docker-based revalidation of `tp_smart_cover_read` at stock entry `00109bf0`.
...
The Joern CPG uses compatibility defines `__user`, `__init` and `__exit`, and excludes `validation` and `build`. The host harness validates callback offset `+0xe38`, value offset `+0x4`, partial reads and EOF using fake memory; no physical NX809J or procfs runtime path is exercised.
