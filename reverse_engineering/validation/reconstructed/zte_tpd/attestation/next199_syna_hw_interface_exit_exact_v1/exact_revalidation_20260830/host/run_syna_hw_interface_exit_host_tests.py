#!/usr/bin/env python3
"""Build and run the syna_hw_interface_exit host harness twice in Docker."""
...
EXPECTED_STDOUT = "syna_hw_interface_exit host oracle: PASS (1 case)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-hw-interface-exit", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_hw_interface_exit_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_hw_interface_exit_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_hw_interface_exit",
              "covered_functions": ["syna_hw_interface_exit", "spi_unregister_driver",
              ...
              "limitations": ["The harness uses deterministic unregister stubs and assert-based oracle.",
                              "No smartphone, SPI controller, platform bus or hardware is exercised."]}
