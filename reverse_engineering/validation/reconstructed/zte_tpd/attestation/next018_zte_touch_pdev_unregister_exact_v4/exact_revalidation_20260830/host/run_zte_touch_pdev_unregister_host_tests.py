#!/usr/bin/env python3
"""Build and run the zte_touch_pdev_unregister host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS zte_touch_pdev_unregister host tests (4 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "zte_touch_pdev_unregister_contract_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "zte_touch_pdev_unregister.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target380-pdev-unregister",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none", "-Wno-int-conversion",
                   "/drivers/validation/zte_tpd/host/zte_touch_pdev_unregister_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_zte_touch_pdev_unregister_asan_ubsan",
              "driver": "zte_tpd", "target": "zte_touch_pdev_unregister",
              "covered_functions": ["zte_touch_pdev_unregister"],
              ...
              "limitations": [
                  "off_338 and platform_device_unregister are stubbed.",
                  "The stock null-slot behavior is reproduced deliberately; it is not a safety approval.",
