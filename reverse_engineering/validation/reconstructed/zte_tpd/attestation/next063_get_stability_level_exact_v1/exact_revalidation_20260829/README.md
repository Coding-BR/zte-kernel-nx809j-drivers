# `get_stability_level` exact revalidation

This package records the Docker-backed offline revalidation of microtask `063_get_stability_level`.

## Gates

- Canonical AArch64 kernel-module build: PASS, two independent cycles; candidate module is 6,307,336 bytes.
- AArch64 function comparison: PASS; 60 instructions, 240-byte symbol, `.text`, and equivalent relocations for stock entry `0x0010c148`.
- KCFI: PASS; type ID `0xf4e9d97c`, `.text`, and 240-byte symbol in stock and candidate.
- Direct source host test: PASS; 2 cases × 2 Docker cycles with AddressSanitizer and UndefinedBehaviorSanitizer.
- Joern strict gate: PASS; one requested function, one mapped source method, zero parse problems.

## Review observation

Joern reports one high-severity `userspace_egress` observation for the proven `simple_read_from_buffer` call. It is retained in the evidence and does not indicate a parser or build failure. The direct test validates the callback slot, procfs offset/EOF behavior, formatting, short-buffer behavior, and the stock ten-byte `snprintf` limit for the maximum unsigned value.
