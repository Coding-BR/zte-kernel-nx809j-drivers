#!/usr/bin/env python3
"""Run the direct get_stability_level source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS get_stability_level source host tests (2 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "get_stability_level_source_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "get_stability_level.c"
    exact_source = drivers / "reconstructed" / "zte_tpd" / "get_stability_level_exact.S"
    if not harness.is_file() or not source.is_file() or not exact_source.is_file():
        raise FileNotFoundError("get_stability_level source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-get-stability-level-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/get_stability_level_source_host_test.c",
    ...
    report = {
        ...
        "limitations": [
            "The harness includes the reconstructed get_stability_level.c and uses fake device memory, procfs helpers and callbacks.",
