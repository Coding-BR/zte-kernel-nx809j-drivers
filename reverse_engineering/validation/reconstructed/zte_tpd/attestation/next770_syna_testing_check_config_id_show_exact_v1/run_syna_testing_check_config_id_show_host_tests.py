#!/usr/bin/env python3
"""Build and run the check-config-ID sysfs harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_testing_check_config_id_show host tests (6 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		...
		default=pathlib.Path(__file__).with_name(
			"syna_testing_check_config_id_show_host_test.c"
		),
	)
	...
	driver_source = (
		...
		/ "reconstructed"
		/ "zte_tpd"
		/ "syna_testing_check_config_id_show.c"
	)
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-frandom-seed=zte-tpd-next224-testing-check-config-id",
	...
	report = {
		...
		"mode": "offline_direct_source_next53_testing_check_config_id_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_testing_check_config_id_show",
		"covered_functions": ["syna_testing_check_config_id_show"],
		"source": str(source),
		...
		"limitations": [
			...
			"It proves sysfs lookup offsets, result formatting, limits and cleanup branches.",
