# `syna_dev_set_follow_hand_level` — exact offline attestation v2

Current revalidation of microtask `147` at stock entry `0x00111718`.

The implementation is supplied by `syna_dev_set_follow_hand_level_exact.S`. The readable C reconstruction is retained for Joern and host-contract evidence; the Makefile excludes it from the module.
...
- AArch64 instructions and relocations: 98/98 and 25/25, with 392-byte symbol parity;
