...
struct syna_tcm {
	...
	uint32_t follow_hand_base;
	unsigned char gap[4];
	uint32_t follow_hand_high;
};
typedef struct tcm_dev *_QWORD;

static char unk_3A85B[] = "entry";
static char unk_3BDBF[] = "invalid";
static char unk_3B08C[] = "level0";
static char unk_387C8[] = "level1";
static char unk_3A1C3[] = "level2";
static char unk_3981B[] = "level3";
static char unk_3B75C[] = "level4";
...
static int printk(const char *format, ...)
{
	...
	if (format == unk_3A85B || format == unk_3BDBF) {
		if (strcmp(name, "syna_dev_set_follow_hand_level") != 0)
			failures++;
...
#include "../../../reconstructed/zte_tpd/syna_dev_set_follow_hand_level.c"
...
{
	reset_trace();
	expect_int("null return", syna_dev_set_follow_hand_level(NULL, 0, 37), -22);
...
{
	static const unsigned int config_values[] = {0x4500, 0x4510, 0x4520, 0x4530, 0x4540};
	static char *const success_formats[] = {
		unk_3B08C, unk_387C8, unk_3A1C3, unk_3981B, unk_3B75C,
	};
	struct syna_tcm tcm = { .tcm_dev = &expected_tcm };

	tcm.follow_hand_base = 0x4500;
	tcm.follow_hand_high = 0;
	for (int level = 0; level <= 4; ++level) {
		reset_trace();
		expect_int("valid return", syna_dev_set_follow_hand_level(&tcm, level, 37), 0);
		...
		expect_true("valid name", last_success_name &&
			    strcmp(last_success_name, "syna_dev_set_follow_hand_level") == 0);
...
{
	struct syna_tcm tcm = { .tcm_dev = &expected_tcm };
	tcm.follow_hand_base = 0x12;
	tcm.follow_hand_high = 0x34;
	reset_trace();
	expect_int("offset return", syna_dev_set_follow_hand_level(&tcm, 4, 91), 0);
	expect_int("offset encoded value", last_config_value, 0x3412 | 0x40);
...
static void test_invalid_levels(void)
{
	...
	for (int level = -1; level <= 5; level += 6) {
		reset_trace();
		expect_int("invalid return", syna_dev_set_follow_hand_level(&tcm, level, 37), 0);
...
static void test_config_error_propagation(void)
{
	...
	expect_int("error return", syna_dev_set_follow_hand_level(&tcm, 2, 99),
		   (int)0x80000005U);
...
int main(void)
{
	...
	if (failures != 0)
		return 1;
	puts("PASS syna_dev_set_follow_hand_level host tests (11 cases)");
	return 0;
}
