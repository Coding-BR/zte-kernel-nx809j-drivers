#!/usr/bin/env python3
"""Run the set_rotation_limit_level host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS set_rotation_limit_level host tests (3 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation/zte_tpd/host/set_rotation_limit_level_host_test.c"
    source = drivers / "reconstructed/zte_tpd/set_rotation_limit_level.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
            ...
            "-frandom-seed=zte-tpd-set-rotation-limit-level", "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/set_rotation_limit_level_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd", "target": "set_rotation_limit_level",
              "mode": "offline_direct_source_asan_ubsan", "source_sha256": sha256(source),
              "harness_sha256": sha256(harness), "expected_cases": 3, "repetitions": 2,
