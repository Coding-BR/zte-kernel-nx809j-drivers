#!/usr/bin/env python3
"""Build and run the tpd_set_singlefpgesture contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_set_singlefpgesture contract test (5 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_set_singlefpgesture_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_set_singlefpgesture.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_set_singlefpgesture_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-set-singlefpgesture",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_set_singlefpgesture_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_set_singlefpgesture_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_set_singlefpgesture", "function": "tpd_set_singlefpgesture",
        ...
        "expected_cases": 5, "repetitions": 2, "sanitizers": ["address", "undefined"],
        "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness validates the observed active and inactive state updates with fake memory.",
