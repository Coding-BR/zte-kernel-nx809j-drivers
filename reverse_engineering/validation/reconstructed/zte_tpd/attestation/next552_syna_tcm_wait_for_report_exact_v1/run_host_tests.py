#!/usr/bin/env python3
"""Run the syna_tcm_wait_for_report host contract twice with sanitizers."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "syna_tcm_wait_for_report_host_test.c "
        "-o /out/wait_for_report_host_test && /out/wait_for_report_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "wait_for_report_host_test"
    ...
    report = {
        "schema_version": 1,
        "target": "syna_tcm_wait_for_report",
