#!/usr/bin/env python3
"""Run the syna_tcm_read_flash_boot_config host contract twice with sanitizers."""
...
def main() -> int:
    ...
    contract_source = repo_root / "kernel_development/drivers/validation/zte_tpd/host/syna_tcm_read_flash_boot_config_host_test.c"
    driver_source = repo_root / "kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_read_flash_boot_config.c"
    build_root = args.build_root.resolve()
    output = args.output.resolve()
    if not contract_source.is_file():
        parser.error(f"contract source not found: {contract_source}")
    ...
    compile_command = (
        ...
        "syna_tcm_read_flash_boot_config_host_test.c "
        "-o /out/syna_tcm_read_flash_boot_config_host_test && "
        "/out/syna_tcm_read_flash_boot_config_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "syna_tcm_read_flash_boot_config_host_test"
    ...
    report = {"schema_version": 1, "target": "syna_tcm_read_flash_boot_config",
              "mode": "direct_source_asan_ubsan_host_contract",
              "source": str(contract_source), "source_sha256": sha256(contract_source),
