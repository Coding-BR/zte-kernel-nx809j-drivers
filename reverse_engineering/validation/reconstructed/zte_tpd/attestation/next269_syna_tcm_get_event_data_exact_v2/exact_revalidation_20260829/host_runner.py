#!/usr/bin/env python3
"""Build and run the syna_tcm_get_event_data host contract twice in Docker."""
...
EXPECTED_STDOUT = "PASS: syna_tcm_get_event_data host contract\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(__file__).with_name(
            "syna_tcm_get_event_data_host_test.c"
        ),
    )
    parser.add_argument(
        "--driver-source",
        ...
        / "reconstructed" / "zte_tpd" / "syna_tcm_get_event_data.c",
    )
    ...
    if not source.is_file():
        parser.error(f"contract source not found: {source}")
    ...
    for cycle in (1, 2):
        ...
        cycle_root.mkdir()
        binary_name = "syna_tcm_get_event_data_host_asan_ubsan"
        ...
        compile_command = [
            ...
            IMAGE, CLANG, "-std=gnu2x", "-O1", "-g", "-Wall", "-Wextra",
            "-Werror", "-fno-omit-frame-pointer", "-fno-pie", "-no-pie",
            "-D", "ZTE_TPD_HOST_TEST", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-get-event-data",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/syna_tcm_get_event_data_host_test.c",
    ...
    reproducible = (
        len(set(binary_hashes)) == 1 and binary_hashes[0] is not None
    )
    passed = all(cycle["passed"] for cycle in cycles) and reproducible
    report = {
        "schema_version": 1,
        "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
        "mode": "offline_direct_source_syna_tcm_get_event_data_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_get_event_data",
        "function": "syna_tcm_get_event_data",
        "covered_functions": ["syna_tcm_get_event_data"],
        "source": str(source),
        ...
        "limitations": [
            "The harness stubs the TCM read callback and buffer-copy transport.",
            "No smartphone, SPI, IRQ, firmware transport or hardware is exercised.",
        ],
    }
