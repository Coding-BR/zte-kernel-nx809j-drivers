#!/usr/bin/env python3
"""Run the syna_spi_power_on contract under Docker clang ASan/UBSan twice."""
...
SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_spi_power_on_host_test.c")
DRIVER = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_spi_power_on.c")
IMAGE = "nubia-sm8850-kernel-builder:latest"
TOOLCHAIN = "nubia_sm8850_kernel_toolchains"
CLANG = "/toolchains/clang-r536225/bin/clang"
EXPECTED = "PASS syna_spi_power_on host tests (8 cases)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-target206-power", "-ffile-prefix-map=/drivers=<drivers>",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_spi_power_on_asan_ubsan_docker",
        "driver": "zte_tpd", "target": "syna_spi_power_on", "source": str(source),
        ...
        "limitations": [
            "The harness stubs regulator, GPIO, delay, printk and hardware callbacks.",
            "It covers regulator and GPIO enable/disable branches, invalid regulator, error and delay paths.",
