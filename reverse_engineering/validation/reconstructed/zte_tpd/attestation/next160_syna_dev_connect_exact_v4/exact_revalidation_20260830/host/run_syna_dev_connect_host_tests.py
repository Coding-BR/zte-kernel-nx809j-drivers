#!/usr/bin/env python3
"""Build and run the syna_dev_connect host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_connect host tests (5 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_dev_connect.c"
)
DRIVER_SOURCE_ASM = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_dev_connect_exact.S"
)
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_dev_connect_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target164-connect", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_connect_contract_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_dev_connect_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_connect",
              "covered_functions": ["syna_dev_connect"], "source": str(source),
              ...
              "inputs": [
                  ...
                  {"path": "kernel_development/drivers/validation/zte_tpd/host/"
                          "syna_dev_connect_contract_test.c",
                   "sha256": sha256(source)},
              ],
              "expected_cases": 5, "repetitions": 2,
              "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "The harness covers null input, already-connected, device-detection failure, managed-device failure and IRQ failure contracts.",
                  "Firmware setup, indirect callbacks, workqueue/timer execution and physical hardware are stubbed and not dynamically exercised."]}
