#!/usr/bin/env python3
"""Run the buffer-copy variant contract under ASan and UBSan."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_tcm_buf_copy_1_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_buf_copy_1.c")
...
EXPECTED = "PASS syna_tcm_buf_copy_1 host tests (3 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            ...
            "-Wno-unused-function", "-Wno-int-conversion", "-Wno-error=uninitialized", "-fno-omit-frame-pointer", "-fno-pie", "-no-pie",
            "-frandom-seed=zte-tpd-next539-syna-tcm-buf-copy-1",
    ...
    report = {
        ...
        "mode": "offline_direct_source_next539_syna_tcm_buf_copy_1_asan_ubsan",
        "driver": "zte_tpd", "target": "syna_tcm_buf_copy_1",
        "covered_functions": ["syna_tcm_buf_copy_1"],
        ...
        "limitations": [
            "The harness exercises the reconstructed buffer contract with deterministic allocation and mutex stubs.",
