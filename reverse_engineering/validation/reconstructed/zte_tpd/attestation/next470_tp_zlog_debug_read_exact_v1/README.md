# `tp_zlog_debug_read` offline exact attestation

This package records the reconstruction of stock `zte_tpd` function
`tp_zlog_debug_read` at entry `0x0010d408` for the ZTE NX809J / RedMagic 11
Pro+.

## Decision

`OFFLINE_EXACT` at function level. The linked candidate matches the stock
function at 39 AArch64 instructions, 156 bytes, and the normalized opcode and
relocation surface. The KCFI type ID is `0xf4e9d97c` and the symbol size
matches exactly.

The candidate object is linked from `tp_zlog_debug_read_exact.S`. It preserves
the stock PAC/stack-protection sequence, zero-offset guard, 11-byte literal
`this no use`, `simple_read_from_buffer` call and stack-check failure path.
`candidate_source.c` is the readable lift and is not the object promoted by
this exact target.

This is not a claim of complete driver or hardware equivalence. User-buffer
behavior, firmware transport, and physical behavior are represented or
deferred; smartphone execution is intentionally outside this attestation.
...
- Exact linked Assembly source: `tp_zlog_debug_read_exact.S`
