#!/usr/bin/env python3
"""Build and run the tpd_proc_deinit host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_proc_deinit host tests (3 cases, 60 removals)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target377-proc-deinit",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_proc_deinit_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_proc_deinit_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_proc_deinit",
              "covered_functions": ["tpd_proc_deinit"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 3,
              "expected_removals_per_cleanup": 30, "repetitions": 2,
              "cycles": cycles,
              ...
              "limitations": [
                  "ProcFS removal is stubbed; the harness checks order, names and parent pointers.",
                  "No smartphone, ProcFS kernel state or physical hardware is exercised."]}
