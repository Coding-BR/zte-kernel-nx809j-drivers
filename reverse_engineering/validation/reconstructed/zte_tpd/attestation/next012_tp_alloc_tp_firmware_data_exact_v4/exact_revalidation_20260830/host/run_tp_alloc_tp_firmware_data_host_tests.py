#!/usr/bin/env python3
"""Build and run the firmware-data allocation harness twice with sanitizers."""
...
EXPECTED_STDOUT = ("PASS alloc_success\n"
	"PASS alloc_replaces_existing_buffer\n"
	"PASS alloc_descriptor_failure\n"
	"PASS alloc_data_failure\n"
	"SUMMARY 4/4 passed\n")
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("tp_alloc_tp_firmware_data_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()

	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "tp_alloc_tp_firmware_data.c"
	...
	for cycle in (1, 2):
		...
		command = [args.compiler, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra",
			...
			"-frandom-seed=zte-tpd-next62-tp-alloc-firmware-data",
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next62_tp_alloc_tp_firmware_data_asan_ubsan",
		"driver": "zte_tpd", "target": "tp_alloc_tp_firmware_data",
		"covered_functions": ["tp_alloc_tp_firmware_data"], "source": str(source),
		...
		"limitations": ["The harness directly exercises success, replacement, descriptor failure and data failure branches.",
