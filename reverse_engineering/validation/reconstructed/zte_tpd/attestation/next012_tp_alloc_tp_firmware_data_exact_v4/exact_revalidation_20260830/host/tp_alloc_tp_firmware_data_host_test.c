...
static void *test_kmalloc(size_t size, gfp_t flags)
{
	...
	kmalloc_size = size;
	kmalloc_flags = flags;
	if (kmalloc_fails || descriptor_index >= POOL_ITEMS)
		return NULL;
...
#define GFP_KERNEL ((gfp_t)3520)

#include "../../../reconstructed/zte_tpd/tp_alloc_tp_firmware_data.c"
...
static struct tpd_firmware_data *prepare_existing(void)
{
	struct tpd_firmware_data *firmware = &descriptor_pool[descriptor_index++];
	void *data = data_pool[data_index++];
...
static bool test_alloc_data_failure(void)
{
...
int main(void)
{
	static const struct test_case tests[] = {
		...
		{ "alloc_data_failure", test_alloc_data_failure },
	};
	size_t index;
	size_t passed = 0;
