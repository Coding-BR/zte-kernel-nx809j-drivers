#!/usr/bin/env python3
"""Build and run the point_is_in_limit_area host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS point_is_in_limit_area host tests (2 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("point_is_in_limit_area_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()
	source = args.source.resolve()
	drivers = source.parents[3]
	driver_source = drivers / "reconstructed" / "zte_tpd" / "point_is_in_limit_area.c"
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
			...
			f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE, CLANG, "-std=gnu11",
			"-O1", "-g", "-Wall", "-Wextra", "-Werror", "-Wno-int-conversion",
			"-fno-omit-frame-pointer", "-fno-pie", "-no-pie",
			"-frandom-seed=zte-tpd-next116-point-limit",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
			"-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/"
			"point_is_in_limit_area_host_test.c", "-o", f"/output/{binary_name}"]
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next116_point_is_in_limit_area_asan_ubsan",
		"driver": "zte_tpd", "target": "point_is_in_limit_area",
		"covered_functions": ["point_is_in_limit_area"], "source": str(source),
		"source_sha256": sha256(source), "expected_cases": 2, "repetitions": 2,
		"cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible,
		"passed": passed, "status": "PASS" if passed else "FAIL",
		"limitations": ["The harness covers stock-observed boundary, mode, ghost-flag and return branches.",
			"No smartphone, MMIO, firmware transport or physical hardware is exercised."]}
