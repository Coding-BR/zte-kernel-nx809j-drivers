...
The module was built from `C:\Users\adria\Desktop\drivers\kernel-docker-workspace\engenharia\curated\zte_tpd` with image `nubia-sm8850-kernel-builder:latest`, toolchain `clang-r536225`, and two canonical build cycles. The host oracle checks the two state reads, destination store and preservation of the second state value; the runner records the current driver-source hash explicitly. Joern was rerun against the current source tree and map.
