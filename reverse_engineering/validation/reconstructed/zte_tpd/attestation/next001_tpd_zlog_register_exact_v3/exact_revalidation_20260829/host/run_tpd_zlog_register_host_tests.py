#!/usr/bin/env python3
"""Build and run the tpd_zlog_register host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_zlog_register host tests (9 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/tpd_zlog_register.c"
)
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "tpd_zlog_register_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target375-zlog-register",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_zlog_register_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "driver": "zte_tpd", "target": "tpd_zlog_register",
              "covered_functions": ["tpd_zlog_register"], "source": str(source),
              ...
              "limitations": [
                  "zlog client, vmalloc, printk and notification transport are stubbed.",
                  "The vmalloc failure branch records the stock memset(NULL) call without dereferencing null; this is a known stock safety defect.",
