#!/usr/bin/env python3
"""Build and run the get_follow_hand_level contract harness twice in Docker."""
...
EXPECTED = "PASS get_follow_hand_level host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = ROOT / "kernel_development/drivers/validation/zte_tpd/host/get_follow_hand_level_host_test.c"
    source = ROOT / "kernel_development/drivers/reconstructed/zte_tpd/get_follow_hand_level.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "get_follow_hand_level_host_test_asan_ubsan"
        command = [
            ...
            "/drivers/validation/zte_tpd/host/get_follow_hand_level_host_test.c",
            "-o", "/out/get_follow_hand_level_host_test_asan_ubsan",
        ]
        ...
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "--network", "none", "-e",
                 ...
                 f"{cycle.resolve()}:/out:ro", IMAGE, "/out/get_follow_hand_level_host_test_asan_ubsan"],
    ...
    report = {
        ...
        "target": "get_follow_hand_level", "function": "get_follow_hand_level",
        ...
        "limitations": [
            "The harness exercises the getter contract with fake cdev memory.",
