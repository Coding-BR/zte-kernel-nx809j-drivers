#!/usr/bin/env python3
"""Build and run the direct zlog_register_work contract in Docker twice."""
...
EXPECTED_STDOUT = "PASS zlog_register_work host tests (4 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/zlog_register_work.c"
)
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "zlog_register_work_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target005",
                   "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/zlog_register_work_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_zlog_register_work_asan_ubsan",
              "driver": "zte_tpd", "target": "zlog_register_work",
              "covered_functions": ["zlog_register_work"],
              ...
              "limitations": [
                  "tpd_zlog_device and tpd_zlog_register are contract stubs.",
                  "The harness verifies registered and unregistered branches, device selection and work argument irrelevance.",
