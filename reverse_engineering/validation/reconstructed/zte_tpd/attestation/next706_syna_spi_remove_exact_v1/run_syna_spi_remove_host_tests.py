#!/usr/bin/env python3
"""Run the syna_spi_remove contract under ASan and UBSan."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_spi_remove_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_spi_remove.c")
...
EXPECTED = "PASS syna_spi_remove host tests (6 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-Wno-unused-variable", "-Wno-error=uninitialized", "-fno-omit-frame-pointer",
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next705-syna-spi-remove",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_spi_remove_asan_ubsan",
        "driver": "zte_tpd", "target": "syna_spi_remove",
        ...
        "limitations": [
            "The harness stubs GPIO, managed-memory, regulator and printk callbacks.",
            "It proves branch returns, resource ordering and global cleanup without a real SPI controller.",
