#!/usr/bin/env python3
"""Build and run the TP chip-ID selector harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS get_tp_chip_id host tests (13 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("get_tp_chip_id_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()

	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "get_tp_chip_id.c"
	...
	for cycle in (1, 2):
		...
		command = [args.compiler, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra",
			...
			"-frandom-seed=zte-tpd-next60-tp-chip-id",
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next60_tp_chip_id_asan_ubsan", "driver": "zte_tpd",
		"target": "get_tp_chip_id", "covered_functions": ["get_tp_chip_id"],
		...
		"sanitizers": ["address", "undefined"], "expected_cases": 13, "repetitions": 2,
		"cycles": cycles,
		...
		"limitations": ["The harness proves the exact vendor table, priority, cdev offset, error and log branches.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone or touch-controller interaction is used."]}
