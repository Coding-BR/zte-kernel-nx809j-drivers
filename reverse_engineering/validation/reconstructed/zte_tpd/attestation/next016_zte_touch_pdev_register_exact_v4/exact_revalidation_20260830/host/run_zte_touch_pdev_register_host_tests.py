...
EXPECTED_STDOUT = "PASS zte_touch_pdev_register host tests (4 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "zte_touch_pdev_register_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target378-pdev-register",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/zte_touch_pdev_register_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "driver": "zte_tpd", "target": "zte_touch_pdev_register",
              "covered_functions": ["zte_touch_pdev_register"],
              ...
              "limitations": [
                  ...
                  "The KCFI guard on dev.release is validated separately from this host harness.",
