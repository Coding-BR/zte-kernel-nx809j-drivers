#!/usr/bin/env python3
"""Build and run the syna_dev_process_touch_report host harness twice in Docker."""
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next170-process-touch-report",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_process_touch_report_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_process_touch_report_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_process_touch_report",
        ...
        "inputs": [
            {
                "path": "kernel_development/drivers/reconstructed/zte_tpd/"
                "syna_dev_process_touch_report.c",
                "sha256": hashlib.sha256(driver_source.read_bytes()).hexdigest(),
            },
            {
                "path": "kernel_development/drivers/validation/zte_tpd/host/"
                "syna_dev_process_touch_report_host_test.c",
        ...
        "limitations": [
            "The harness covers direct null, invalid-event, parser-error, no-input-device and empty-report contracts.",
            "Contact iteration, gesture, large-area and hardware paths remain independent gates.",
