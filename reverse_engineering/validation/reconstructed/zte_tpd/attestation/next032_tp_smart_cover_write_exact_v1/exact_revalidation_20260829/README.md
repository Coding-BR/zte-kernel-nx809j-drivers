# Exact revalidation: 032_tp_smart_cover_write

This package records the Docker-based revalidation of `tp_smart_cover_write` at stock entry `00109ce4`.
...
The Joern CPG uses compatibility defines `__user`, `__init` and `__exit`, and excludes `validation` and `build`. The host harness validates the `+0xe40` callback offset, bool normalization, `-EINVAL` and count-return contracts with fake memory; no physical NX809J or procfs runtime path is exercised.
