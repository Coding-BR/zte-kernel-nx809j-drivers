...
static void awake_callback(long long device)
{
	callback_calls++;
						...
						size_t available)
{
	...
	*offset += (__int64)length;
	return (ssize_t)length;
}

#include "../../../reconstructed/zte_tpd/get_screen_off_awake.c"
			  ...
			  unsigned int value, loff_t *offset)
{
	...
	*offset = 0;
	if (callback_enabled)
		*(void (**)(long long))(device + 0xfd8) = awake_callback;
	else
		*(void (**)(long long))(device + 0xfd8) = NULL;
	*(unsigned int *)(device + 1156) = value;
...
int main(void)
{
	...
	result = get_screen_off_awake(NULL, output, sizeof(output), &offset);
	if (check(result == 2 && strcmp(output, "7\n") == 0 && offset == 2 &&
			  callback_calls == 1 && callback_device == tpd_cdev,
			  "first read and awake callback"))
		return 1;
	...
	result = get_screen_off_awake(NULL, output, 2, &offset);
	...
	puts("PASS get_screen_off_awake: 4 cases");
	return 0;
}
