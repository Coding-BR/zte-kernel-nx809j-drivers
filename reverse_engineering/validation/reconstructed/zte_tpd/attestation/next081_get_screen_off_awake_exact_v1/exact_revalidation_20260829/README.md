# `get_screen_off_awake` exact revalidation

- Target: `get_screen_off_awake` at stock entry `0x0010d240`.
- Stock body: 240 bytes / 60 AArch64 instructions.
- The C reconstruction was corrected from callback offset `0xe20` to the stock offset `0xfd8`; the Docker exact assembly source remains included as the canonical code-generation input.
...
- This target is distinct from the internal `tpd_get_screen_off_awake` helper.
