#!/usr/bin/env python3
"""Build and run the direct tpd_zlog_init contract in Docker twice."""
...
EXPECTED_STDOUT = "PASS tpd_zlog_init host tests (4 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/tpd_zlog_init.c"
)
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target007",
                   "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_zlog_init_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_zlog_init_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_zlog_init",
              "covered_functions": ["tpd_zlog_init"],
              ...
              "limitations": [
                  "tpd_zlog_device and jiffies are contract stubs.",
                  "The harness verifies buffer/flag clearing, reset timer and all 19 log timer entries.",
