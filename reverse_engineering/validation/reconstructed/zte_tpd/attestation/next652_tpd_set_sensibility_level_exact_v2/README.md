# `tpd_set_sensibility_level` offline exact attestation

This package records the function-level reconstruction of
`tpd_set_sensibility_level` at stock entry `0x0012c1a8` for the ZTE NX809J /
RedMagic 11 Pro+ target.

## Decision

`PROMOTED_OFFLINE_EXACT` at function level. The candidate matches the stock
function at 48 AArch64 instructions and 192 bytes, including 13 relocation
sites and the stock `.rodata.str1.1` strings. KCFI type ID `0xe1d63dc1` and
symbol size match exactly.

The exact assembly source is emitted by `tpd_set_sensibility_level_exact.S`;
its sparse stock string bytes are kept in `tpd_set_sensibility_level_exact.inc`.
