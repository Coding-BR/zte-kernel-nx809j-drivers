# Exact revalidation: 033_tp_glove_read

This package records the Docker-based revalidation of `tp_glove_read` at stock entry `00109db8`.
...
The Joern CPG uses compatibility defines `__user`, `__init` and `__exit`, and excludes `validation` and `build`. The host harness validates callback offset `+0xe48`, value offset `+0x8`, partial reads and EOF using fake memory; no physical NX809J or procfs runtime path is exercised.
