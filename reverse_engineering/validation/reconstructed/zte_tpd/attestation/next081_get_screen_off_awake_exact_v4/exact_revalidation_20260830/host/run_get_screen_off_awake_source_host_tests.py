#!/usr/bin/env python3
"""Run the direct get_screen_off_awake source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS get_screen_off_awake: 4 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "get_screen_off_awake_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "get_screen_off_awake.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("get_screen_off_awake source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-Wl,--build-id=none", "-frandom-seed=zte-tpd-get-screen-off-awake-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/get_screen_off_awake_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "get_screen_off_awake", "mode": "offline_direct_source_asan_ubsan",
              ...
              "limitations": ["The harness includes the reconstructed C source and uses fake device memory.",
