#!/usr/bin/env python3
"""Build and run the testing-directory harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_testing_create_dir host tests (4 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		...
		default=pathlib.Path(__file__).with_name(
			"syna_testing_create_dir_host_test.c"
		),
	)
	...
	driver_source = (
		...
		/ "reconstructed"
		/ "zte_tpd"
		/ "syna_testing_create_dir.c"
	)
	layout_header = (
		source.parents[3]
		/ "reconstructed"
		/ "zte_tpd"
		/ "zte_tpd_syna_layout.h"
	)
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-fno-pie",
			"-no-pie",
			"-frandom-seed=zte-tpd-next50-testing-create",
	...
	report = {
		...
		"mode": "offline_direct_source_next50_testing_create_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_testing_create_dir",
		"covered_functions": ["syna_testing_create_dir"],
		"source": str(source),
		...
		"limitations": [
			"The harness stubs kobject, sysfs and printk APIs.",
			"It proves offsets, branches, call order, cleanup, arguments, returns and logs.",
