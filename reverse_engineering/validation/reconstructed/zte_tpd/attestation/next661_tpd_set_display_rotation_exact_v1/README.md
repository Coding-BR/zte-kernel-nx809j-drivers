# `tpd_set_display_rotation` offline exact attestation

This package records the function-level reconstruction of
`tpd_set_display_rotation` at stock entry `0x0012c50c` for the ZTE NX809J /
RedMagic 11 Pro+ target.

## Decision

`PROMOTED_OFFLINE_EXACT` at function level. The candidate matches the stock
function at 111 AArch64 instructions and 444 bytes, including relocation sites
and the stock `.rodata.str1.1` strings. KCFI type ID `0x6af8b499` and symbol
size match exactly.

The exact assembly source is emitted by `tpd_set_display_rotation_exact.S`;
