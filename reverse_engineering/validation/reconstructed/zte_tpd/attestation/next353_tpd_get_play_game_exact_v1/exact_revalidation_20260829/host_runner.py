#!/usr/bin/env python3
"""Build and run the tpd_get_play_game contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_get_play_game contract test (4 checks)\n"
...
def main() -> int:
    ...
    test_source = runner.with_name("tpd_get_play_game_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_play_game.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_play_game_contract_asan_ubsan"
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-get-play-game", "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_get_play_game_contract_test.c", "-o", f"/output/{binary.name}",
        ]
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_play_game_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_get_play_game", "function": "tpd_get_play_game",
