#!/usr/bin/env python3
"""Run the direct set_play_game source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS set_play_game source host tests (3 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "set_play_game_source_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "set_play_game.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("set_play_game source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-set-play-game-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/set_play_game_source_host_test.c",
    ...
    report = {
        ...
        "limitations": [
            "The harness includes the reconstructed set_play_game.c and uses fake device memory, parser, callback and printk helpers.",
