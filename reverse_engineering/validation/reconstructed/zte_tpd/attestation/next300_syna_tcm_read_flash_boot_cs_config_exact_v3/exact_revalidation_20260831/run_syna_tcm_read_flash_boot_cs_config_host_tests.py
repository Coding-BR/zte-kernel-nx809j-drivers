#!/usr/bin/env python3
"""Run the syna_tcm_read_flash_boot_cs_config host contract twice with sanitizers."""
...
HOST_SOURCE = pathlib.Path(
    ...
    "syna_tcm_read_flash_boot_cs_config_host_test.c"
)
DRIVER_SOURCE = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/"
    "syna_tcm_read_flash_boot_cs_config.c"
)
...
def main() -> int:
    ...
    compile_command = (
        ...
        "syna_tcm_read_flash_boot_cs_config_host_test.c "
        "-o /out/syna_tcm_read_flash_boot_cs_config_host_test && "
        "/out/syna_tcm_read_flash_boot_cs_config_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "syna_tcm_read_flash_boot_cs_config_host_test"
    ...
    report = {"schema_version": 1, "target": "syna_tcm_read_flash_boot_cs_config",
              "mode": "direct_source_asan_ubsan_host_contract",
              "source": str(repo_root / HOST_SOURCE),
              "source_sha256": sha256(repo_root / HOST_SOURCE),
              ...
              "inputs": [
                  ...
                  {"path": HOST_SOURCE.as_posix(),
                   "sha256": sha256(repo_root / HOST_SOURCE)},
              ],
              "expected_cases": 8,
