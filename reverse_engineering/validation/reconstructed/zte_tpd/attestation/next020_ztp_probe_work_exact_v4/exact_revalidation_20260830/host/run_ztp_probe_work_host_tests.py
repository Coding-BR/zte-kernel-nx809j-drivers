#!/usr/bin/env python3
"""Build and run the ztp_probe_work host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS ztp_probe_work host tests (3 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target382-ztp-probe-work",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/ztp_probe_work_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_ztp_probe_work_asan_ubsan",
              "driver": "zte_tpd", "target": "ztp_probe_work",
              "covered_functions": ["ztp_probe_work"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 3,
              ...
              "limitations": [
                  "printk and syna_dev_module_init are stubbed; the harness checks call order/count and the format contract.",
                  "No workqueue scheduler, Synaptics transport or physical hardware is exercised.",
