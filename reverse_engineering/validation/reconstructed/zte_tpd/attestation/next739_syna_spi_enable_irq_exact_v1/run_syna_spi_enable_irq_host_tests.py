#!/usr/bin/env python3
"""Build and run the syna_spi_enable_irq harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_spi_enable_irq host tests (6 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(__file__).with_name(
            "syna_spi_enable_irq_host_test.c"
        ),
    )
    ...
    driver_dir = source.parents[3] / "reconstructed" / "zte_tpd"
    driver_source = driver_dir / "syna_spi_enable_irq.c"
    layout_header = driver_dir / "zte_tpd_syna_layout.h"
    build_root = args.build_root.resolve()
    output = args.output.resolve()
    for path in (source, driver_source, layout_header):
        if not path.is_file():
    ...
    inputs = [source, driver_source, layout_header]
    report = {
        "schema_version": 1,
        "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
        "mode": "offline_direct_source_next20_irq_asan_ubsan",
        "target": "syna_spi_enable_irq",
        "source": str(source),
        ...
        "limitations": [
            "The harness stubs printk, mutex and Linux IRQ primitives.",
            "It proves branch, lock, argument, return, state and log behavior.",
            "Assembly, KCFI and Ghidra equivalence remain independent gates.",
            "No smartphone, ADB, fastboot, IRQ controller or hardware is used.",
        ],
    }
