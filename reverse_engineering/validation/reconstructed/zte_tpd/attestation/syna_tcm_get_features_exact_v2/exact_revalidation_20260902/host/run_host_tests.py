#!/usr/bin/env python3
"""Run the syna_tcm_get_features host contract twice with sanitizers."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "-Wl,--build-id=none "
        "/drivers/reverse_engineering/validation/reconstructed/zte_tpd/attestation/"
        "syna_tcm_get_features_exact_v2/exact_revalidation_20260902/host/host_test.c "
        "-o /out/syna_tcm_get_features_host_test && /out/syna_tcm_get_features_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "syna_tcm_get_features_host_test"
