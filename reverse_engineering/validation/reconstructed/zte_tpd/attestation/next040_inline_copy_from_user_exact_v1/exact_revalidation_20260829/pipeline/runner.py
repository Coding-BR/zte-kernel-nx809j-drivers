#!/usr/bin/env python3
"""Run the isolated host contract harness for _inline_copy_from_user."""
...
EXPECTED_STDOUT = "PASS _inline_copy_from_user host tests (5 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "inline_copy_from_user_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "_inline_copy_from_user.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("inline copy harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-inline-copy-from-user",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/inline_copy_from_user_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_inline_copy_from_user_asan_ubsan",
        "driver": "zte_tpd",
        "target": "_inline_copy_from_user",
        "covered_functions": ["_inline_copy_from_user"],
        ...
        "limitations": [
            "The harness replaces privileged TTBR/DAIF behavior with deterministic usercopy contracts.",
            "It does not execute privileged AArch64 system-register instructions or hardware paths.",
