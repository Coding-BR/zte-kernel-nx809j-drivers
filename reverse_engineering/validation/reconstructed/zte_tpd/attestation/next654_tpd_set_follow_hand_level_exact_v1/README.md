# `tpd_set_follow_hand_level` offline exact attestation

This package records the function-level reconstruction of
`tpd_set_follow_hand_level` at stock entry `0x0012c288` for the ZTE NX809J /
...
The exact assembly source is emitted by `tpd_set_follow_hand_level_exact.S`;
its sparse stock string bytes are kept in `tpd_set_follow_hand_level_exact.inc`.
