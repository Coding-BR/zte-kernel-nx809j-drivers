#!/usr/bin/env python3
"""Build and run the tpd_test_cmd_show contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_test_cmd_show contract test (3 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_test_cmd_show_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_test_cmd_show.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_test_cmd_show_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-test-cmd-show",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_test_cmd_show_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_test_cmd_show_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_test_cmd_show", "function": "tpd_test_cmd_show",
        ...
        "limitations": [
            "The harness validates formatting and printk invocation with fake output memory.",
            "It does not exercise the real kernel buffer, transport, IRQ, MMIO or module loading.",
        ],
    }
