...
static int kstrtouint_from_user(__int64 buffer, __int64 count, int base,
						unsigned int *value)
{
	char local[32];
	char *end;
...
static void awake_callback(long long device, int value)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/set_screen_off_awake.c"
...
static void reset(unsigned char *device, int callback_enabled)
{
	...
	parse_failure = 0;
	if (callback_enabled)
		*(void (**)(long long, int))(device + 0xfe0) = awake_callback;
	else
		*(void (**)(long long, int))(device + 0xfe0) = NULL;
...
int main(void)
{
	...
	reset(device, 1);
	result = set_screen_off_awake(NULL, "2", 1, &offset);
	if (check(result == 1 && callback_calls == 1 && callback_value == 1,
			  "nonzero input normalized"))
		return 1;

	puts("PASS set_screen_off_awake: 4 cases");
	return 0;
}
