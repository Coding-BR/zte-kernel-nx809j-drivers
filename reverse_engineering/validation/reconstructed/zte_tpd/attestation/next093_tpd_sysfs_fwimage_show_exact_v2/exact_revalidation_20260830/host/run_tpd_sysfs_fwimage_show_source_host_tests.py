#!/usr/bin/env python3
"""Run the direct tpd_sysfs_fwimage_show source contract twice with ASan/UBSan."""
...
EXPECTED = "tpd_sysfs_fwimage_show host contract: PASS (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tpd_sysfs_fwimage_show_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tpd_sysfs_fwimage_show.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tpd_sysfs_fwimage_show source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-fwimage-show-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tpd_sysfs_fwimage_show_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tpd_sysfs_fwimage_show", "mode": "offline_direct_source_asan_ubsan",
              ...
              "limitations": ["The harness includes the reconstructed C source and uses userspace stubs.",
                              "It does not execute a real kernel sysfs path or NX809J hardware.",
