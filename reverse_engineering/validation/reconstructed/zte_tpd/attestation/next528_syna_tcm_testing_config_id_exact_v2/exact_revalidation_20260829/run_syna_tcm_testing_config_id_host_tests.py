#!/usr/bin/env python3
"""Run the direct syna_tcm_testing_config_id host contract under sanitizers."""
...
EXPECTED = "syna_tcm_testing_config_id host oracle: PASS (5 groups)\n"
...
def main() -> int:
    ...
    source = repo_root / "kernel_development/drivers/validation/zte_tpd/host/syna_tcm_testing_config_id_host_test.c"
    driver_source = repo_root / "kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_testing_config_id.c"
    ...
    for cycle in (1, 2):
        ...
        binary = cycle_root / "config_id_host_test"
        command = [
            ...
            f"/drivers/{source.relative_to(repo_root).as_posix()}", "-o", "/out/config_id_host_test",
        ]
        compiled = subprocess.run(command, text=True, capture_output=True, check=False)
        executed = subprocess.run(
            ["docker", "run", "--rm", "-v", f"{cycle_root}:/out:ro", IMAGE, "/out/config_id_host_test"],
    ...
    report = {
        ...
        "target": "syna_tcm_testing_config_id",
        ...
        "limitations": ["The harness isolates the reconstructed function and stubs its proven collaborators.", "No Android device or physical touch controller was exercised."],
    }
