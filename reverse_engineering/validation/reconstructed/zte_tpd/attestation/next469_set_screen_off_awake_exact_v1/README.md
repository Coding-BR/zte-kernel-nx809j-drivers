# `set_screen_off_awake` offline exact attestation

This package records the reconstruction of stock `zte_tpd` function
`set_screen_off_awake` at entry `0x0010d334` for the ZTE NX809J / RedMagic 11
Pro+.
...
The candidate object is linked from `set_screen_off_awake_exact.S`. It preserves
the stock PAC/stack-protector sequence, `kstrtouint_from_user` parsing,
nonzero-to-one normalization, log format, callback offset `0xfe0` from
`tpd_cdev`, and callback guard `0x6af8b499`. `candidate_source.c` is the
...
operations, callback behavior, firmware transport, and physical behavior are
represented or deferred; smartphone execution is intentionally outside this
attestation.
...
- Exact linked Assembly source: `set_screen_off_awake_exact.S`
