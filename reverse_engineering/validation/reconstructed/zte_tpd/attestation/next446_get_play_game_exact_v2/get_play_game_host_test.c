...
static long get_play_game_contract(struct fake_cdev *cdev, char *buffer,
...
int main(void)
{
    ...
    *(uint32_t *)(cdev.bytes + 0x45c) = 1;
    callback_count = 0;
    result = get_play_game_contract(&cdev, buffer, sizeof(buffer), &offset,
                                    fake_callback);
    if (check(result == 2 && strcmp(buffer, "1\n") == 0,
              "initial value read"))
        return 1;
    ...
    offset = 0;
    *(uint32_t *)(cdev.bytes + 0x45c) = 0;
    result = get_play_game_contract(&cdev, buffer, 2, &offset, NULL);
    if (check(result == 2 && strncmp(buffer, "0\n", 2) == 0,
              "zero value short read"))
        return 1;

    offset = 0;
    *(uint32_t *)(cdev.bytes + 0x45c) = UINT32_MAX;
    memset(buffer, 0, sizeof(buffer));
    result = get_play_game_contract(&cdev, buffer, sizeof(buffer), &offset,
                                    NULL);
    ...
    puts("PASS get_play_game host tests (4 cases)");
    return 0;
}
