#!/usr/bin/env python3
"""Run the syna_tcm_update_cs_config host contract twice with sanitizers."""
...
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_update_cs_config.c"
)
DRIVER_SOURCE_ASM = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_update_cs_config_exact.S"
)
...
def main() -> int:
    ...
    repo_root = source_file.parents[5]
    ...
    compile_command = (
        ...
        "syna_tcm_update_cs_config_host_test.c "
        "-o /out/syna_tcm_update_cs_config_host_test && "
        "/out/syna_tcm_update_cs_config_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "syna_tcm_update_cs_config_host_test"
    ...
    report = {"schema_version": 1,
              "target": "syna_tcm_update_cs_config",
              ...
              "expected_cases": 11, "cycles": cycles,
