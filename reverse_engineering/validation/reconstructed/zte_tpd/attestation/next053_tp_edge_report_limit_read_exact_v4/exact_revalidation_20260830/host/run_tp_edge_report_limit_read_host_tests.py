#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for tp_edge_report_limit_read."""
...
EXPECTED_STDOUT = "PASS tp_edge_report_limit_read host tests (5 cases)\n"
EXPECTED_STDERR = "edge report allocation failed 0 0\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tp_edge_report_limit_read_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tp_edge_report_limit_read.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("edge report harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        passed = (
            ...
            and execute["stdout"] == EXPECTED_STDOUT
            and execute["stderr"] == EXPECTED_STDERR
        )
    ...
    report = {
        ...
        "mode": "offline_direct_source_tp_edge_report_limit_read_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tp_edge_report_limit_read",
        "covered_functions": ["tp_edge_report_limit_read"],
        ...
        "limitations": [
            "Kernel allocation and procfs copy helpers are deterministic host stubs.",
            "The harness checks output fields and recovered cdev offsets without hardware.",
            "Assembly, KCFI, Ghidra, Joern, and whole-module gates remain separate.",
        ],
    }
