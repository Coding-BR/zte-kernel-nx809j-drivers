#!/usr/bin/env python3
"""Build and run the tpd_get_stability_level contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_get_stability_level contract test (4 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_get_stability_level_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_stability_level.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_stability_level_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-get-stability-level",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_get_stability_level_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_stability_level_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_get_stability_level", "function": "tpd_get_stability_level",
        ...
        "limitations": [
            "The harness validates the stability-level state-to-device mirror transfer with fake memory.",
