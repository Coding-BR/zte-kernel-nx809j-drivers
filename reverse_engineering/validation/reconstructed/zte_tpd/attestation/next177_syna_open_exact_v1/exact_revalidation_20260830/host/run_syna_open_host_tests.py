#!/usr/bin/env python3
"""Build and run the syna_open host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_open host tests (2 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_open_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_open.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-syna-open", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_open_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_open_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_open",
              "covered_functions": ["syna_open", "printk"],
              ...
              "limitations": ["The harness uses deterministic stubs for printk and does not exercise VFS concurrency.",
                              "No smartphone, IRQ, MMIO, firmware transport or hardware is exercised."]}
