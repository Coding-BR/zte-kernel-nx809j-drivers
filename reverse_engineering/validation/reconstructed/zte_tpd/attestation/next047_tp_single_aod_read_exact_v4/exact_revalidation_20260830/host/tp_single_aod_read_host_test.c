...
static int refresh_single_aod(struct ztp_device *cdev)
{
  callback_calls++;
...
#include "../../../reconstructed/zte_tpd/tp_single_aod_read.c"

struct fixture {
  uint8_t device[0xec8 + sizeof(void *)];
};
                         ...
                         int with_callback)
{
  ...
  *(uint32_t *)(fixture->device + 0x450) = value;
  store_ptr(fixture->device, 0xec0, (void *)wrong_callback);
  store_ptr(fixture->device, 0xec8,
            with_callback ? (void *)refresh_single_aod : NULL);
...
static void test_zero_offset_refreshes_and_formats_value(void)
{
  ...
  expect(tp_single_aod_read(&file, output, sizeof(output), &offset) == 3,
...
static void test_nonzero_offset_has_no_side_effects(void)
{
  ...
  expect(tp_single_aod_read(&file, output, sizeof(output), &offset) == 0,
...
static void test_zero_offset_without_callback_still_reads(void)
{
  ...
  expect(tp_single_aod_read(&file, output, sizeof(output), &offset) == 2,
...
int main(void)
{
  ...
  puts("PASS tp_single_aod_read host tests (3 cases)");
  return 0;
}
