# `tp_single_aod_read` — offline exact revalidation

- Driver: `zte_tpd`
- Stock entry: `0x0010ac28`
- Stock body: 60 AArch64 instructions / 240 bytes
- Candidate source: `candidate_source.c` (`tp_single_aod_read.c`)
...
The function performs work only at file offset zero, invokes the callback at `tpd_cdev + 0xec8` when present, logs the AOD value at `tpd_cdev + 0x450`, formats it with the stock ten-byte bound, and delegates the userspace copy/offset update to `simple_read_from_buffer`.
