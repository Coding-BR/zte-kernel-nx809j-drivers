#!/usr/bin/env python3
"""Build and run the syna_dev_resume host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_resume host tests (6 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_resume_contract_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = source.parents[5] / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "syna_dev_resume.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-D", "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target370-resume",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_resume_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_syna_dev_resume_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_resume",
              "covered_functions": ["syna_dev_resume", "syna_tcm_reset",
                                     "syna_tcm_get_event_data", "syna_dev_set_up_app_fw",
                                     "syna_tcm_set_dynamic_config", "syna_dev_set_charger_mode",
                                     "syna_dev_set_display_rotation", "syna_dev_set_play_game",
                                     "syna_dev_set_tp_report_rate", "syna_dev_set_sensibility_level",
                                     "syna_dev_set_follow_hand_level", "syna_dev_set_stability_level"],
              "source": str(source), "source_sha256": sha256(source),
              "driver_source": str(driver_source),
              "driver_source_sha256": sha256(driver_source), "expected_cases": 6,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "Callbacks, firmware transport, completion, sleep and user-space events are stubbed; no physical state machine is exercised.",
                  "No smartphone, SPI, IRQ, firmware blob, power rail or physical hardware is exercised."]}
