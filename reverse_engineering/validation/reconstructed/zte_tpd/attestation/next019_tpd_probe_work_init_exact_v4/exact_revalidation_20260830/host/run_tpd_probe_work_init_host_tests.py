#!/usr/bin/env python3
"""Build and run the tpd_probe_work_init host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_probe_work_init host tests (2 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target381-probe-work-init",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none", "-Wno-int-conversion",
                   "/drivers/validation/zte_tpd/host/tpd_probe_work_init_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_probe_work_init_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_probe_work_init",
              "covered_functions": ["tpd_probe_work_init"],
              ...
              "limitations": [
                  "printk and init_timer_key are stubbed.",
                  "No timer scheduler, workqueue or physical hardware is exercised.",
