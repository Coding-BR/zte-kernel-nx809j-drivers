#!/usr/bin/env python3
"""Build and run the zte_touch_remove host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS zte_touch_remove host tests (3 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		"--source",
		type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("zte_touch_remove_host_test.c"),
	)
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "zte_touch_remove.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie",
			"-no-pie",
			"-frandom-seed=zte-tpd-next80-zte-touch-remove",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined",
			"-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/zte_touch_remove_host_test.c",
			"-o",
	...
	report = {
		...
		"mode": "offline_direct_source_next80_zte_touch_remove_asan_ubsan",
		"driver": "zte_tpd",
		"target": "zte_touch_remove",
		"covered_functions": ["zte_touch_remove"],
		"source": str(source),
		...
		"limitations": [
			"The harness proves the exact log arguments and one zte_touch_deinit call.",
