#!/usr/bin/env python3
"""Build and run the tpd_workqueue_deinit host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_workqueue_deinit host tests (2 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "tpd_workqueue_deinit_contract_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "tpd_workqueue_deinit.c"
    if not driver_source.is_file():
        parser.error(f"driver source does not exist: {driver_source}")
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "ZTE_TPD_HOST_TEST",
                   "-frandom-seed=zte-tpd-target386-workqueue-deinit",
                   "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/"
                   "tpd_workqueue_deinit_contract_test.c", "-o",
                   f"/output/{binary_name}"]
        compiled = execute(command)
        run = (execute(["docker", "run", "--rm", "-v",
                        f"{cycle_root}:/output:ro", IMAGE,
                        f"/output/{binary_name}"])
               if compiled.returncode == 0 else None)
        passed = bool(run and run.returncode == 0 and
                      run.stdout == EXPECTED_STDOUT and run.stderr == "")
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_workqueue_deinit_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_workqueue_deinit",
              "covered_functions": ["tpd_workqueue_deinit"],
              ...
              "expected_cases": 2, "repetitions": 2, "cycles": cycles,
              "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "printk, tpd_report_work_deinit, tpd_resume_work_deinit, "
                  "cancel_delayed_work_sync and vfree are stubbed.",
                  "The harness checks source-level teardown order and offsets; "
                  "kernel scheduler behavior is not executed.",
                  "No smartphone runtime test was performed."]}
