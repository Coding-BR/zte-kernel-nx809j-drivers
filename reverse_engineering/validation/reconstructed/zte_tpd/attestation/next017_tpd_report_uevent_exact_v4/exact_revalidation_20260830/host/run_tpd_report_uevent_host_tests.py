#!/usr/bin/env python3
"""Build and run the tpd_report_uevent host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_report_uevent host tests (5 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-fno-omit-frame-pointer", "-fno-pie", "-no-pie", "-Wno-int-conversion", "-D",
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target379-uevent",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_report_uevent_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_report_uevent_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_report_uevent",
              "covered_functions": ["tpd_report_uevent"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 5,
              ...
              "limitations": [
                  "SP_EL0, printk and kobject_uevent_env are stubbed; the harness checks branch, environment and offsets.",
                  "No smartphone, kernel uevent subsystem or physical hardware is exercised."]}
