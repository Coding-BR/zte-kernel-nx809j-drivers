#!/usr/bin/env python3
"""Run the syna_sysfs_reset_store contract under ASan and UBSan."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_sysfs_reset_store_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_sysfs_reset_store.c")
EXACT_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_sysfs_reset_store_exact.S")
...
EXPECTED = "PASS syna_sysfs_reset_store host tests (8 cases)\n"
...
def main() -> int:
    ...
    exact_source = REPO_ROOT / EXACT_SOURCE
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-Wno-error=uninitialized", "-fno-omit-frame-pointer",
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next702-syna-sysfs-reset-store",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_sysfs_reset_store_asan_ubsan",
        "driver": "zte_tpd", "target": "syna_sysfs_reset_store",
        ...
        "inputs": [
            ...
            {"path": str(exact_source), "size": exact_source.stat().st_size, "sha256": sha256(exact_source)},
        ],
        "limitations": [
            "The harness stubs kstrtouint, printk, syna_tcm_reset, syna_tcm_get_event_data and the hardware callback.",
            "It proves branch returns, offsets and side-effect ordering without a real kernel kobject or touch controller.",
            "Assembly, relocation, KCFI and Ghidra equivalence are independent gates.",
        ],
    }
