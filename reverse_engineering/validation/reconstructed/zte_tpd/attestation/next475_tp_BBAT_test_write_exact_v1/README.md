# `tp_BBAT_test_write` offline attestation

This package records the complete offline reconstruction evidence for the ZTE NX809J `zte_tpd` function at stock entry `0x0010db98`.
...
`OFFLINE_EXACT` means that the available stock binary evidence is matched at function level. It does not claim equivalent Android runtime behavior, SPI/touch hardware behavior, or boot safety. Hardware validation remains deferred.
