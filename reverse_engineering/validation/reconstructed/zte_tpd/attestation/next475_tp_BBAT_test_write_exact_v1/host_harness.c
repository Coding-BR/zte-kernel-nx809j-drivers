...
#include "../../../reconstructed/zte_tpd/tp_BBAT_test_write.c"
			       ...
			       size_t_kernel count, ssize_t expected)
{
	...
	const ssize_t result = tp_BBAT_test_write(NULL, buffer, count, NULL);
...
int main(void)
{
	...
	(void)printf("tp_BBAT_test_write host contract: PASS (%u printk calls)\n",
		     printk_calls);
	return 0;
}
