#!/usr/bin/env python3
"""Build and run the tpd_get_singlefpgesture contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_get_singlefpgesture contract test (3 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_get_singlefpgesture_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_singlefpgesture.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_singlefpgesture_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-get-singlefpgesture",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_get_singlefpgesture_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_singlefpgesture_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_get_singlefpgesture", "function": "tpd_get_singlefpgesture",
        ...
        "expected_cases": 3, "repetitions": 2, "sanitizers": ["address", "undefined"],
        "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness validates the observed state read and destination store with fake memory.",
            "It does not exercise kernel printk, transport, IRQ, MMIO, flash hardware or module loading.",
        ],
    }
