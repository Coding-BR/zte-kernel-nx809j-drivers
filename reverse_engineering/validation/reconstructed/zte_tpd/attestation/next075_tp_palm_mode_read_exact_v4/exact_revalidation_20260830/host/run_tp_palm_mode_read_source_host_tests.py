#!/usr/bin/env python3
"""Run the direct tp_palm_mode_read source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_palm_mode_read: 3 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_palm_mode_read_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_palm_mode_read.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_palm_mode_read source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-palm-mode-read-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_palm_mode_read_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_palm_mode_read", "mode": "offline_direct_source_asan_ubsan",
