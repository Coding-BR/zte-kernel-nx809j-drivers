#!/usr/bin/env python3
"""Build and run the firmware position reset harness twice with sanitizers."""
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("tpd_reset_fw_data_pos_and_size_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()

	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "tpd_reset_fw_data_pos_and_size.c"
	...
	for cycle in (1, 2):
		...
		command = [args.compiler, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra",
			...
			"-frandom-seed=zte-tpd-next64-reset-fw-data-pos-size",
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next64_tpd_reset_fw_data_pos_and_size_asan_ubsan",
		"driver": "zte_tpd", "target": "tpd_reset_fw_data_pos_and_size",
		"covered_functions": ["tpd_reset_fw_data_pos_and_size"], "source": str(source),
		...
		"limitations": ["The harness directly exercises the observed descriptor store and position reset.",
