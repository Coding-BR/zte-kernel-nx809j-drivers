...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "tpd_workqueue_init_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target384-workqueue-init",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_workqueue_init_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_workqueue_init_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_workqueue_init",
              "covered_functions": ["tpd_workqueue_init"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 4,
              ...
              "limitations": [
                  "alloc_workqueue, destroy_workqueue, printk, timers and dependent work initializers are stubbed.",
                  "The harness verifies branch contracts, offsets, callbacks and all external-call arguments but does not run a kernel workqueue.",
