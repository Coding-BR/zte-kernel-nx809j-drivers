...
#include "../../../reconstructed/zte_tpd/get_one_key.c"
                         ...
                         int with_callback)
{
    ...
    store_ptr(fixture->device, 0xd30, (void *)stale_callback);
    store_ptr(fixture->device, 0xd38,
              with_callback ? (void *)fake_callback : NULL);
    *(uint32_t *)(fixture->device + 0x458) = value;
...
static void test_full_read_and_callback(void)
{
    ...
    result = get_one_key(&file, output, sizeof(output), &offset);
...
static void test_eof_does_not_repeat_callback(void)
{
    ...
    fixture_init(&fixture, 7, 1);
    expect(get_one_key(&file, output, sizeof(output), &offset) == 0,
           name, "EOF result differs");
    expect(callback_calls == 0 && printk_calls == 0, name,
           "side effects occurred after EOF");
}

static void test_short_buffer_and_maximum_value(void)
{
    const char *name = "short_buffer_and_maximum_value";
    struct fixture fixture;
    struct file file = {0};
    loff_t offset = 0;
    char output[32] = {0};
    ...
    expect(get_one_key(&file, output, 5, &offset) == 5, name,
           "short read length differs");
    expect(strncmp(output, "42949", 5) == 0, name,
           "short read data differs");
    offset = 0;
    memset(output, 0, sizeof(output));
    expect(get_one_key(&file, output, sizeof(output), &offset) == 11,
...
int main(void)
{
    ...
    test_short_buffer_and_maximum_value();
    puts("PASS get_one_key source host tests (3 cases)");
    return 0;
}
