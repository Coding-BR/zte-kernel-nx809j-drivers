#!/usr/bin/env python3
"""Run the direct get_one_key source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS get_one_key source host tests (3 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "get_one_key_source_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "get_one_key.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("get_one_key source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-get-one-key-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/get_one_key_source_host_test.c",
    ...
    report = {
        ...
        "limitations": [
            "The harness includes the reconstructed get_one_key.c and uses fake device memory, callback and procfs helpers.",
            "It does not execute a real procfs path or NX809J hardware.",
