#!/usr/bin/env python3
"""Run the offline ASan/UBSan contract harness for syna_dev_set_up_input_device."""
...
def main() -> int:
    ...
    harness = root / "kernel_development" / "drivers" / "validation" / "zte_tpd" / "host" / "syna_dev_set_up_input_device_host_test.c"
    source = root / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "syna_dev_set_up_input_device.c"
    evidence_root = root / "reverse_engineering" / "validation" / "reconstructed" / "zte_tpd" / "offline_static"
    evidence = [
        evidence_root / "ghidra_stock" / "decompiled" / "0166_001136c4_syna_dev_set_up_input_device.c",
        evidence_root / "ghidra_stock" / "pcode" / "0166_001136c4_syna_dev_set_up_input_device.jsonl",
        evidence_root / "stock_assembly" / "0166_syna_dev_set_up_input_device_001136c4.asm",
    ]
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-target167-input-device",
            "-ffile-prefix-map=/repo=<repo>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/repo/kernel_development/drivers/validation/zte_tpd/host/syna_dev_set_up_input_device_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_contract_model_input_setup_asan_ubsan",
        "driver": "zte_tpd",
        ...
        "limitations": [
            "The harness models the observed source-level contract with deterministic host state; it does not execute kernel APIs.",
            "It does not prove electrical MMIO behavior, Android input-core integration, or device boot behavior.",
