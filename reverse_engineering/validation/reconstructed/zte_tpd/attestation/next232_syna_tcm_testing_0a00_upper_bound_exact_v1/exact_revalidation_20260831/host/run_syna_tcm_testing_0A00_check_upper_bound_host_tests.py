#!/usr/bin/env python3
"""Build and run the 0A00 upper-bound checker harness twice."""
...
EXPECTED = "PASS syna_tcm_testing_0A00_check_upper_bound host tests (3 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path, default=pathlib.Path(__file__).with_name(
        "syna_tcm_testing_0A00_check_upper_bound_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "syna_tcm_testing_0A00_check_upper_bound.c"
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_0A00_upper_bound_asan_ubsan", "driver": "zte_tpd",
              "target": "syna_tcm_testing_0A00_check_upper_bound",
              "covered_functions": ["syna_tcm_testing_0A00_check_upper_bound"],
