#!/usr/bin/env python3
"""Run reproducible ASan/UBSan tests for tp_edge_report_limit_write."""
...
EXPECTED_STDOUT = "PASS tp_edge_report_limit_write host tests (5 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tp_edge_report_limit_write_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tp_edge_report_limit_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("edge report write harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-edge-report-limit-write",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tp_edge_report_limit_write_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tp_edge_report_limit_write_asan_ubsan",
        "driver": "zte_tpd", "target": "tp_edge_report_limit_write",
        "covered_functions": ["tp_edge_report_limit_write"],
        ...
        "driver_source_sha256": sha256_file(source),
        ...
        "limitations": [
            "User-copy, parser and printk helpers are deterministic host stubs.",
            "The harness checks recovered offsets, conversions, return values and error paths without hardware.",
