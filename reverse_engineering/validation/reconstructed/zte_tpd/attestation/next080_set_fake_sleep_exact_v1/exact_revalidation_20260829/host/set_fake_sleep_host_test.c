...
static void sleep_callback(long long device, int value)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/set_fake_sleep.c"
...
static void reset(unsigned char *device, int callback_enabled)
{
	...
	parse_failure = 0;
	if (callback_enabled)
		*(void (**)(long long, int))(device + 0xfd0) = sleep_callback;
	else
		*(void (**)(long long, int))(device + 0xfd0) = NULL;
