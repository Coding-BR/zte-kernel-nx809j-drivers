#!/usr/bin/env python3
"""Run the tp_wake_gesture_read host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_wake_gesture_read host tests (2 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "tp_wake_gesture_read_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "tp_wake_gesture_read.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-frandom-seed=zte-tpd-tp-wake-gesture-read",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/tp_wake_gesture_read_host_test.c",
		...
		passed = bool(
			...
			and executed.stdout == EXPECTED and executed.stderr == ""
			and binary.is_file()
		)
		cycles.append({
			"cycle": cycle,
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe28 callback offset, value read, partial read and EOF contract with fake memory.",
