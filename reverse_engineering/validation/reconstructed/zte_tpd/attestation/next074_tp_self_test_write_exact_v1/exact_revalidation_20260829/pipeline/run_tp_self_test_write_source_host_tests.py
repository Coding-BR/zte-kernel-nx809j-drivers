#!/usr/bin/env python3
"""Run the direct tp_self_test_write source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_self_test_write: 3 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_self_test_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_self_test_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_self_test_write source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-self-test-write-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_self_test_write_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_self_test_write", "mode": "offline_direct_source_asan_ubsan",
              ...
              "limitations": ["The harness includes the reconstructed C source and uses fake device memory and kernel helpers.",
