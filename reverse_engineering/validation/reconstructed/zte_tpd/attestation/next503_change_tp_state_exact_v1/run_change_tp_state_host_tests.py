#!/usr/bin/env python3
"""Build and run the change_tp_state host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS change_tp_state host tests (5 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("change_tp_state_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next118-change-state", "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/change_tp_state_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_change_tp_state_asan_ubsan",
        "driver": "zte_tpd",
        "target": "change_tp_state",
        "covered_functions": ["change_tp_state"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers valid LCD transitions, ignored transitions, completion reset, queue offsets, lock cleanup and invalid input cleanup.",
            "No smartphone, input subsystem scheduler, firmware transport or physical hardware is exercised.",
        ],
    }
