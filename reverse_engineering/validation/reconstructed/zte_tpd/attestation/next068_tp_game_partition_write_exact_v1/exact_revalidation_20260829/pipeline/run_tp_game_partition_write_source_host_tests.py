#!/usr/bin/env python3
"""Run the direct tp_game_partition_write source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS tp_game_partition_write: 5 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_game_partition_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_game_partition_write.c"
    exact_source = drivers / "reconstructed" / "zte_tpd" / "tp_game_partition_write_exact.S"
    if not harness.is_file() or not source.is_file() or not exact_source.is_file():
        raise FileNotFoundError("tp_game_partition_write source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-game-partition-write-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tp_game_partition_write_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_game_partition_write",
              "mode": "offline_direct_source_asan_ubsan", "source": str(source),
              "source_sha256": sha256(source), "driver_source_sha256": sha256(source),
              "candidate_source": str(exact_source),
              "candidate_source_sha256": sha256(exact_source), "harness": str(harness),
              "harness_sha256": sha256(harness), "compiler": CLANG,
              "container_image": IMAGE, "toolchain_volume": TOOLCHAIN_VOLUME,
              "sanitizers": ["address", "undefined"], "expected_cases": 5,
              ...
              "limitations": ["The harness includes the reconstructed C source and uses fake device memory, user-copy and callback metadata.",
