#!/usr/bin/env python3
"""Run the direct tp_sensibility_level_write source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS tp_sensibility_level_write: 6 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_sensibility_level_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_sensibility_level_write.c"
    exact_source = drivers / "reconstructed" / "zte_tpd" / "tp_sensibility_level_write_exact.S"
    if not harness.is_file() or not source.is_file() or not exact_source.is_file():
        raise FileNotFoundError("tp_sensibility_level_write source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-sensibility-level-write-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tp_sensibility_level_write_host_test.c",
    ...
    report = {
        "schema_version": 1,
        "driver": "zte_tpd",
        "target": "tp_sensibility_level_write",
        ...
        "limitations": [
            "The harness includes the reconstructed C source and uses fake device memory, user-copy and callbacks.",
