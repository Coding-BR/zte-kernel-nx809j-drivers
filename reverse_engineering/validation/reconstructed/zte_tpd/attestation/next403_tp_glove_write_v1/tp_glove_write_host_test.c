...
static int enable_glove(struct ztp_device *cdev, int value)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/tp_glove_write.c"

struct fixture {
	uint8_t device[0xe50 + sizeof(void *)];
};
...
static void fixture_init(struct fixture *fixture, int with_callback)
{
	...
	store_ptr(fixture->device, 0xe48, (void *)wrong_callback);
	if (with_callback)
		store_ptr(fixture->device, 0xe50, (void *)enable_glove);
	else
		store_ptr(fixture->device, 0xe50, NULL);
...
static void test_valid_write_normalizes_and_uses_correct_slot(void)
{
	...
	result = tp_glove_write(&file, input, sizeof(input) - 1, &offset);
	expect(result == 2, case_name, "successful write did not return count");
	expect(callback_calls == 1, case_name, "callback was not called");
	expect(wrong_callback_calls == 0, case_name, "stale callback slot was used");
...
static void test_zero_write_without_callback(void)
{
	...
	expect(wrong_callback_calls == 0, case_name, "stale callback was invoked");
...
int main(void)
{
	...
	puts("PASS tp_glove_write host tests (3 cases)");
	return 0;
}
