#!/usr/bin/env python3
"""Run direct-source ASan/UBSan tests for syna_tcm_v1_read_message."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "-Wno-cast-function-type -Wno-sign-compare -Wno-error=uninitialized "
        "-Wl,--build-id=none "
        "/drivers/kernel_development/drivers/validation/zte_tpd/host/"
        "syna_tcm_v1_read_message_host_test.c "
        "-o /out/read_message_host_test && /out/read_message_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "read_message_host_test"
