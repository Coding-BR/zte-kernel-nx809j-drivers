#!/usr/bin/env python3
"""Run the direct set_finger_lock_flag source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS set_finger_lock_flag: 4 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "set_finger_lock_flag_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "set_finger_lock_flag.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("set_finger_lock_flag source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-set-finger-lock-flag-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/set_finger_lock_flag_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "set_finger_lock_flag", "mode": "offline_direct_source_asan_ubsan",
              ...
              "limitations": ["The harness includes the reconstructed C source and uses fake device memory and callbacks.",
