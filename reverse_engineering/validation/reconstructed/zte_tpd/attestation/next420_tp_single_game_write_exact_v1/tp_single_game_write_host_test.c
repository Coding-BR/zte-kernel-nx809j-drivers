...
static int set_single_game(struct ztp_device *cdev, int value)
{
  callback_calls++;
...
static int stale_single_game(struct ztp_device *cdev, int value)
{
  (void)cdev;
  (void)value;
...
#include "../../../reconstructed/zte_tpd/tp_single_game_write.c"

struct fixture {
  uint8_t device[0xee0 + sizeof(void *)];
};
...
static void fixture_init(struct fixture *fixture, int with_callback)
{
  ...
  store_ptr(fixture->device, 0xd28, (void *)stale_single_game);
  store_ptr(fixture->device, 0xee0,
            with_callback ? (void *)set_single_game : NULL);
...
static void test_nonzero_input_maps_to_five(void)
{
  ...
  expect(tp_single_game_write(&file, input, sizeof(input) - 1, &offset) == 1,
...
static void test_zero_input_maps_to_zero_without_callback(void)
{
  ...
  expect(tp_single_game_write(&file, input, sizeof(input) - 1, &offset) == 2,
...
static void test_invalid_input_returns_einval_without_side_effects(void)
{
  ...
  fixture_init(&fixture, 1);
  expect(tp_single_game_write(&file, input, sizeof(input) - 1, &offset) == -EINVAL,
...
int main(void)
{
  ...
  puts("PASS tp_single_game_write host tests (3 cases)");
  return 0;
}
