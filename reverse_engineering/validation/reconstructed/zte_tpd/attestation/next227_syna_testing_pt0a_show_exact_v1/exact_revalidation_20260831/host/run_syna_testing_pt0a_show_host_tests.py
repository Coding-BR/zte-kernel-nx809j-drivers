#!/usr/bin/env python3
"""Build and run the PT0A sysfs harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_testing_pt0a_show host tests (8 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		"--source",
		type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("syna_testing_pt0a_show_host_test.c"),
	)
	...
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "syna_testing_pt0a_show.c"
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-fno-pie",
			"-no-pie",
			"-frandom-seed=zte-tpd-next56-testing-pt0a",
	...
	report = {
		...
		"mode": "offline_direct_source_next56_testing_pt0a_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_testing_pt0a_show",
		"covered_functions": ["syna_testing_pt0a_show"],
		"source": str(source),
		...
		"limitations": [
			...
			"It proves sysfs lookup offsets, PT0A limit placement, signed matrix stride and cleanup branches.",
			"Assembly, KCFI, Ghidra and Joern equivalence remain independent gates.",
