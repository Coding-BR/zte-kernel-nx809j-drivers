...
struct lock_class_key { unsigned long token; };
struct device { unsigned long token; };
struct kobject { struct kobject *parent; };
struct kobj_attribute { u8 unused; };
...
const unsigned char pt0a_hi_limits[3200] = { 0x5a };
const unsigned char pt0a_lo_limits[3200] = { 0xa5 };
...
#include "../../../reconstructed/zte_tpd/syna_testing_pt0a_show.c"
...
static void reset_fixture(struct fixture *fixture)
{
	...
	test_item.version = 10;
	test_item.id = 0x0a00;
	test_item.name = "PT0A Test";
...
static void test_disconnected(void)
{
	...
	expect(syna_testing_pt0a_show(&fixture.testing_kobj, NULL, output) ==
	       (int)strlen("Device is NOT connected\n"), name, "return mismatch");
...
static void test_missing_item(void)
{
	...
	selected_item = NULL;
	expect(syna_testing_pt0a_show(&fixture.testing_kobj, NULL, output) ==
	       (int)strlen("Invalid testing item id:2560\n"), name, "return mismatch");
	expect(strcmp(output, "Invalid testing item id:2560\n") == 0, name,
	       "output mismatch");
	expect(mutex_init_calls == 0 && run_calls == 0 && !contract_failed, name,
	       "unexpected item-path action");
}

static void test_signed_matrix_with_device_stride(void)
{
	const char *name = "signed_matrix_with_device_stride";
	struct fixture fixture;
	char output[4096] = {};
	...
	expect(syna_testing_pt0a_show(&fixture.testing_kobj, NULL, output) > 0,
	       name, "return mismatch");
	expect(strcmp(output,
	       "\nPT0A Test (version.10): Pass\n\n-11 22 33 \n-55 66 77 \n") == 0,
	       name, "signed matrix or stride mismatch");
...
static void test_callback_error(void)
{
	...
	expect(syna_testing_pt0a_show(&fixture.testing_kobj, NULL, output) > 0,
	       name, "return mismatch");
	expect(strcmp(output, "\nPT0A Test (version.10): Fail\n\n") == 0, name,
	       "failure header mismatch");
	expect(strcmp(last_log,
	       "\0013[error] syna_testing_pt0a_show: Fail to run test, PT0A Test\n") == 0,
...
static void test_zero_columns_emits_row_newlines(void)
{
	...
	expect(devm_kfree_calls == 1 && !contract_failed, name, "cleanup mismatch");
}
...
static void test_busy_buffer_logs_then_releases(void)
{
	...
	expect(syna_testing_pt0a_show(&fixture.testing_kobj, NULL, output) > 0,
	       name, "return mismatch");
	expect(strstr(output, "PT0A Test (version.10): Fail") != NULL, name,
	       "failure header mismatch");
	expect(strcmp(last_log,
	...
	expect(printk_calls == 1 && devm_kfree_calls == 1 &&
	       last_result_buffer == NULL && !contract_failed, name, "busy cleanup mismatch");
}
