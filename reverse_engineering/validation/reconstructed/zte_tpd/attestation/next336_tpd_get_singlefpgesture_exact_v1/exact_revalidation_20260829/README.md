# `tpd_get_singlefpgesture` exact revalidation

This package records the Docker-backed revalidation of microtask `336_tpd_get_singlefpgesture`.
...
- AArch64 function comparison: PASS; 24 instructions, 96-byte symbol, and equivalent relocations for stock entry `0x0012bde4`.
...
The module was built from the Docker-curated snapshot with image `nubia-sm8850-kernel-builder:latest`, toolchain `clang-r536225`, and two canonical build cycles. The host oracle validates the observed nested-state read, destination store, return value and preservation of the source value.
