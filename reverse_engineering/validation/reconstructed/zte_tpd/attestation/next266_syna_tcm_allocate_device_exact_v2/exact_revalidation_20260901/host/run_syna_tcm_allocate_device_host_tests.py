#!/usr/bin/env python3
"""Run the syna_tcm_allocate_device host contract twice with sanitizers."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "-fno-pie -no-pie -fsanitize=address,undefined -fno-sanitize=alignment "
        "-Wl,--build-id=none "
        "/drivers/kernel_development/drivers/validation/zte_tpd/host/"
        "syna_tcm_allocate_device_host_test.c "
        "-o /out/allocate_device_host_test && /out/allocate_device_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "allocate_device_host_test"
    ...
    report = {
        "schema_version": 1,
        "target": "syna_tcm_allocate_device",
