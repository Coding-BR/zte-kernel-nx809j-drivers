#!/usr/bin/env python3
"""Build and run the syna_dev_disconnect host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_disconnect host tests (5 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_dev_disconnect.c"
)
DRIVER_SOURCE_ASM = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/syna_dev_disconnect_exact.S"
)
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_disconnect_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-D", "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target368-disconnect",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_disconnect_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_syna_dev_disconnect_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_disconnect",
              "covered_functions": ["syna_dev_disconnect", "cancel_delayed_work_sync",
                                     "__flush_workqueue", "destroy_workqueue",
                                     "syna_request_managed_device", "devm_free_irq",
              ...
              "limitations": [
                  "Indirect KCFI callbacks are kept NULL in the harness to avoid executing fabricated function-pointer memory.",
                  "Firmware transport, SPI, IRQ hardware, workqueue concurrency and physical hardware are not exercised."]}
