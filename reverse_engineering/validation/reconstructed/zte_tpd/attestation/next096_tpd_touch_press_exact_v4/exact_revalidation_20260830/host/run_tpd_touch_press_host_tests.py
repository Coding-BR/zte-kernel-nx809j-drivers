#!/usr/bin/env python3
"""Run reproducible ASan/UBSan tests for tpd_touch_press."""
...
EXPECTED_STDOUT = "PASS tpd_touch_press host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tpd_touch_press_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tpd_touch_press.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tpd_touch_press harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-touch-press",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tpd_touch_press_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_touch_press_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_touch_press",
        "covered_functions": ["tpd_touch_press"],
        ...
        "limitations": [
            "Input, point-report state, jiffies, touch reporting and release are deterministic host stubs.",
            "The harness covers invalid input, first-contact/move, limit-area and debounce-release contracts without hardware.",
