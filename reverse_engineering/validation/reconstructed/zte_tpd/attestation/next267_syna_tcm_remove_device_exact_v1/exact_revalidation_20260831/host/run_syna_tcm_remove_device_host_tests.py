#!/usr/bin/env python3
"""Build and run the remove-device lifecycle harness twice."""
...
EXPECTED = "PASS syna_tcm_remove_device host tests (3 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path, default=pathlib.Path(__file__).with_name(
		"syna_tcm_remove_device_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()
	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "syna_tcm_remove_device.c"
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
			  "mode": "offline_direct_source_remove_device_lifecycle_asan_ubsan", "driver": "zte_tpd",
			  "target": "syna_tcm_remove_device", "covered_functions": ["syna_tcm_remove_device"],
			  ...
			  "limitations": ["Kernel printk, managed-device API and devm_kfree are stubbed; Android runtime, allocator ownership and NX809J hardware remain outside this unit gate."]}
