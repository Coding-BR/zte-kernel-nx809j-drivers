...
#include "../../../reconstructed/zte_tpd/syna_tcm_buf_alloc_0.c"
...
static void test_existing_large_buffer(void)
{
  ...
  *(unsigned int *)(state + 12) = 9;
  reset_state();
  assert(syna_tcm_buf_alloc_0((__int64)(uintptr_t)state) == 0);
...
static void test_allocate_success(void)
{
  ...
  assert(syna_tcm_buf_alloc_0((__int64)(uintptr_t)buffer) == 0);
...
static void test_allocate_no_managed_device(void)
{
  ...
  reset_state();
  set_requests(requests, 1);
  assert(syna_tcm_buf_alloc_0((__int64)(uintptr_t)buffer) == 4294967053LL);
  assert(request_calls == 1 && kmalloc_calls == 0 && printk_calls == 2);
  assert(*(void **)buffer == NULL && *(unsigned int *)(buffer + 8) == 0);
  assert(strcmp(last_function, "syna_tcm_buf_alloc") == 0);
...
int main(void)
{
  ...
  puts("syna_tcm_buf_alloc_0 host oracle: PASS (5 groups)");
  return 0;
}
