...
static int refresh_smart_cover(struct ztp_device *cdev)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/tp_smart_cover_read.c"
				...
				int with_callback)
{
	...
	store_u32(fixture->device, 4, value);
	store_ptr(fixture->device, 0xc80, (void *)wrong_callback);
	if (with_callback)
		store_ptr(fixture->device, 0xe38, (void *)refresh_smart_cover);
	else
		store_ptr(fixture->device, 0xe38, NULL);
...
static void test_full_read_and_correct_callback_offset(void)
{
	...
	expect(printk_calls == 1 && last_logged_value == 9, case_name,
...
static void test_partial_read_and_eof(void)
{
	...
	result = tp_smart_cover_read(&file, fixture.output, 1, &position);
...
int main(void)
{
	...
	puts("PASS tp_smart_cover_read host tests (2 cases)");
	return 0;
}
