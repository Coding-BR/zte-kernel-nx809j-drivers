#!/usr/bin/env python3
"""Run reproducible ASan/UBSan tests for ghost_debug_write."""
...
EXPECTED_STDOUT = "PASS ghost_debug_write host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "ghost_debug_write_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "ghost_debug_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("ghost_debug_write harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-ghost-debug-write",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/ghost_debug_write_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_ghost_debug_write_asan_ubsan",
        "driver": "zte_tpd", "target": "ghost_debug_write",
        "covered_functions": ["ghost_debug_write"],
        ...
        "limitations": [
            "Kernel user-copy, printk and cdev storage are deterministic host stubs.",
            "The harness checks parser order, ten-token limit, 100-byte count cap, null input and copy failure without hardware.",
