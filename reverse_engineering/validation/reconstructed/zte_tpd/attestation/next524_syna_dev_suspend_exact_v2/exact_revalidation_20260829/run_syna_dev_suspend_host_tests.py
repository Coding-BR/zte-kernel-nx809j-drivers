#!/usr/bin/env python3
"""Build and run the syna_dev_suspend host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_suspend host tests (6 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_suspend_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-D", "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target371-suspend",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_suspend_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_syna_dev_suspend_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_suspend",
              ...
              "limitations": [
                  "Low-power gesture, sleep, callbacks and input cleanup are deterministic stubs; no actual power rail or firmware state is exercised.",
                  "No smartphone, SPI, IRQ, firmware blob or physical hardware is exercised."]}
