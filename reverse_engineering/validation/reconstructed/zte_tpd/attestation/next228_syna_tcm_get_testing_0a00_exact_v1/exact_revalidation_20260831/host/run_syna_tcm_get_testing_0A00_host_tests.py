#!/usr/bin/env python3
"""Build and run the PT0A testing-item getter harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_tcm_get_testing_0A00 host tests (1 case)\n"
...
def main() -> int:
	...
	parser.add_argument(
		"--source",
		type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("syna_tcm_get_testing_0A00_host_test.c"),
	)
	...
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "syna_tcm_get_testing_0A00.c"
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-fno-pie",
			"-no-pie",
			"-frandom-seed=zte-tpd-next57-get-testing-0a00",
	...
	report = {
		...
		"mode": "offline_direct_source_next57_get_testing_0a00_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_tcm_get_testing_0A00",
		"covered_functions": ["syna_tcm_get_testing_0A00"],
		"source": str(source),
		...
		"limitations": [
			"The harness supplies a minimal stand-in for the static testing item.",
			"It proves only the object-identity contract of the zero-argument getter.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone or firmware interaction is used.",
		],
	}
