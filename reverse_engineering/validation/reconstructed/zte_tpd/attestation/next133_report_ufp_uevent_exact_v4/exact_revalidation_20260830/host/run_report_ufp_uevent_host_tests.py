#!/usr/bin/env python3
"""Build and run the report_ufp_uevent host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS report_ufp_uevent host tests (7 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("report_ufp_uevent_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = source.parents[5] / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "report_ufp_uevent.c"
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next133-report", "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/report_ufp_uevent_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_report_ufp_uevent_asan_ubsan",
        "driver": "zte_tpd",
        "target": "report_ufp_uevent",
        "covered_functions": ["report_ufp_uevent"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers down, repeated down, up, AOD gesture, completion and null platform-device paths.",
            "No smartphone, kernel uevent subsystem, firmware transport or physical hardware is exercised.",
        ],
    }
