# `tp_glove_read` — offline exact revalidation

- Driver: `zte_tpd`
- Stock entry: `0x00109db8`
- Stock body: 60 AArch64 instructions / 240 bytes
- Candidate source: `candidate_source.c` (`tp_glove_read.c`)
...
- The Docker ASan/UBSan host contract passed twice with 2 direct cases covering the callback slot at `tpd_cdev + 0xe48`, value at `tpd_cdev + 0x8`, partial read, and EOF behavior.

## Reconstructed behavior

The function only performs the read operation when the file offset is zero. It reads the glove value from `tpd_cdev + 0x8`, invokes the callback at `tpd_cdev + 0xe48` when present, logs the stock value, formats it with the stock ten-byte limit, and delegates the result to `simple_read_from_buffer`.
