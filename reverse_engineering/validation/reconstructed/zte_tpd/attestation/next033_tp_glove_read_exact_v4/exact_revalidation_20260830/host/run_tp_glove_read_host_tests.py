#!/usr/bin/env python3
"""Run the tp_glove_read host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_glove_read host tests (2 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "tp_glove_read_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "tp_glove_read.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-tp-glove-read",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/tp_glove_read_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe48 callback offset, +0x8 value read, partial read and EOF contract with fake memory.",
