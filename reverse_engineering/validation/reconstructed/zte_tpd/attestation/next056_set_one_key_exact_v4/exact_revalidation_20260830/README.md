# `set_one_key` offline-exact revalidation

This package records the reproducible offline revalidation of `zte_tpd::set_one_key` at stock entry `0x0010bb40`.

- Stock body: 208 bytes; stock and candidate assembly each contain 52 instructions and 208 bytes in `.text`.
...
- KCFI: passed with stock/candidate type ID `0xc3d43b4d`, `.text`, 208-byte symbol.
- Strict Joern: passed; one mapped function, 30 calls, 3 control structures, zero unresolved calls and zero parser problems. No review findings were emitted.
