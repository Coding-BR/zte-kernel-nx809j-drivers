#!/usr/bin/env python3
"""Build and run the tpd_get_one_key contract oracle twice in Docker."""
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_get_one_key_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_one_key.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_one_key_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-get-one-key",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_one_key_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_get_one_key", "function": "tpd_get_one_key",
