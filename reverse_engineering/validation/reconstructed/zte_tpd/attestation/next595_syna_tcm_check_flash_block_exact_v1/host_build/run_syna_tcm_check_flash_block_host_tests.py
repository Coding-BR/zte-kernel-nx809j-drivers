#!/usr/bin/env python3
"""Run the syna_tcm_check_flash_block host contract twice with sanitizers."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "syna_tcm_check_flash_block_host_test.c "
        "-o /out/syna_tcm_check_flash_block_host_test && "
        "/out/syna_tcm_check_flash_block_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "syna_tcm_check_flash_block_host_test"
    ...
    report = {"schema_version": 1, "target": "syna_tcm_check_flash_block",
              "mode": "direct_source_asan_ubsan_host_contract",
              "expected_cases": 18, "cycles": cycles,
