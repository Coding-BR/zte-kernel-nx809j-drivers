# `tpd_set_play_game` offline exact attestation

This package records the function-level reconstruction of
`tpd_set_play_game` at stock entry `0x0012c6e8` for the ZTE NX809J /
RedMagic 11 Pro+ target.

## Decision

`PROMOTED_OFFLINE_EXACT` at function level. The candidate matches the stock
function at 54 AArch64 instructions and 216 bytes, including relocation
...
`tpd_set_play_game_exact.S`; the preserved sparse stock string bytes are
