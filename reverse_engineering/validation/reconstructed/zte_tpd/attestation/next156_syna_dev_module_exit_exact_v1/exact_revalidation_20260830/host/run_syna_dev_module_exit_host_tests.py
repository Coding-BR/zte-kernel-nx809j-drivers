#!/usr/bin/env python3
"""Build and run the syna_dev_module_exit host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_module_exit host tests (1 case)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_dev_module_exit_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_dev_module_exit.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target163-module-exit", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_module_exit_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_dev_module_exit_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_module_exit",
              "covered_functions": ["syna_dev_module_exit", "platform_driver_unregister", "syna_hw_interface_exit"],
              "source": str(source), "source_sha256": sha256(source), "expected_cases": 1,
              ...
              "limitations": ["The harness uses deterministic stubs for unregister and hardware-interface exit.",
