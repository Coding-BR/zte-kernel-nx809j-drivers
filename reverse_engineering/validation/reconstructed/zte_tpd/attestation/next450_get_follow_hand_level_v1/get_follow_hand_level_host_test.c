...
static long get_follow_hand_level_contract(struct fake_cdev *cdev,
                                           ...
                                           long long *offset)
{
    ...
    if (*offset != 0)
        return 0;
    callback = *(level_callback *)(cdev->bytes + 0xf28);
    if (callback != NULL)
        callback(cdev);
    length = snprintf(formatted, sizeof(formatted), "%u\n",
                      *(uint32_t *)(cdev->bytes + 0x468));
    if (length < 0)
        return length;
...
int main(void)
{
    ...
    *(uint32_t *)(cdev.bytes + 0x468) = 8;
    *(level_callback *)(cdev.bytes + 0xf28) = fake_callback;
    callback_count = 0;
    result = get_follow_hand_level_contract(&cdev, buffer, sizeof(buffer),
                                            &offset);
    if (check(result == 2 && strcmp(buffer, "8\n") == 0 && offset == 2 &&
              callback_count == 1, "initial level read and callback"))
        return 1;
    ...
    offset = 0;
    *(uint32_t *)(cdev.bytes + 0x468) = 0;
    *(level_callback *)(cdev.bytes + 0xf28) = NULL;
    result = get_follow_hand_level_contract(&cdev, buffer, 2, &offset);
    if (check(result == 2 && strncmp(buffer, "0\n", 2) == 0,
              "zero level short read"))
        return 1;

    offset = 0;
    *(uint32_t *)(cdev.bytes + 0x468) = UINT32_MAX;
    memset(buffer, 0, sizeof(buffer));
    result = get_follow_hand_level_contract(&cdev, buffer, sizeof(buffer),
                                            &offset);
    ...
    puts("PASS get_follow_hand_level host tests (4 cases)");
    return 0;
}
