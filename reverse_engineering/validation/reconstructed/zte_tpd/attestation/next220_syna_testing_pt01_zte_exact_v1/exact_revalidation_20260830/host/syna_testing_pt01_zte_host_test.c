...
const unsigned char pt01_limits[16] = { 0 };
void *init_timer_key;
		       ...
		       bool dual_firmware)
{
	static const u8 samples[8] = {
		0x00, 0x7f, 0x80, 0xff, 0x11, 0x22, 0xaa, 0x55
	};

	run_calls++;
	if (!tcm || !item || dual_firmware ||
	    item->limit_primary == NULL || item->limit_secondary != NULL ||
	    item->limit_primary->data != pt01_limits ||
	    item->limit_primary->size != 16 ||
	    item->limit_primary->data_length != 0 ||
	    item->image_rows != 0 || item->image_cols != 0 ||
	    !item->result_data)
		contract_failed = true;
...
#include "../../../reconstructed/zte_tpd/syna_testing_pt01_zte.c"
...
static void test_allocation_failure(void)
{
	...
	allocation_fails = 1;
	expect(syna_testing_pt01_zte(&fixture.tcm) == -ENOMEM, name,
	       "return mismatch");
	expect(allocation_calls == 1 && run_calls == 0 && data_kfree_calls == 0,
	       name, "unexpected side effect");
	expect(strcmp(last_log,
	       "\0013[error] syna_testing_pt01_zte: alloc data_buf failed") == 0,
	       name, "log mismatch");
}
...
static void test_missing_item_preserves_stock_leak(void)
{
	...
	reset_fixture(&fixture);
	selected_item = NULL;
	result = syna_testing_pt01_zte(&fixture.tcm);
	expect(result == (int)strlen("Invalid testing item id:256\n"), name,
	       "return mismatch");
...
static void test_run_failure(void)
{
	...
	reset_fixture(&fixture);
	run_return = -5;
	expect(syna_testing_pt01_zte(&fixture.tcm) > 0, name, "return mismatch");
	expect(strstr(copied_text, "PT01 (version.7): Fail") != NULL, name,
	       "failure status missing");
	expect(strcmp(last_log,
	       "\0013[error] syna_testing_pt01_zte: Fail to run test, PT01\n") == 0,
	       name, "log mismatch");
...
static void test_result_pass(void)
{
	...
	expect(fixture.tcm.connection_flags == 0 && run_calls == 1, name,
	       "unexpected connection gate");
}
...
static void test_hex_byte_output(void)
{
	const char *name = "hex_byte_output";
	struct fixture fixture;
	...
	expect(strcmp(copied_text,
	       "\nPT01 (version.7): Pass\n\nx00 x7F x80 xFF "
	       "x11 x22 xAA x55 \n") == 0,
	       name, "hex byte formatting mismatch");
