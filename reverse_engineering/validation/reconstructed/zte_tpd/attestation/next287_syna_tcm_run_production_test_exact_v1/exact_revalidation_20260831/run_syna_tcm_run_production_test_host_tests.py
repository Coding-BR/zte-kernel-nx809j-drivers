#!/usr/bin/env python3
"""Run the syna_tcm_run_production_test contract oracle twice with sanitizers."""
...
def main() -> int:
    ...
    driver_source = (repo_root / "kernel_development" / "drivers" /
                     ...
                     "syna_tcm_run_production_test.c")
    test_source = "/workspace/kernel_development/drivers/validation/zte_tpd/host/syna_tcm_run_production_test_host_test.c"
    ...
    for cycle in (1, 2):
        ...
        binary = output_dir / "syna_tcm_run_production_test_host_test"
        ...
        passed = (
            ...
            run_stdout == "PASS syna_tcm_run_production_test host tests (9 cases)\n" and
    ...
    report = {
        "schema_version": 1,
        "mode": "offline_direct_source_run_production_test_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_run_production_test",
        "covered_functions": ["syna_tcm_run_production_test"],
        "source": test_source,
        "source_sha256": sha256(runner),
        "driver_source": "/workspace/kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_run_production_test.c",
        ...
        "limitations": [
            "Kernel printk, transport write callback and response buffer copy are stubbed; Android runtime and NX809J hardware remain outside this unit gate."
        ],
    }
