#!/usr/bin/env python3
"""Run the charger work state-machine contract twice with ASan/UBSan."""
...
SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_work_charger_detect_work_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_work_charger_detect_work.c")
IMAGE = "nubia-sm8850-kernel-builder:latest"
EXPECTED = "syna_work_charger_detect_work host contract: PASS\n"
...
def main() -> int:
    ...
    if not (ROOT / SOURCE).is_file():
        parser.error(f"host source not found: {ROOT / SOURCE}")
    ...
    for cycle in (1, 2):
        ...
        compile_command = (
            ...
            f"/drivers/{SOURCE.relative_to('kernel_development/drivers').as_posix()} "
            f"-o {binary}"
        )
        compiled = subprocess.run([
            "docker", "run", "--rm", "-v", f"{ROOT / 'kernel_development/drivers'}:/drivers:ro",
            "-v", f"{cycle_root}:/out", IMAGE, "bash", "-lc", compile_command,
        ], check=False, text=True, capture_output=True)
        executed = None
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "-v", f"{cycle_root}:/out:ro", IMAGE, binary],
                check=False, text=True, capture_output=True,
            )
    ...
    report = {
        "schema_version": 1,
        "driver": "zte_tpd",
        "target": "syna_work_charger_detect_work",
        "mode": "offline_fake_power_supply_state_machine_asan_ubsan",
        ...
        "limitations": [
            "The harness uses a fake power-supply callback and does not touch hardware.",
