...
static int enable_wake_gesture(struct ztp_device *cdev, int value)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/tp_wake_gesture_write.c"
...
static void fixture_init(struct fixture *fixture, int with_callback)
{
	...
	store_ptr(fixture->device, 0xc78, (void *)wrong_callback);
	if (with_callback)
		store_ptr(fixture->device, 0xe30, (void *)enable_wake_gesture);
	else
		store_ptr(fixture->device, 0xe30, NULL);
...
static void test_valid_write_uses_correct_callback_slot(void)
{
	...
	result = tp_wake_gesture_write(&file, input, sizeof(input) - 1, &offset);
...
int main(void)
{
	...
	puts("PASS tp_wake_gesture_write host tests (3 cases)");
	return 0;
}
