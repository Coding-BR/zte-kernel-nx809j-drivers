#!/usr/bin/env python3
"""Compile and run the zlog_exception host harness twice in Docker."""
...
COVERED_FUNCTIONS = [
    "zlog_write_internal", "zlog_read", "zlog_write", "zlog_poll",
    "zlog_ioctl", "zlog_open", "zlog_release", "init_module",
    "zlog_create_log_dev", "cleanup_module",
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compile and run the zlog_exception host harness twice in Docker."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "out",
        help="Directory for binaries and host_test_report.json (default: tests/out)",
    )
...
def main() -> int:
    ...
    passed = (
        ...
        and all("PASS zlog_exception host tests" in item["stdout"] for item in run_reports)
    )
    payload = {
        "schema_version": "1.0",
        "generated_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
        "driver": "zlog_exception",
        "source": str(driver / "zlog_exception.c"),
