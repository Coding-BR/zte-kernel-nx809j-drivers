...
noinline void netlink_exit(void)
{
	if (nl_sk) {
		netlink_kernel_release(nl_sk);
		nl_sk = NULL;
	}
	printk("\0016self module exited\n");
}
...
noinline int gf_power_on(struct gf_dev *gf_dev_opaque)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
	int ret;
...
noinline int gf_power_off(struct gf_dev *gf_dev_opaque)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
	int ret;
...
noinline int gf_hw_reset(struct gf_dev *gf_dev_opaque, unsigned int delay_ms)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
...
noinline void gf_cleanup(struct gf_dev *gf_dev_opaque)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
...
noinline int zte_goodix_pinctrl_init(struct gf_dev *gf_dev_opaque)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
	const char *error;
	void **failed_field;
	...
	failed_field = (void **)&gf_dev->pinctrl;
	...
	failed_field = (void **)&gf_dev->pins_active;
	...
	failed_field = (void **)&gf_dev->pins_suspend;
...
err_pinctrl:
	...
	ret = PTR_ERR(*failed_field);
	gf_dev->pinctrl = NULL;
	return ret;
}

noinline int zte_goodix_pinctrl_select(struct gf_dev *gf_dev_opaque,
						      bool active)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
...
noinline int gf_parse_dts(struct gf_device *gf_dev)
{
	...
	ret = zte_goodix_pinctrl_init((struct gf_dev *)gf_dev);
	if (ret)
...
#else
/* The production module supplies the exact AArch64 body from gf_parse_dts_exact.S. */
extern noinline int gf_parse_dts(struct gf_device *gf_dev);
#endif

noinline int gf_irq_num(struct gf_dev *gf_dev_opaque)
{
	struct gf_device *gf_dev = (struct gf_device *)gf_dev_opaque;
...
static int gf_open(struct inode *inode, struct file *file)
{
	...
	if (list_entry_is_head(gf_dev, &device_list, device_entry)) {
		...
		if (gf_dev->ref_count == 1) {
			...
			gf_dev->irq_num = gf_irq_num((struct gf_dev *)gf_dev);
			...
			if (ret) {
				...
				pr_err("zte_fingerprint: failed to request IRQ:%d\n", ret);
				#endif
				gf_cleanup((struct gf_dev *)gf_dev);
				goto err_unlock_return;
			}
		...
		gf_hw_reset((struct gf_dev *)gf_dev, 5);
		gf_dev->opened = 1;
		barrier();
		ret = 0;
	}
...
static long gf_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case 0x6707: // 26375
		...
		if (gf_dev->opened) {
			...
			ret = gf_power_on((struct gf_dev *)gf_dev);
		}
		gf_dev->opened = 1;
		break;
	case 0x6708: // 26376
		...
		if (!gf_dev->opened) {
			...
			ret = gf_power_off((struct gf_dev *)gf_dev);
		}
		gf_dev->opened = 0;
		break;
	case 0x670a:
...
static int __init zte_fp_init(void)
{
	...
	if (ret < 0) {
		pr_warn("zte_fingerprint: Failed to register char device!\n");
		return ret;
	}
	SPIDEV_MAJOR = ret;
	...
	if (IS_ERR(gf_class)) {
		...
		pr_warn("zte_fingerprint: Failed to create class.\n");
	...
	if (ret < 0) {
		...
		unregister_chrdev(SPIDEV_MAJOR, gf_driver.driver.name);
		pr_warn("zte_fingerprint: Failed to register SPI driver.\n");
	}

	netlink_init();
	pr_info("zte_fingerprint: status = 0x%x\n", ret);
	return 0;
}
