# `aw22xxx_task1_show` exact-island attestation

Status: `PROMOTED_OFFLINE_EXACT`.

This attestation binds stock `aw22xxx_task1_show@001048a4` to the reconstructed `zte_led` candidate. The exact island preserves the 56-byte/14-instruction AArch64 body, `snprintf` format `task1 = 0x%02x\n`, task1 field offset `0x2ed`, output bound `0x1000`, return length and stack/PAC sequence.
