# Offline exact attestation: `accel_y_axial_store`

This package records the offline-static promotion of `accel_y_axial_store` at Ghidra stock entry `0x00100e6c` from `reference_modules/stock/zte_sensor_sensitivity.ko`.

The reconstructed callback preserves the stock AArch64 body, 59 instructions, 236-byte symbol size, KCFI type ID `0x95c41a97`, section, stack-canary path, input conversion, range check, store, call sites, relocation targets, and referenced rodata strings. The source keeps a host-test model while the non-host build includes `accel_y_axial_store_exact.inc`.
