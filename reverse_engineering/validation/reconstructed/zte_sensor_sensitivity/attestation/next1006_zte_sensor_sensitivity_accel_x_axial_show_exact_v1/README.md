# Offline exact attestation: `accel_x_axial_show`

This package records the offline-static promotion of `accel_x_axial_show` at stock entry `0x00100c74` from `reference_modules/stock/zte_sensor_sensitivity.ko`.

The reconstructed callback preserves the stock AArch64 body, 32 instructions, 128-byte symbol size, KCFI type ID `0xae84bcd4`, section, call sites, relocation targets, and the referenced rodata strings. The source keeps a host-test model while the non-host build includes `accel_x_axial_show_exact.inc`.
