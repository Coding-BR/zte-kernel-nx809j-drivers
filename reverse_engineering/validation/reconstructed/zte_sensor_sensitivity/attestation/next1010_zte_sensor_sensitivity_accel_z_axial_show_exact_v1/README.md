# Offline exact attestation: `accel_z_axial_show`

This package records the offline-static promotion of `accel_z_axial_show` at Ghidra stock entry `0x00100f5c` from `reference_modules/stock/zte_sensor_sensitivity.ko`.

The reconstructed callback preserves the stock AArch64 body, 32 instructions, 128-byte symbol size, KCFI type ID `0xae84bcd4`, section, field offset, call sites, relocation targets, and referenced rodata strings. The source keeps a host-test model while the non-host build includes `accel_z_axial_show_exact.inc`.
