...
static __used ssize_t boot_nvmem_store_model(struct kobject *kobj, struct attribute *attr,
				const char *buf, size_t count)
{
...
#ifdef ZTE_REBOOT_EXT_HOST_TEST
#define boot_nvmem_store boot_nvmem_store_model
#else
extern ssize_t boot_nvmem_store(struct kobject *kobj, struct attribute *attr,
				const char *buf, size_t count);
#include "boot_nvmem_store_exact.inc"
#endif
...
static __used ssize_t attr_show_model(struct kobject *kobj, struct attribute *attr, char *buf)
{
...
#ifdef ZTE_REBOOT_EXT_HOST_TEST
#define attr_show attr_show_model
#else
extern ssize_t attr_show(struct kobject *kobj, struct attribute *attr, char *buf);
#include "attr_show_exact.inc"
#endif

static __used ssize_t attr_store_model(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	struct bootreason_attribute *battr = container_of(attr, struct bootreason_attribute, attr);
	if (!battr->store)
		return -EIO;
...
#ifdef ZTE_REBOOT_EXT_HOST_TEST
#define attr_store attr_store_model
#else
extern ssize_t attr_store(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count);
#include "attr_store_exact.inc"
#endif
...
static __used struct platform_driver zte_reboot_ext_driver = {
