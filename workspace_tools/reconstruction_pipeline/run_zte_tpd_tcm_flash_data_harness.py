#!/usr/bin/env python3
"""Compile and execute the zte_tpd CS/MTP flash-data direct-source harness."""
...
SOURCE_FILES = (
    ...
    "syna_tcm_read_cs_data.c",
    "syna_tcm_read_mtp_data.c",
    "syna_tcm_update_cs_config.c",
    "syna_tcm_update_mtp_data.c",
)
EXPECTED_TESTS = 9
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "tcm_flash_data_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "tcm_flash_data_harness.c"
    ...
    shell_command = (
        ...
        f"-I{container_source} tcm_flash_data_harness.c "
        "-o build/tcm_flash_data_harness && ./build/tcm_flash_data_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        ...
        "limitations": [
            "Flash setup, read/write, allocation, mode-switch, mutex, and logging behavior use deterministic host stubs.",
            "The executable paths stop at a controlled syna_tcm_set_up_flash_access failure.",
            "Successful MMIO/SPI flash read and write behavior is not exercised or claimed.",
