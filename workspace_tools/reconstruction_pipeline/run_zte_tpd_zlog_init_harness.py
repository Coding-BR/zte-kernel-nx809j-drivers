#!/usr/bin/env python3
"""Run the isolated tpd_zlog_init host harness in reproducible ASan/UBSan cycles."""
...
PASS test_initialize_state
PASS test_reinitialize_and_preserve_adjacent_fields
...
def main() -> int:
    ...
    harness = (
        ...
        / "zte_tpd"
        / "harness"
        / "tpd_zlog_init_harness.c"
    )
    inputs = [harness, source_root / "zte_tpd_zlog.h", source_root / "tpd_zlog_init.c"]
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        cycle_root.mkdir()
        binary = cycle_root / "tpd_zlog_init_harness_asan_ubsan"
        compile_result = run_command([
            ...
            "-frandom-seed=zte-tpd-next109-tpd-zlog-init",
            "-ffile-prefix-map=/source=<source>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "-I/source", "/harness/tpd_zlog_init_harness.c",
            "-o", "/output/tpd_zlog_init_harness_asan_ubsan",
        ])
        ...
        if compile_result["returncode"] == 0 and binary.is_file():
            run_result = run_command([
                ...
                args.image, "/output/tpd_zlog_init_harness_asan_ubsan",
            ])
