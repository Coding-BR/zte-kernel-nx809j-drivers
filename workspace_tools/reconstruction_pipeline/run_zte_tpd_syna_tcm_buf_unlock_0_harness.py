#!/usr/bin/env python3
"""Compile and execute the direct-source syna_tcm_buf_unlock_0 harness."""
...
SOURCE_FILE = "syna_tcm_buf_unlock_0.c"
HARNESS_FILE = "syna_tcm_buf_unlock_0_harness.c"
EXPECTED_TESTS = 14
...
def main() -> int:
    ...
    if missing:
        raise FileNotFoundError("missing syna_tcm_buf_unlock_0 harness inputs: " + ", ".join(missing))

    output = (args.output or harness_dir.parent / "syna_tcm_buf_unlock_0_harness_report.json").resolve()
    ...
    command = [
        ...
        f"-o build/syna_tcm_buf_unlock_0_harness && "
        "ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/syna_tcm_buf_unlock_0_harness",
    ]
    executions = []
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_syna_tcm_buf_unlock_0_asan_ubsan",
