#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_tcm_set_post_reset_callback."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_tcm_set_post_reset_callback_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_tcm_set_post_reset_callback.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("post-reset callback harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next97-post-reset-callback",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_tcm_set_post_reset_callback_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_tcm_set_post_reset_callback host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_post_reset_callback_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_set_post_reset_callback",
        "covered_functions": ["syna_tcm_set_post_reset_callback"],
        ...
        "limitations": [
            "The harness validates only the two pointer stores, null error path, logger contract, and return values.",
