#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for tpd_suspend_work."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tpd_suspend_work_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tpd_suspend_work.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tpd_suspend_work harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next109-tpd-suspend-work",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tpd_suspend_work_host_test.c",
        ...
        passed = (
            ...
            and "PASS tpd_suspend_work host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_suspend_work_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_suspend_work",
        "covered_functions": ["tpd_suspend_work"],
        ...
        "limitations": [
            "The harness validates the null callback branch, the 0xe10 callback slot, and context forwarding from 0xdd8.",
