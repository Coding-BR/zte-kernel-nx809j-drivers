#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for tpd_resume_work_init."""
...
def main() -> int:
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next126-tpd-resume-work-init",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tpd_resume_work_init_host_test.c",
        ...
        passed = (
            ...
            and "PASS tpd_resume_work_init host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_resume_work_init_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_resume_work_init",
        "covered_functions": ["tpd_resume_work_init"],
        ...
        "inputs": [
            {"path": str(harness), "size": harness.stat().st_size,
        ...
        "limitations": [
            "The harness validates the three work layouts, callback addresses, printk label and timer initialization contract.",
