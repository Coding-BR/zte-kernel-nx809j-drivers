#!/usr/bin/env python3
"""Compile and execute the direct-source zte_tpd proc feature harness."""
...
SOURCE_FILES = (
    "tp_wake_gesture_read.c",
    "tp_wake_gesture_write.c",
    "tp_smart_cover_read.c",
    "tp_smart_cover_write.c",
    "tp_glove_read.c",
    "tp_glove_write.c",
    "tp_single_tap_read.c",
    "tp_single_tap_write.c",
    "tp_single_aod_read.c",
    "tp_single_aod_write.c",
    "tp_single_game_read.c",
    "tp_single_game_write.c",
    "tp_palm_mode_read.c",
    "tp_palm_mode_write.c",
    "tp_fold_state_read.c",
    "tp_fold_state_write.c",
    "tp_frame_data_read.c",
    "tp_frame_data_write.c",
)
EXPECTED_TESTS = 7
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "proc_feature_harness_report.json").resolve()
    (harness_dir / "build").mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "proc_feature_callbacks_harness.c"
    ...
    shell_command = (
        ...
        f"-Wno-unused-but-set-variable -Wno-self-assign "
        ...
        f"-I{container_source} proc_feature_callbacks_harness.c "
        f"-o build/proc_feature_callbacks_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/proc_feature_callbacks_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        "schema_version": "1.0",
        "mode": "offline_direct_source_proc_feature_host_harness_asan_ubsan",
        ...
        "limitations": [
            "Kernel procfs and userspace access helpers are modeled by deterministic host stubs.",
            "Indirect getters and setters use signature-compatible fixture callbacks.",
            "The recovered tpd_cdev field and callback offsets are exercised under ASan and UBSan.",
