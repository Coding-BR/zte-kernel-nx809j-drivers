#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the sixth zte_tpd ABI group."""
...
SOURCE_FILES = (
    ...
    "syna_tcm_v1_set_up_max_rw_size.c",
    "syna_tcm_config_timings.c",
    "syna_tcm_enable_predict_reading.c",
...
def main() -> int:
    ...
    harness_source = harness_dir / "next6_abi_harness.c"
    inputs = [
        harness_source,
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next6_abi_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        ...
        f"next6_abi_harness.c -o build/next6_abi_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next6_abi_harness"
    )
    command = [
        "docker",
        "run",
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_next6_abi_host_harness_asan_ubsan",
                ...
                "coverage": {
                    ...
                    "report_dispatcher_slots_checked": 256,
                    "boundary_report_codes": [0, 15, 16, 255],
                    "timing_fields_checked": [1, 2, 4, 8, 256, 512],
                ...
                "limitations": [
                    ...
                    "ASAN and UBSAN validate memory bounds and scalar behavior, not physical touch hardware.",
