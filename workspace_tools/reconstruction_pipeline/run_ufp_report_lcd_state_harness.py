#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for ufp_report_lcd_state."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "ufp_report_lcd_state_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "ufp_report_lcd_state.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("ufp_report_lcd_state harness or source is missing")

    output = args.output.resolve()
    build_root = output.parent / f"{output.stem}-build"
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next136-ufp-report-lcd-state",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/ufp_report_lcd_state_host_test.c",
        ...
        passed = (
            ...
            and "PASS ufp_report_lcd_state host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_ufp_report_lcd_state_asan_ubsan",
        "driver": "zte_tpd",
        "target": "ufp_report_lcd_state",
        "covered_functions": ["ufp_report_lcd_state"],
        ...
        "limitations": [
            "The harness exercises the C reconstruction with controlled UFP and uevent/printk stubs.",
