#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the twelfth zte_tpd ABI group."""
...
SOURCE_FILES = ("tpd_sysfs_fwimage_show.c", "tpd_sysfs_fwimage_store.c")
EXPECTED_TESTS = 15
...
def main() -> int:
    ...
    harness_source = harness_dir / "next12_abi_harness.c"
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next12_abi_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        f"{clang} -std=gnu11 -O1 -Wall -Wextra -Werror -Wno-unused-parameter "
        f"-fsanitize=address,undefined -fno-omit-frame-pointer "
        f"next12_abi_harness.c -o build/next12_abi_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next12_abi_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_next12_abi_host_harness_asan_ubsan",
                ...
                "coverage": {
                    ...
                    "null_descriptor_and_data_errors": True,
                    "zero_size_error": True,
                    "full_and_truncated_read_write": True,
                    "position_update_and_reset": True,
                    "firmware_cleanup_order": True,
                    "mutex_pointer_and_order": True,
                    "exact_diagnostic_strings": True,
                    "bin_attribute_callback_signatures": True,
                    "ignored_sysfs_metadata": True,
                ...
                "limitations": [
                    "The harness executes both recovered sources directly with deterministic host data.",
                    "ASAN and UBSAN validate covered memory, alignment and scalar behavior, not physical touch hardware.",
                    "Kernel allocation, mutex, printk and memcpy APIs are deterministic host doubles.",
                    "Assembly, Ghidra P-Code, KCFI and ELF layout comparisons remain independent gates.",
    ...
    print(json.dumps({"output": str(output), "passed": passed, "repetitions": args.repetitions, "tests": EXPECTED_TESTS, "functions": len(SOURCE_FILES)}))
