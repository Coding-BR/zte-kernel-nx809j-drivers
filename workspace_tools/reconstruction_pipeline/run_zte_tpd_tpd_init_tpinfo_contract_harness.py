#!/usr/bin/env python3
"""Run the reproducible ASan/UBSan contract harness for tpd_init_tpinfo."""
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tpd_init_tpinfo_contract_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tpd_init_tpinfo.c"
    ...
    for index in range(1, args.repetitions + 1):
        ...
        compile_result = run([
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next164-tpinfo",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/tpd_init_tpinfo_contract_test.c",
        ...
        if compile_result["returncode"] == 0 and binary.is_file():
            run_result = run(["docker", "run", "--rm", "-v", f"{cycle}:/output:ro", args.image,
                              "/output/contract_asan_ubsan"])
            digest = sha256_file(binary)
            hashes.append(digest)
        passed = (compile_result["returncode"] == 0 and run_result["returncode"] == 0 and
                  "PASS tpd_init_tpinfo contract test (6 checks)" in str(run_result["stdout"]))
    ...
    report = {
        ...
        "driver": "zte_tpd", "target": "tpd_init_tpinfo", "covered_functions": ["tpd_init_tpinfo"],
        "mode": "offline_contract_model_asan_ubsan", "container_image": args.image,
        "compiler": f"/toolchains/{args.clang_revision}/bin/clang", "repetitions": args.repetitions,
        "inputs": [{"path": str(harness), "sha256": sha256_file(harness)},
                   {"path": str(source), "sha256": sha256_file(source)}],
        "cycles": cycles, "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": ["The final function contains target-specific AArch64 inline assembly and is not executed on the x86 host.",
