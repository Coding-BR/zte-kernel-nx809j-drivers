#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the next zte_tpd ABI group."""
...
def main() -> int:
    ...
    harness_source = harness_dir / "next2_abi_harness.c"
    ...
    output = (args.output or root / "validation" / "zte_tpd" /
              "next2_abi_harness_report.json").resolve()
    ...
    shell_command = (
        ...
        f"next2_abi_harness.c -o build/next2_abi_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next2_abi_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    output.write_text(json.dumps({
        "schema_version": "1.0",
        "mode": "offline_direct_source_next2_abi_host_harness_asan_ubsan",
        ...
        "limitations": [
            "UFP kobject, printk, completion, gesture and kasprintf calls are deterministic host stubs.",
            "The harness validates recovered state transitions, flags, offsets and callback contracts, not hardware behavior.",
            "The report_ufp_uevent opcode comparison remains separate because compiler layout is not proven equivalent.",
