#!/usr/bin/env python3
"""Compile and execute the zte_tpd TCM lifecycle direct-source harness."""
...
SOURCE_FILES = (
    ...
    "syna_tcm_clear_command_processing.c",
    "syna_tcm_remove_device.c",
    "syna_tcm_v1_terminate.c",
)
EXPECTED_TESTS = 11
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "tcm_lifecycle_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "tcm_lifecycle_harness.c"
    ...
    shell_command = (
        ...
        f"-I{container_source} tcm_lifecycle_harness.c "
        "-o build/tcm_lifecycle_harness && ./build/tcm_lifecycle_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        ...
        "limitations": [
            "completion, managed-device release, callback, logging, and allocation use deterministic host stubs.",
