#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan direct-source harness for syna_cdev_write."""
...
EXPECTED_STDOUT = "PASS syna_cdev_write host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_cdev_write_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_cdev_write.c"
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_result = run_command([
            ...
            "-frandom-seed=zte-tpd-next122-syna-cdev-write",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_cdev_write_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_cdev_write_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_cdev_write",
        "covered_functions": ["syna_cdev_write"],
        ...
        "limitations": [
            "The harness validates direct-source cdev write control flow and ABI-visible arguments only.",
