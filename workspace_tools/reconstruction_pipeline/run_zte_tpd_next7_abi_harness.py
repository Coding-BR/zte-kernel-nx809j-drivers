#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the seventh zte_tpd ABI group."""
...
SOURCE_FILES = (
    "syna_tcm_get_boot_info.c",
    "syna_tcm_get_app_info.c",
    "syna_tcm_get_features.c",
    "syna_tcm_identify.c",
)
EXPECTED_TESTS = 23
...
def main() -> int:
    ...
    harness_source = harness_dir / "next7_abi_harness.c"
    inputs = [
        harness_source,
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next7_abi_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        ...
        f"next7_abi_harness.c -o build/next7_abi_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next7_abi_harness"
    )
    command = [
        "docker",
        "run",
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_next7_abi_host_harness_asan_ubsan",
                ...
                "coverage": {
                    ...
                    "commands_checked": ["0x10", "0x20", "0x32", "0x02"],
                    "protocol_payload_sizes": [0x20, 0x30, 0x10, 0x30],
                    "irq_and_polling_paths": True,
                    "copy_truncation_and_error_paths": True,
                    "nested_lock_balance_path": True,
                ...
                "limitations": [
                    "The harness executes the four recovered functions directly with deterministic host data.",
                    "ASAN and UBSAN validate covered memory, alignment and scalar behavior, not physical touch hardware.",
                    "The write_message callback and mutex primitives are deterministic host doubles.",
