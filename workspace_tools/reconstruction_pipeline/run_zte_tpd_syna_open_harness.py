#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan host harness for zte_tpd syna_open."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_open_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_open.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("syna_open harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next96-syna-open",
        ...
        passed = (
            ...
            and "PASS syna_open host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "limitations": [
            "The harness verifies only the private_data offset, logger contract, and zero return.",
