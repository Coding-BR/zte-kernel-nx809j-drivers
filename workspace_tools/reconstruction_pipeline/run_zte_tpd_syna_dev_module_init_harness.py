#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_dev_module_init."""
...
def main() -> int:
    ...
    harness = (drivers_root / "validation" / "zte_tpd" / "host" /
               "syna_dev_module_init_host_test.c")
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_dev_module_init.c"
    header = drivers_root / "reconstructed" / "zte_tpd" / "defs.h"
    if not harness.is_file() or not source.is_file() or not header.is_file():
        raise FileNotFoundError("syna_dev_module_init harness, source, or header is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next114-syna-dev-module-init",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_module_init_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_dev_module_init host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_module_init_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_module_init",
        "covered_functions": ["syna_dev_module_init"],
        ...
        "inputs": [
            ...
            {"path": str(header), "size": header.stat().st_size, "sha256": sha256_file(header)},
        ],
        "cycles": cycles,
        ...
        "limitations": [
            "The harness validates the negative-init early return plus the nonnegative-init delay and driver registration contract.",
