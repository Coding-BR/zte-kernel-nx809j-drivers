#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_pm_resume."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_pm_resume_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_pm_resume.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("syna_pm_resume harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next117-syna-pm-resume",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_pm_resume_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_pm_resume host tests (1 case)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_pm_resume_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_pm_resume",
        "covered_functions": ["syna_pm_resume"],
        ...
        "limitations": [
            "The harness validates the exact printk strings, state clear, and completion address.",
