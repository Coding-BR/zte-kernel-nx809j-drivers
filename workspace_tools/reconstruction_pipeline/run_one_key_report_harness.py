#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for one_key_report."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "one_key_report_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "one_key_report.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("one_key_report harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next134-one-key-report",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/one_key_report_host_test.c",
        ...
        passed = (
            ...
            and "PASS one_key_report host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_one_key_report_asan_ubsan",
        "driver": "zte_tpd",
        "target": "one_key_report",
        "covered_functions": ["one_key_report"],
        ...
        "limitations": [
            "The harness exercises the C reconstruction with controlled UFP and uevent stubs.",
