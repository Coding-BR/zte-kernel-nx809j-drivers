#!/usr/bin/env python3
"""Compile and execute the remaining direct-source zte_tpd host harness."""
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "remaining_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "remaining_harness.c"
    ...
    shell_command = (
        f"{clang} -std=gnu11 -Wall -Wextra -Werror -Wno-unused-parameter "
        f"-I{container_source} remaining_harness.c -o build/remaining_harness && ./build/remaining_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        ...
        "limitations": [
            "Uevents and command transport are controlled host stubs.",
