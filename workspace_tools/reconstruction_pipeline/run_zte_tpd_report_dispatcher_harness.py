#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_tcm_set_report_dispatcher."""
...
def main() -> int:
    ...
    harness = (
        ...
        "syna_tcm_set_report_dispatcher_host_test.c"
    )
    source = (
    ...
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("report dispatcher harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next100-report-dispatcher",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_tcm_set_report_dispatcher_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_tcm_set_report_dispatcher host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_report_dispatcher_asan_ubsan",
        "driver": "zte_tpd",
        ...
        "limitations": [
            "The harness validates indexed dispatcher stores, warning, info, and error logger contracts, and return values.",
            "It does not invoke the stored callbacks or load the module on an Android device.",
