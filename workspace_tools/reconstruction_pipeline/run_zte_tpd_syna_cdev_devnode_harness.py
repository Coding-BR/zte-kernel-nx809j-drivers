#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan direct-source harness for syna_cdev_devnode."""
...
EXPECTED_STDOUT = "PASS syna_cdev_devnode host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_cdev_devnode_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_cdev_devnode.c"
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next119-syna-cdev-devnode",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_cdev_devnode_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_cdev_devnode_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_cdev_devnode",
        "covered_functions": ["syna_cdev_devnode"],
        ...
        "limitations": [
            "The harness validates direct-source devnode control flow and return propagation only.",
            "It does not load the module or emulate Android device-core behavior.",
