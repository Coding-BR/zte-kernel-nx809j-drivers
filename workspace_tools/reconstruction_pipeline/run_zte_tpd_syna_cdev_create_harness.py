#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan direct-source harness for syna_cdev_create."""
...
EXPECTED_STDOUT = "PASS syna_cdev_create host tests (6 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_cdev_create_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_cdev_create.c"
    layout = drivers_root / "reconstructed" / "zte_tpd" / "zte_tpd_syna_layout.h"
    globals_header = drivers_root / "reconstructed" / "zte_tpd" / "globals.h"
    for path in (harness, source, layout, globals_header):
        if not path.is_file():
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next118-syna-cdev-create",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "-I/drivers/validation/zte_tpd/host/include",
            "/drivers/validation/zte_tpd/host/syna_cdev_create_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_cdev_create_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_cdev_create",
        "covered_functions": ["syna_cdev_create"],
        ...
        "limitations": [
            "The harness validates direct-source control flow and cleanup only.",
            "It does not load the module or emulate Android character-device core behavior.",
            "Assembly, KCFI, Ghidra, Joern, and canonical module build remain separate gates.",
        ],
    }
