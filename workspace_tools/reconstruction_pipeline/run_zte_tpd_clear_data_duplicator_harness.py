#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_tcm_clear_data_duplicator."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_tcm_clear_data_duplicator_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_tcm_clear_data_duplicator.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("clear data duplicator harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next99-clear-data-duplicator",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_tcm_clear_data_duplicator_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_tcm_clear_data_duplicator host tests (2 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_clear_data_duplicator_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_clear_data_duplicator",
        "covered_functions": ["syna_tcm_clear_data_duplicator"],
        ...
        "limitations": [
            "The harness validates only the exact data-duplicator clear range, bounds, null error path, logger contract, and return values.",
            "It does not invoke callbacks or load the module on an Android device.",
