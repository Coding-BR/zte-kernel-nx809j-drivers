#!/usr/bin/env python3
"""Compile and execute the zte_tpd TCM delay/reset direct-source harness."""
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "tcm_delay_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "tcm_delay_harness.c"
    ...
    shell_command = (
        ...
        f"-I{container_source} tcm_delay_harness.c -o build/tcm_delay_harness "
        "&& ./build/tcm_delay_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        ...
        "limitations": [
            "Transport, allocation, mutex, and callback behavior use deterministic host stubs.",
            "The erase routine is executed only through its null-contract path.",
            "No ADB, fastboot, insmod, rmmod, SPI, IRQ, or partition operation is performed.",
