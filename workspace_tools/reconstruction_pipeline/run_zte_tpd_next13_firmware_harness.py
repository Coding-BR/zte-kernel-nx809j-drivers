#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the zte_tpd firmware lifecycle."""
...
SOURCE_FILES = ("tp_alloc_tp_firmware_data.c", "tpd_copy_to_tp_firmware_data.c")
EXPECTED_TESTS = 12
...
def main() -> int:
    ...
    harness_source = harness_dir / "next13_firmware_harness.c"
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next13_firmware_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        ...
        f"next13_firmware_harness.c -o build/next13_firmware_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next13_firmware_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    for index in range(args.repetitions):
        ...
        passed_lines = [line[5:] for line in completed.stdout.splitlines() if line.startswith("PASS ")]
        failed_lines = [line[5:] for line in completed.stdout.splitlines() if line.startswith("FAIL ")]
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_next13_firmware_lifecycle_asan_ubsan",
                ...
                "coverage": {
                    ...
                    "descriptor_size_and_gfp_flags": True,
                    "allocation_success_and_failures": True,
                    "old_buffer_cleanup_order": True,
                    "stock_dangling_slot_on_data_failure": True,
                    "full_empty_and_truncated_copy": True,
                    "position_update_and_end_behavior": True,
                    "exact_diagnostic_strings": True,
                    "old_offset_rejected": True,
                ...
                "limitations": [
                    "The harness executes the recovered sources with deterministic host pools.",
                    "ASAN and UBSAN validate covered memory and scalar behavior, not physical touch hardware.",
                    "The OEM data-allocation failure path leaves the freed descriptor pointer in the cdev slot; the harness records this behavior without dereferencing it.",
                    "Assembly, Ghidra P-Code, KCFI and integration callers remain independent gates.",
    ...
    print(json.dumps({"output": str(output), "passed": passed, "repetitions": args.repetitions, "tests": EXPECTED_TESTS}))
