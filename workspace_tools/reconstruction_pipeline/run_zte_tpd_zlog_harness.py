#!/usr/bin/env python3
"""Compile and execute the direct-source zte_tpd zlog host harness."""
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "zlog_harness_report.json").resolve()
    ...
    shell_command = (
        ...
        f"-I{container_source} zlog_harness.c -o build/zlog_harness && ./build/zlog_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        ...
        "limitations": [
            "Host stubs validate reconstructed control flow, not Android scheduler timing.",
