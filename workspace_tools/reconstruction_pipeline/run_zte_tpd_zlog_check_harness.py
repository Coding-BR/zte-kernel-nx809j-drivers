#!/usr/bin/env python3
"""Run the isolated tpd_zlog_check host harness in reproducible ASan/UBSan cycles."""
...
PASS test_first_event_increments_without_log
PASS test_threshold_is_not_repeated_event
PASS test_repeated_event_logs_and_fails
PASS test_zero_index_is_valid_array_entry
...
def main() -> int:
    ...
    harness = (
        ...
        / "zte_tpd"
        / "harness"
        / "tpd_zlog_check_harness.c"
    )
    inputs = [harness, source_root / "zte_tpd_zlog.h", source_root / "tpd_zlog_check.c"]
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        cycle_root.mkdir()
        binary = cycle_root / "tpd_zlog_check_harness_asan_ubsan"
        compile_result = run_command([
            ...
            "-frandom-seed=zte-tpd-next110-tpd-zlog-check",
            "-ffile-prefix-map=/source=<source>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "-I/source", "/harness/tpd_zlog_check_harness.c",
            "-o", "/output/tpd_zlog_check_harness_asan_ubsan",
        ])
        ...
        if compile_result["returncode"] == 0 and binary.is_file():
            run_result = run_command([
                ...
                args.image, "/output/tpd_zlog_check_harness_asan_ubsan",
            ])
    ...
    report = {
        ...
        "driver": "zte_tpd",
        "target": "tpd_zlog_check",
        "covered_functions": ["tpd_zlog_check"],
        ...
        "limitations": [
            "The host harness validates direct reconstructed control flow, not Android jiffies timing.",
            "The invalid enum path is evidenced by exact stock/candidate assembly, not executed in host mode.",
