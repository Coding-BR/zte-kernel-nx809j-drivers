#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for tpd_resume_work_deinit."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tpd_resume_work_deinit_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tpd_resume_work_deinit.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tpd_resume_work_deinit harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next111-tpd-resume-work-deinit",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tpd_resume_work_deinit_host_test.c",
        ...
        passed = (
            ...
            and "PASS tpd_resume_work_deinit host tests (1 case)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_resume_work_deinit_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_resume_work_deinit",
        "covered_functions": ["tpd_resume_work_deinit"],
        ...
        "limitations": [
            "The harness validates the stock printk format and name plus the ordered cancellations at 0x9c0, 0x9a0, and 0x9e0.",
