#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for uf_touch_report."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "uf_touch_report_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "uf_touch_report.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("uf_touch_report harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next135-uf-touch-report",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/uf_touch_report_host_test.c",
        ...
        passed = (
            ...
            and "PASS uf_touch_report host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_uf_touch_report_asan_ubsan",
        "driver": "zte_tpd",
        "target": "uf_touch_report",
        "covered_functions": ["uf_touch_report"],
        ...
        "limitations": [
            "The harness exercises the C reconstruction with controlled UFP, cdev and uevent stubs.",
            "The exact assembly preserves the stock invalid-index trap; the C safety guard is tested separately as a non-trapping host safety path.",
