#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for suspend_tp_need_awake."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "suspend_tp_need_awake_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "suspend_tp_need_awake.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("suspend_tp_need_awake harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next108-suspend-tp-need-awake",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/suspend_tp_need_awake_host_test.c",
        ...
        passed = (
            ...
            and "PASS suspend_tp_need_awake host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_suspend_tp_need_awake_asan_ubsan",
        "driver": "zte_tpd",
        "target": "suspend_tp_need_awake",
        "covered_functions": ["suspend_tp_need_awake"],
        ...
        "limitations": [
            "The harness validates the null callback branch, the 0xe68 callback slot, context forwarding, and boolean return normalization.",
