#!/usr/bin/env python3
"""Compile and execute the direct-source charger notifier harness."""
...
def main() -> int:
    ...
    inputs = [
        workspace / "kernel_development/drivers/reconstructed/zte_tpd/syna_charger_notify_call.c",
        harness_dir / "syna_charger_notify_call_harness.c",
    ]
    if any(not path.is_file() for path in inputs):
        raise FileNotFoundError("missing charger notifier harness input")
    output = (args.output or harness_dir.parent / "syna_charger_notify_call_harness_report.json").resolve()
    ...
    command = [
        ...
        "-v", f"{args.toolchain_volume}:/work/toolchains:ro", "-w", container_harness,
        args.image, "sh", "-lc",
        f"mkdir -p build && {clang} -std=gnu11 -O1 -Wall -Wextra -Werror "
        "-fsanitize=address,undefined -fno-omit-frame-pointer "
        "syna_charger_notify_call_harness.c -o build/syna_charger_notify_call_harness && "
        "ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 "
        "./build/syna_charger_notify_call_harness",
    ]
    executions = []
    ...
    report = {
        "schema_version": "1.0",
        "mode": "offline_direct_source_syna_charger_notify_call_asan_ubsan",
        ...
        "limitations": [
            "The harness uses a deterministic queue_delayed_work_on stub.",
            "No smartphone, module loading, power-supply notifier registration or physical hardware is exercised.",
        ],
    }
