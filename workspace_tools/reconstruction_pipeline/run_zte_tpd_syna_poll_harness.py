#!/usr/bin/env python3
"""Compile and execute a direct-source syna_poll harness."""
...
def main() -> int:
	...
	inputs = [workspace / "kernel_development/drivers/reconstructed/zte_tpd/syna_poll.c", harness_dir / "syna_poll_harness.c"]
	if any(not path.is_file() for path in inputs):
		raise FileNotFoundError("missing syna_poll harness input")
	output = (args.output or harness_dir.parent / "syna_poll_harness_report.json").resolve()
	...
	command = ["docker", "run", "--rm", "-v", f"{workspace}:{container_root}", "-v", f"{args.toolchain_volume}:/work/toolchains:ro", "-w", container_harness, args.image, "sh", "-lc", f"{clang} -std=gnu11 -O1 -Wall -Wextra -Werror -fsanitize=address,undefined -fno-omit-frame-pointer syna_poll_harness.c -o build/syna_poll_harness && ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 ./build/syna_poll_harness"]
	executions = []
	...
	report = {"schema_version": "1.0", "mode": "offline_direct_source_syna_poll_asan_ubsan", "generated_utc": datetime.now(timezone.utc).isoformat(), "passed": passed, "command": command, "repetitions": args.repetitions, "executions": executions, "tests": {"expected": EXPECTED_TESTS, "passed": executions[0]["tests_passed"], "failed": executions[0]["tests_failed"], "summary": executions[0]["summary"]}, "coverage": {"private_data_offset_0x20": True, "frame_wait_offset_0x448": True, "frame_available_offset_0x468": True, "null_poll_table": True, "available_mask": True, "hardware_paths_exercised": False}, "inputs": [{"path": str(path), "size": path.stat().st_size, "sha256": sha256_file(path)} for path in inputs], "limitations": ["The harness uses a deterministic host poll_wait stub.", "No smartphone, module loading, SPI, IRQ, MMIO, firmware transport or physical hardware is exercised."]}
