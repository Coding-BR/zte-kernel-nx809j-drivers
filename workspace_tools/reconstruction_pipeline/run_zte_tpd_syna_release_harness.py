#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan host harness for zte_tpd syna_release."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_release_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_release.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("syna_release harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next95-syna-release",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_release_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_release host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_release_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_release",
        "covered_functions": ["syna_release"],
        ...
        "limitations": [
            "The harness verifies only the release logger contract and zero return.",
            "It does not load the module or communicate with touch hardware, firmware, display, or storage.",
