#!/usr/bin/env python3
"""Compile and execute the direct-source unexpected-reset callback harness."""
...
SOURCE_FILES = (
    "syna_dev_process_unexpected_reset.c",
    "zte_tpd_syna_device_api.h",
)
EXPECTED_TESTS = 6
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "report_dispatch_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "report_dispatch_unexpected_reset_harness.c"
    ...
    shell_command = (
        ...
        f"-I{container_source} report_dispatch_unexpected_reset_harness.c "
        f"-o build/report_dispatch_unexpected_reset_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/report_dispatch_unexpected_reset_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        "schema_version": "1.0",
        "mode": "offline_direct_source_report_dispatch_unexpected_reset_host_harness_asan_ubsan",
        ...
        "coverage": {
            ...
            "source_files": ["syna_dev_process_unexpected_reset.c"],
        ...
        "limitations": [
            "The workqueue, logging and context memory are deterministic host stubs.",
            "The harness validates recovered offsets, state branches and queue-once behavior, not electrical hardware.",
            "The cdev payload and touch-report parser callbacks remain READY for a dedicated parser fixture.",
