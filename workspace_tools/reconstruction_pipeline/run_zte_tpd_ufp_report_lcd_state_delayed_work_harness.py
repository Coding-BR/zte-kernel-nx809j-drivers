#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for ufp_report_lcd_state_delayed_work."""
...
def main() -> int:
    ...
    harness = (drivers_root / "validation" / "zte_tpd" / "host" /
               "ufp_report_lcd_state_delayed_work_host_test.c")
    source = (drivers_root / "reconstructed" / "zte_tpd" /
              "ufp_report_lcd_state_delayed_work.c")
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("ufp_report_lcd_state_delayed_work harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next112-ufp-report-lcd-state-delayed-work",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/ufp_report_lcd_state_delayed_work_host_test.c",
        ...
        passed = (
            ...
            and "PASS ufp_report_lcd_state_delayed_work host tests (1 case)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_ufp_report_lcd_state_delayed_work_asan_ubsan",
        "driver": "zte_tpd",
        "target": "ufp_report_lcd_state_delayed_work",
        "covered_functions": ["ufp_report_lcd_state_delayed_work"],
        ...
        "limitations": [
            "The harness validates the delay conversion input, CPU 32, workqueue offset 0x4b0, delayed_work offset 0x9e0, and converted jiffies forwarding.",
