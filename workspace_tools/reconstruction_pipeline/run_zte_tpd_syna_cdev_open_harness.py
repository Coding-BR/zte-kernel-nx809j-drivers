#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan direct-source harness for syna_cdev_open."""
...
EXPECTED_STDOUT = "PASS syna_cdev_open host tests (3 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_cdev_open_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_cdev_open.c"
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_result = run_command([
            ...
            "-frandom-seed=zte-tpd-next191-syna-cdev-open",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_cdev_open_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_cdev_open_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_cdev_open",
        "covered_functions": ["syna_cdev_open"],
        ...
        "limitations": [
            "The harness validates direct-source cdev open state, queue teardown, locks, and logging only.",
            ...
            "Only decompiler-origin pointer diagnostics are suppressed; ASan and UBSan remain enabled.",
        ],
    }
