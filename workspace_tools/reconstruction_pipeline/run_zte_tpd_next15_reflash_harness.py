#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for syna_dev_do_reflash."""
...
def main() -> int:
    ...
    harness_source = harness_dir / "next15_reflash_harness.c"
    driver_source = root / "curated" / "zte_tpd" / "syna_dev_do_reflash.c"
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next15_reflash_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        ...
        "next15_reflash_harness.c -o build/next15_reflash_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next15_reflash_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        "schema_version": "1.0",
        "mode": "offline_direct_source_next15_reflash_asan_ubsan",
        ...
        "coverage": {
            ...
            "null_input_and_request_failure": True,
            "exact_diagnostic_strings": True,
            "request_update_release_order": True,
            "update_error_and_positive_return_propagation": True,
            "force_bit_and_size_truncation": True,
            "firmware_mode_formatting": True,
        ...
        "limitations": [
            "The harness uses controlled firmware and update stubs and proves the recovered orchestration contract only.",
            "It does not parse a real firmware image or execute flash commands.",
