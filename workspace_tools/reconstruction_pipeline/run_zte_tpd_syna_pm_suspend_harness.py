#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_pm_suspend."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--build-root",
        type=Path,
        help="Unique external directory for untracked host-build outputs.",
    )
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_pm_suspend_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_pm_suspend.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("syna_pm_suspend harness or source is missing")

    output = args.output.resolve()
    build_root = (args.build_root or output.parent / "host-build").resolve()
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next116-syna-pm-suspend",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_pm_suspend_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_pm_suspend host tests (1 case)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_pm_suspend_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_pm_suspend",
        "covered_functions": ["syna_pm_suspend"],
        ...
        "limitations": [
            "The harness validates the exact printk strings and state writes at offsets 0x578 and 0x558.",
