#!/usr/bin/env python3
"""Compile and execute the zte_tpd TCM static-config direct-source harness."""
...
SOURCE_FILES = (
    "zte_tpd_tcm_layout.h",
    "syna_tcm_get_static_config.c",
    "syna_tcm_set_static_config.c",
    "syna_tcm_set_touch_report_config.c",
)
EXPECTED_TESTS = 20
...
def main() -> int:
    ...
    output = (args.output or harness_dir.parent / "tcm_static_config_harness_report.json").resolve()
    build_dir = harness_dir / "build"
    build_dir.mkdir(parents=True, exist_ok=True)
    harness_source = harness_dir / "tcm_static_config_harness.c"
    ...
    shell_command = (
        ...
        f"-Wno-unused-parameter -Wno-unused-function -Wno-unused-variable "
        f"-Wno-unused-but-set-variable -Wno-int-conversion "
        f"-Wno-uninitialized -Wno-sometimes-uninitialized "
        f"-Wno-logical-op-parentheses -Wno-sign-compare "
        f"-Wno-incompatible-pointer-types "
        f"-Wno-incompatible-pointer-types-discards-qualifiers "
        f"-I{container_source} tcm_static_config_harness.c "
        "-o build/tcm_static_config_harness && ./build/tcm_static_config_harness"
    )
    command = [
        "docker", "run", "--rm",
    ...
    report = {
        "schema_version": "1.0",
        "mode": "offline_direct_source_host_harness",
        ...
        "limitations": [
            "write_message, allocation, response buffers, locks, and logging use deterministic host stubs.",
            "The stock KCFI preamble check is a no-op in the non-KCFI host executable; module KCFI is validated separately from the built ELF.",
            "No real SPI transport, firmware response, scheduler timing, IRQ, or electrical behavior is exercised.",
            "No ADB, fastboot, insmod, rmmod, MMIO, or partition operation is performed.",
