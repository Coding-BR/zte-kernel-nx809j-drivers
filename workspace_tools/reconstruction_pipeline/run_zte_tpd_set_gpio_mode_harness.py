#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for set_gpio_mode."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "set_gpio_mode_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "set_gpio_mode.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("set_gpio_mode harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next107-set-gpio-mode",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/set_gpio_mode_host_test.c",
        ...
        passed = (
            ...
            and "PASS set_gpio_mode host tests (3 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_set_gpio_mode_asan_ubsan",
        "driver": "zte_tpd",
        "target": "set_gpio_mode",
        "covered_functions": ["set_gpio_mode"],
        ...
        "limitations": [
            "The harness validates the null callback branch, the 0xf58 callback slot, argument forwarding, and return propagation.",
