#!/usr/bin/env python3
"""Run the offline ASAN/UBSAN harness for the ninth zte_tpd ABI group."""
...
SOURCE_FILES = (
    "syna_poll.c",
    "syna_spi_hw_reset.c",
    "syna_dev_set_screen_on_fp_mode.c",
    "ufp_report_gesture_uevent.c",
    "syna_dev_isr.c",
)
EXPECTED_TESTS = 31
...
def main() -> int:
    ...
    harness_source = harness_dir / "next9_abi_harness.c"
    inputs = [
        harness_source,
    ...
    output = (
        args.output
        or root / "validation" / "zte_tpd" / "next9_abi_harness_report.json"
    ).resolve()
    ...
    shell_command = (
        ...
        f"next9_abi_harness.c -o build/next9_abi_harness "
        "&& ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 "
        "UBSAN_OPTIONS=halt_on_error=1 ./build/next9_abi_harness"
    )
    command = [
        "docker",
        "run",
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "mode": "offline_direct_source_next9_abi_host_harness_asan_ubsan",
                ...
                "coverage": {
                    ...
                    "poll_wait_and_frame_ready": True,
                    "gpio_reset_polarities_and_delays": True,
                    "dynamic_config_success_and_failure": True,
                    "uevent_wakeup_and_delayed_work": True,
                    "irq_pm_wait_gpio_event_and_pid_paths": True,
                ...
                "limitations": [
                    ...
                    "Kernel GPIO, polling, completion, uevent, workqueue and TCM calls are deterministic host doubles.",
