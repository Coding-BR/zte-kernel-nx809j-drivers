#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for syna_pal_mem_cpy_0."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "syna_pal_mem_cpy_0_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "syna_pal_mem_cpy_0.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("syna_pal_mem_cpy_0 harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next105-pal-mem-cpy-0",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_pal_mem_cpy_0_host_test.c",
        ...
        passed = (
            ...
            and "PASS syna_pal_mem_cpy_0 host tests (4 cases)" in run_result["stdout"]
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_pal_mem_cpy_0_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_pal_mem_cpy_0",
        "covered_functions": ["syna_pal_mem_cpy_0"],
        ...
        "limitations": [
            "The harness validates pointer guards, the sixteen-byte copy, and the short-size logger arguments.",
