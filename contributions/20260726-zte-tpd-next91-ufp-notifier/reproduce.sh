...
python workspace_tools/reconstruction_pipeline/validate_reconstructed_drivers.py --curated-root "$ENGINEERING_ROOT/curated" --driver "$DRIVER" --rebuild --work-root "$ENGINEERING_ROOT/validation/contribution-work-next91-ufp-notifier" --output "$REPORTS/double_clean_rebuild_next91.json"
python workspace_tools/reconstruction_pipeline/verify_llm_reconstruction_cycle.py --driver "$DRIVER" --curated-root "$ENGINEERING_ROOT/curated" --evidence-root "$ENGINEERING_ROOT/validation" --audit "$REPORTS/double_clean_rebuild_next91.json" 2>&1 | tee "$REPORTS/llm_cycle.log"
