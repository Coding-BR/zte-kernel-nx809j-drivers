...
try {
    ...
    python .\workspace_tools\reconstruction_pipeline\validate_reconstructed_drivers.py `
        ...
        --work-root (Join-Path $EngineeringRoot "validation\contribution-work-next81-syna-cdev-llseek") `
        --output (Join-Path $Reports "double_clean_rebuild_next81.json")
    Invoke-Logged "llm_cycle.log" {
        python .\workspace_tools\reconstruction_pipeline\verify_llm_reconstruction_cycle.py `
            ...
            --audit (Join-Path $Reports "double_clean_rebuild_next81.json")
    } | Out-Null
