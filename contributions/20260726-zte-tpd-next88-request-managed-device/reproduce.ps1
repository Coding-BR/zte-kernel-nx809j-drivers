...
try {
    ...
    python .\workspace_tools\reconstruction_pipeline\validate_reconstructed_drivers.py `
        ...
        --work-root (Join-Path $EngineeringRoot "validation\contribution-work-next88-request-managed-device") `
        --output (Join-Path $Reports "double_clean_rebuild_next88.json")
    Invoke-Logged "llm_cycle.log" {
        python .\workspace_tools\reconstruction_pipeline\verify_llm_reconstruction_cycle.py `
            ...
            --audit (Join-Path $Reports "double_clean_rebuild_next88.json")
    } | Out-Null
