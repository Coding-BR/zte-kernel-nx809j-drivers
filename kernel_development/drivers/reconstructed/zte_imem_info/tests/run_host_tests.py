#!/usr/bin/env python3
"""Build and run the zte_imem_info host harness twice."""
...
COVERED_FUNCTIONS = [
    "request_board_id", "ddr_id_proc_open", "ddr_id_read_proc",
    "board_id_proc_open", "board_id_read_proc", "init_module",
]
...
def main() -> int:
    ...
    output = engineering / "validation" / "zte_imem_info" / "host"
    ...
    passed = (reproducible and len(runs) == 2
              ...
              and all("PASS zte_imem_info host tests" in item["stdout"] for item in runs))
    payload = {
        "schema_version": "1.0", "generated_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
        "driver": "zte_imem_info", "source": str(driver / "zte_imem_info.c"),
