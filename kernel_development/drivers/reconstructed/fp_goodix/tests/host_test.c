...
void unregister_chrdev(int major, const char *name)
{
	...
	CHECK(strcmp(name, gf_driver.driver.name) == 0);
}
				     ...
				     dev_t dev, void *drvdata, const char *name)
{
	...
	CHECK(parent != NULL);
	CHECK(drvdata == &gf && strcmp(name, "goodix_fp") == 0);
...
int regulator_is_enabled(struct regulator *regulator) { return regulator ? regulator->enabled : 0; }
...
void mdelay(unsigned int msec) { (void)msec; }
...
static void test_full_lifecycle(void)
{
	...
	inode.i_rdev = gf.devt;
	CHECK(gf_open(&inode, &file) == 0);
	CHECK(file.private_data == &gf);
	CHECK(gf_ioctl(&file, 0x80016700, (unsigned long)&version) == 0);
	CHECK(version == 25);
	CHECK(gf_compat_ioctl(&file, 0x80016700, (unsigned long)&version) == 0);
	...
	goodixfp_init_drm_notifier(&gf.screen_state_work.work);
	...
	packet->header.nlmsg_pid = 42;
	nl_data_ready(packet);
	notification.notif_type = DRM_PANEL_EVENT_UNBLANK;
	goodix_fb_state_chg_callback(PANEL_EVENT_NOTIFICATION_PRIMARY, &notification, &gf);
...
static void test_failure_paths(void)
{
	...
	host_reset_state();
	gf.pdev = &host.platform_object;
	gf.zlog_client = (void *)0x77770000;
	host.fail_regulator_enable = 1;
	CHECK(gf_parse_dts(&gf) != 0);
	CHECK(host.zlog_notify_calls == 1);

	host_reset_state();
	host.fail_netlink = 1;
	CHECK(netlink_init() == 1);
	netlink_exit();
}
...
static void test_direct_helpers(void)
{
	host_reset_state();
	gf.pdev = &host.platform_object;
	gf.vdd_reg = &host.regulator_object;
	gf.reset_gpio = host.reset_desc.gpio;
	gf.irq_gpio = host.irq_desc.gpio;
	CHECK(gf_power_on(&gf) == 0);
	CHECK(gf_power_off(&gf) == 0);
	CHECK(gf_hw_reset(&gf, 5) == 0);
	CHECK(gf_irq_num(&gf) >= 0);
	gf_enable_irq();
	gf_disable_irq(&gf);
	CHECK(zte_goodix_pinctrl_init(&gf) == 0);
	CHECK(zte_goodix_pinctrl_select(&gf, true) == 0);
	gf_cleanup(&gf);
	nav_event_input(1);
}
...
int main(void)
{
	...
	if (failures)
		return 1;
	puts("PASS fp_goodix host tests");
	return 0;
}
