...
static int gpio_keys_show_keys(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	(void)attr;
	return gpio_keys_attr_show_helper(dev->ddata, buf, 1, false);
}
...
static bool test_forwards_keys_query(void)
{
	...
	reset_trace();
	REQUIRE(gpio_keys_show_keys(&dev, &attr, buffer) == 37);
...
static bool test_forwards_null_buffer_without_reordering(void)
{
	...
	reset_trace();
	REQUIRE(gpio_keys_show_keys(&dev, &attr, NULL) == 37);
...
int main(void)
{
	if (!test_forwards_keys_query() ||
	    !test_forwards_null_buffer_without_reordering())
		return 1;

	puts("PASS gpio_keys_show_keys host tests (2 scenarios)");
	return 0;
}
