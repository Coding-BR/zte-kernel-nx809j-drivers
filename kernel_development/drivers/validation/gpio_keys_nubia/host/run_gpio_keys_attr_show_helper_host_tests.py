#!/usr/bin/env python3
"""Build and run the gpio_keys_attr_show_helper host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_attr_show_helper host tests (3 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "gpio_keys_attr_show_helper_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-attr-show-helper",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_attr_show_helper_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": "gpio_keys_attr_show_helper",
        "covered_functions": ["gpio_keys_attr_show_helper"],
        "source": str(source),
        ...
        "limitations": [
            "The harness models the stock bitmap and formatting helpers.",
            "The AArch64 assembly body is validated separately by strict opcode comparison.",
            "The stock brk path for unsupported event types is not executed.",
        ],
    }
