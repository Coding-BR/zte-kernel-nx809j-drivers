#!/usr/bin/env python3
"""Build and run the gpio_keys probe host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys probe host tests (6 scenarios)\n"
...
def main() -> int:
    ...
    version = execute(["docker", "run", "--rm", "-v",
                       f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE,
                       CLANG, "--version"])
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-probe",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/gpio_keys_nubia/host/"
            "gpio_keys_probe_host_test.c", "-o", f"/output/{binary_name}",
        ]
        compiled = execute(command)
        run = (execute(["docker", "run", "--rm", "-e",
                        "ASAN_OPTIONS=detect_leaks=0", "-v",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_probe_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": ["gpio_keys_probe"],
        "covered_functions": ["gpio_keys_probe"],
        "source": str(source), "source_sha256": sha256(source),
        "expected_scenarios": 6, "repetitions": 2, "cycles": cycles,
        "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness models the platform, firmware-node, allocator and input subsystems.",
            "The AArch64 probe body is validated separately by strict opcode and relocation comparison.",
            "No hardware GPIO, IRQ, DT or input registration is exercised.",
        ],
    }
