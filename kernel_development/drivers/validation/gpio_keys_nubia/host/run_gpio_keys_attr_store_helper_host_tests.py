#!/usr/bin/env python3
"""Build and run the gpio_keys_attr_store_helper host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_attr_store_helper host tests (5 scenarios)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path, default=pathlib.Path(__file__).with_name("gpio_keys_attr_store_helper_host_test.c"))
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-fno-omit-frame-pointer", "-fno-pie", "-no-pie", "-frandom-seed=gpio-keys-nubia-attr-store-helper",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/gpio_keys_nubia/host/gpio_keys_attr_store_helper_host_test.c",
	...
	report = {
		...
		"mode": "offline_direct_source_gpio_keys_attr_store_helper_asan_ubsan",
		"driver": "gpio_keys_nubia",
		"target": "gpio_keys_attr_store_helper",
		"covered_functions": ["gpio_keys_attr_store_helper"],
		"source": str(source),
		...
		"limitations": [
			"The harness models kernel bitmap, IRQ, wake-IRQ, timer, workqueue, and mutex primitives.",
			"The AArch64 assembly body is validated separately by strict opcode and relocation comparison.",
		],
	}
