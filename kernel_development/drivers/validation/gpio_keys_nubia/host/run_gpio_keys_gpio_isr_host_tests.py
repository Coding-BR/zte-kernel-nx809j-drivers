#!/usr/bin/env python3
"""Build and run the gpio_keys_gpio_isr host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_gpio_isr host tests (4 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(__file__).with_name(
            "gpio_keys_gpio_isr_host_test.c"
        ),
    )
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-isr",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_gpio_isr_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": "gpio_keys_gpio_isr",
        "covered_functions": ["gpio_keys_gpio_isr"],
        "source": str(source),
        ...
        "limitations": [
            "The harness uses byte-addressed stand-ins for the observed driver offsets.",
            "The AArch64 assembly body is validated separately by strict opcode comparison.",
            "BUG_ON for an unrelated IRQ is not executed because it intentionally does not return.",
        ],
    }
