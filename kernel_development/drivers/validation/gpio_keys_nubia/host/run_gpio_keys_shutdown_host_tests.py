#!/usr/bin/env python3
"""Build and run the gpio_keys_shutdown host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_shutdown host tests (2 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        type=pathlib.Path,
        default=pathlib.Path(__file__).with_name("gpio_keys_shutdown_host_test.c"),
    )
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-fno-pie",
            "-no-pie",
            "-frandom-seed=gpio-keys-nubia-shutdown",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/gpio_keys_nubia/host/gpio_keys_shutdown_host_test.c",
            "-o",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_shutdown_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": "gpio_keys_shutdown",
        "covered_functions": ["gpio_keys_shutdown"],
        "source": str(source),
        ...
        "limitations": [
            "The harness validates the Ghidra-proven delegation and error branch with controlled stubs.",
            "The AArch64 assembly body is validated separately by the strict opcode and relocation comparison.",
        ],
    }
