#!/usr/bin/env python3
"""Build and run the gpio_keys_irq_timer host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_irq_timer host tests (2 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(__file__).with_name(
            "gpio_keys_irq_timer_host_test.c"
        ),
    )
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-irq-timer",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_irq_timer_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": "gpio_keys_irq_timer",
        "covered_functions": ["gpio_keys_irq_timer"],
        "source": str(source),
        ...
        "limitations": [
            "The harness uses a timer-to-button stand-in for from_timer().",
