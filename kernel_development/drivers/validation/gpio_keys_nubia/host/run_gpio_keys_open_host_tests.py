#!/usr/bin/env python3
"""Build and run the gpio_keys_open host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_open host tests (3 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        type=pathlib.Path,
        default=pathlib.Path(__file__).with_name("gpio_keys_open_host_test.c"),
    )
    ...
    for cycle in (1, 2):
        ...
        command = [
            ...
            "-frandom-seed=gpio-keys-nubia-open",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/gpio_keys_nubia/host/gpio_keys_open_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_open_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": "gpio_keys_open",
        "covered_functions": ["gpio_keys_open"],
        "source": str(source),
