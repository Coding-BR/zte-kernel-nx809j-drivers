#!/usr/bin/env python3
"""Build and run the gpio_keys lifecycle host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys lifecycle host tests (4 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "gpio_keys_lifecycle_host_test.c"))
    ...
    report = {
        ...
        "mode": "offline_direct_source_gpio_keys_lifecycle_asan_ubsan",
        "driver": "gpio_keys_nubia",
        "target": ["init_module", "cleanup_module"],
        "covered_functions": ["init_module", "cleanup_module"],
        "source": str(source), "source_sha256": sha256(source),
        "expected_scenarios": 4, "repetitions": 2, "cycles": cycles,
        "reproducible": reproducible, "reproducible_binary": reproducible,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": [
            "The harness uses stand-ins for the platform driver registry and module owner.",
            "The AArch64 lifecycle bodies are validated separately by strict opcode comparison.",
        ],
    }
