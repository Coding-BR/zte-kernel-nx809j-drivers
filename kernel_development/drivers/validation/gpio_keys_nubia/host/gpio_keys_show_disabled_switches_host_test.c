...
static int gpio_keys_show_disabled_switches(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	(void)attr;
	return gpio_keys_attr_show_helper(dev->ddata, buf, 5, true);
}
...
static bool test_forwards_disabled_switch_query(void)
{
	...
	reset_trace();
	REQUIRE(gpio_keys_show_disabled_switches(&dev, &attr, buffer) == 41);
	REQUIRE(helper_calls == 1 && last_ddata == &ddata && last_buf == buffer);
	REQUIRE(last_type == 5 && last_only_disabled);
	return true;
}
...
static bool test_forwards_null_buffer_without_reordering(void)
{
	...
	reset_trace();
	REQUIRE(gpio_keys_show_disabled_switches(&dev, &attr, NULL) == 41);
...
int main(void)
{
	if (!test_forwards_disabled_switch_query() ||
	    !test_forwards_null_buffer_without_reordering())
		return 1;
	puts("PASS gpio_keys_show_disabled_switches host tests (2 scenarios)");
	return 0;
}
