#!/usr/bin/env python3
"""Build and run the gpio_keys_show_disabled_keys host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS gpio_keys_show_disabled_keys host tests (2 scenarios)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
						default=pathlib.Path(__file__).with_name(
							"gpio_keys_show_disabled_keys_host_test.c"))
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
			...
			"-frandom-seed=gpio-keys-nubia-show-disabled-keys",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
			"-Wl,--build-id=none", "/drivers/validation/gpio_keys_nubia/host/"
			"gpio_keys_show_disabled_keys_host_test.c", "-o", f"/output/{binary_name}"]
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_gpio_keys_show_disabled_keys_asan_ubsan",
		"driver": "gpio_keys_nubia", "target": "gpio_keys_show_disabled_keys",
		"covered_functions": ["gpio_keys_show_disabled_keys"], "source": str(source),
		...
		"limitations": ["The harness uses a stand-in for gpio_keys_attr_show_helper.",
			"The AArch64 assembly body is validated separately by strict opcode comparison."]}
