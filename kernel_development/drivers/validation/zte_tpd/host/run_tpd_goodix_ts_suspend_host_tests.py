#!/usr/bin/env python3
"""Run the isolated tpd_goodix_ts_suspend host contract under sanitizers."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/tpd_goodix_ts_suspend_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/tpd_goodix_ts_suspend.c")
...
EXPECTED = "PASS tpd_goodix_ts_suspend host tests (2 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next90-goodix-suspend",
    ...
    report = {
        ...
        "mode": "offline_direct_source_next90_tpd_goodix_ts_suspend_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_goodix_ts_suspend",
        "covered_functions": ["tpd_goodix_ts_suspend"],
        "source": str(source),
        ...
        "limitations": [
            "The stock function dereferences pdev and has no null-input branch; the harness tests two valid layouts only.",
            "The harness proves pdev + 8, the +0x10 adjustment, return forwarding, and no extra call.",
            "Module assembly, KCFI, Ghidra and Joern remain independent gates.",
            "No smartphone, touch hardware or PM interaction is used.",
        ],
    }
