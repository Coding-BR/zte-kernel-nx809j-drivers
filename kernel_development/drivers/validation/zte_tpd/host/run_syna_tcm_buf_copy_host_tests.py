#!/usr/bin/env python3
"""Run the target-230 buffer-copy host contract under ASan and UBSan."""
...
HOST = Path("kernel_development/drivers/validation/zte_tpd/host/syna_tcm_buf_copy_host_test.c")
...
EXPECTED = "PASS syna_tcm_buf_copy_0 host tests (5 cases)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        compile_command = [
            "docker", "run", "--rm", "-v", f"{ROOT / 'kernel_development/drivers'}:/drivers:ro",
            ...
            "-Wl,--build-id=none", f"/drivers/{HOST.relative_to('kernel_development/drivers').as_posix()}",
            "-o", binary,
        ]
        ...
        passed = bool(
            compiled.returncode == 0 and executed and executed.returncode == 0
    ...
    report = {
        ...
        "mode": "offline_direct_source_next230_syna_tcm_buf_copy_asan_ubsan",
        "driver": "zte_tpd", "target": "syna_tcm_buf_copy", "covered_functions": ["syna_tcm_buf_copy"],
        "source": str(source), "source_sha256": sha256(source), "compiler": CLANG,
        "container_image": IMAGE, "toolchain_volume": TOOLCHAINS,
        "sanitizers": ["address", "undefined"], "expected_cases": 5, "repetitions": 2,
        "cycles": cycles, "reproducible_binary": len({c["binary_sha256"] for c in cycles}) == 1,
        "passed": passed, "status": "PASS" if passed else "FAIL",
        "limitations": ["The harness aliases the target-230 function name onto the existing five-case contract harness.",
