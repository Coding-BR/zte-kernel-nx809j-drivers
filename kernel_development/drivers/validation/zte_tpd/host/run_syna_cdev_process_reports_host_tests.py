"""Build and run the syna_cdev_process_reports host harness twice."""
...
EXPECTED_STDOUT = "PASS syna_cdev_process_reports host tests (6 cases)\n"
...
def main() -> int:
    ...
    source = root / "kernel_development" / "drivers" / "validation" / "zte_tpd" / "host" / "syna_cdev_process_reports_host_test.c"
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-Wno-sign-compare", "-Wno-ignored-attributes",
            "-Wno-unused-but-set-variable", "-Wno-sometimes-uninitialized",
            "-fno-omit-frame-pointer", "-fno-pie", "-no-pie",
            "-frandom-seed=zte-tpd-next195-process-reports",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_cdev_process_reports_host_test.c",
        ...
        passed = (
            ...
            and ran.stdout == EXPECTED_STDOUT
            and not ran.stderr
        )
        cycles.append({
            "cycle": cycle,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_cdev_process_reports_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_cdev_process_reports",
        "covered_functions": ["syna_cdev_process_reports"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers null arguments, missing managed device and allocation-failure cleanup contracts.",
            "FIFO linked-list insertion, contact payload processing and hardware callbacks remain separate gates.",
            "No smartphone, input subsystem or Synaptics hardware interaction is used.",
        ],
    }
