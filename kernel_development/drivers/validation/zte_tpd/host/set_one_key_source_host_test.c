...
static void fake_callback(__int64 cdev, _BOOL8 value)
{
    callback_calls++;
...
static void stale_callback(__int64 cdev, _BOOL8 value)
{
    (void)cdev;
    (void)value;
    callback_calls += 100;
...
#include "../../../reconstructed/zte_tpd/set_one_key.c"
...
static void fixture_init(struct fixture *fixture, int with_callback)
{
    ...
    store_ptr(fixture->device, 0xd38, (void *)stale_callback);
    store_ptr(fixture->device, 0xd40,
...
static void test_valid_zero_and_nonzero(void)
{
    const char *name = "valid_zero_and_nonzero";
    struct fixture fixture;
    struct file file = {0};
    loff_t offset = 0;

    fixture_init(&fixture, 1);
    expect(set_one_key(&file, "0", 1, &offset) == 1, name,
           "zero return length differs");
    expect(callback_calls == 1 && callback_value == 0, name,
           "zero was not normalized to false");
...
static void test_invalid_input_has_no_side_effect(void)
{
    ...
    fixture_init(&fixture, 1);
    expect(set_one_key(&file, "invalid", 7, &offset) == -EINVAL, name,
           "invalid input return differs");
    expect(callback_calls == 0 && printk_calls == 0, name,
           "invalid input caused side effects");
    expect(set_one_key(&file, "4294967296", 10, &offset) == -EINVAL, name,
...
int main(void)
{
    test_valid_zero_and_nonzero();
    test_invalid_input_has_no_side_effect();
    test_null_callback_and_empty_input();
    puts("PASS set_one_key source host tests (3 cases)");
    return 0;
}
