#!/usr/bin/env python3
"""Build and run the tpd_probe_work_deinit host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_probe_work_deinit host tests (2 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target383-probe-work-deinit",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/tpd_probe_work_deinit_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_tpd_probe_work_deinit_asan_ubsan",
              "driver": "zte_tpd", "target": "tpd_probe_work_deinit",
              "covered_functions": ["tpd_probe_work_deinit"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 2,
              ...
              "limitations": [
                  "printk and cancel_delayed_work_sync are stubbed; the harness checks the format, argument and offset contract.",
                  "No workqueue cancellation, timer scheduler or physical hardware is exercised.",
