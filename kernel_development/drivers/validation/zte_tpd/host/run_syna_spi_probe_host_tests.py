#!/usr/bin/env python3
"""Run the syna_spi_probe contract under ASan and UBSan twice."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_spi_probe_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_spi_probe.c")
...
EXPECTED = "PASS syna_spi_probe host tests (5 cases)\n"
...
def main() -> int:
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-target201-probe",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_spi_probe_asan_ubsan",
        "driver": "zte_tpd", "target": "syna_spi_probe",
        ...
        "limitations": [
            "The harness stubs Device Tree, SPI, GPIO, regulator, managed-memory and printk callbacks.",
            "It covers the default success path, SPI setup/allocation failures and two GPIO failure paths.",
