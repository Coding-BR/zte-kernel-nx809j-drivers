#!/usr/bin/env python3
"""Build and run the syna_dev_shutdown host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_shutdown host tests (2 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_dev_shutdown_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "-frandom-seed=zte-tpd-target159-shutdown", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_shutdown_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_dev_shutdown_asan_ubsan", "driver": "zte_tpd",
              "target": "syna_dev_shutdown", "covered_functions": ["syna_dev_shutdown"],
              "source": str(source), "source_sha256": sha256(source), "expected_scenarios": 2,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": ["The harness validates exact pointer delegation for a real and NULL platform_device pointer.",
                              "The delegated syna_dev_remove implementation is not re-executed here; it has its own attestation."]}
