#!/usr/bin/env python3
"""Run the isolated tpd_reset_gpio_output contract under ASan/UBSan."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/tpd_reset_gpio_output_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/tpd_reset_gpio_output.c")
...
EXPECTED = "PASS tpd_reset_gpio_output host tests (4 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-target123-reset-gpio-output",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_reset_gpio_output_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_reset_gpio_output",
        "covered_functions": ["tpd_reset_gpio_output"],
        "source": str(source),
        ...
        "limitations": [
            "The harness proves the null-callback, callback invocation and one-bit masking contract.",
            "Module assembly, KCFI, Ghidra and Joern remain independent gates.",
            "No smartphone, touch hardware or runtime kernel integration is exercised.",
        ],
    }
