#!/usr/bin/env python3
"""Build and run the panel-event registration harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_ts_register_for_panel_events host tests (4 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_ts_register_for_panel_events_host_test.c"))
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_ts_register_for_panel_events_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_ts_register_for_panel_events",
        "covered_functions": ["syna_ts_register_for_panel_events"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers registration failure, successful handle storage, null panel forwarding and adjacent-field preservation.",
            "No panel notifier framework, kernel callback dispatch, firmware or smartphone hardware is exercised.",
        ],
    }
