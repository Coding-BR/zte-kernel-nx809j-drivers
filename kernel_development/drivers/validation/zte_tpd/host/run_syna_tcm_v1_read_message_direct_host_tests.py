#!/usr/bin/env python3
"""Build and run the direct-source read-message harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_tcm_v1_read_message host tests (5 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_tcm_v1_read_message_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_tcm_v1_read_message.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-Wno-int-conversion", "-Wno-cast-function-type", "-Wno-sign-compare",
                   "-Wno-error=uninitialized", "-fno-omit-frame-pointer", "-fno-pie", "-no-pie",
                   "-frandom-seed=zte-tpd-target544-direct-read-message",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-fno-sanitize=alignment", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_tcm_v1_read_message_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_tcm_v1_read_message_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_v1_read_message",
        "covered_functions": ["syna_tcm_v1_read_message"],
        ...
        "alignment_sanitizer": "disabled due to decompiled pointer-offset accesses",
        "expected_cases": 5,
        "repetitions": 2,
        "cycles": cycles,
        ...
        "limitations": [
            "The harness includes the reconstructed source directly and stubs transport, managed memory, locks and callbacks.",
            "The five cases do not prove continued-read, response-dispatch, firmware transport, exact AArch64 identity, KCFI or hardware behavior."
        ],
    }
