#!/usr/bin/env python3
"""Build and run the tpd_get_singlefpgesture contract harness twice."""
...
def main() -> int:
    ...
    report = {
        "schema_version": 1,
        "driver": "zte_tpd",
        "target": "tpd_get_singlefpgesture",
        "mode": "offline_contract_model_asan_ubsan",
        "source": str(ROOT / "kernel_development/drivers/validation/zte_tpd/host/tpd_get_singlefpgesture_contract_test.c"),
        "source_sha256": sha256(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/tpd_get_singlefpgesture.c"),
        ...
        "status": "PASS" if all(item["passed"] for item in cycles) and len(hashes) == 1 and None not in hashes else "FAIL",
        "limitations": ["Fake memory only; real hardware execution is a separate gate.", "AArch64, KCFI, Joern and canonical build are separate gates."],
    }
