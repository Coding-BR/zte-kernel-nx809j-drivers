#!/usr/bin/env python3
"""Build and run the syna_dev_isr host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_isr host tests (5 cases)\n"
...
def main() -> int:
    ...
    source = root / "kernel_development" / "drivers" / "validation" / "zte_tpd" / "host" / "syna_dev_isr_host_test.c"
    driver_source = root / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "syna_dev_isr.c"
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next171-isr",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_isr_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_isr_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_isr",
        "covered_functions": ["syna_dev_isr"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers PM timeout, bypass, GPIO mismatch, event success and event failure.",
            "No smartphone, interrupt controller or Synaptics hardware interaction is used.",
        ],
    }
