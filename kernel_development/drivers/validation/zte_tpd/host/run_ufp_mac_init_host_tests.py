#!/usr/bin/env python3
"""Run the isolated ufp_mac_init initialization contract under sanitizers."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/ufp_mac_init_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/ufp_mac_init.c")
...
EXPECTED = "PASS ufp_mac_init host tests (3 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next92-ufp-mac-init",
    ...
    report = {
        ...
        "mode": "offline_direct_source_next92_ufp_mac_init_asan_ubsan",
        "driver": "zte_tpd",
        "target": "ufp_mac_init",
        "covered_functions": ["ufp_mac_init"],
        "source": str(source),
        ...
        "limitations": [
            "The harness proves the initialization layout, external call order, platform-device branch and retained NULL workqueue behavior.",
            "It intentionally does not invoke the asynchronous delayed-work callback or touch hardware.",
            "The raw tpd_cdev NULL-dereference behavior is not exercised because the stock function dereferences it unconditionally.",
