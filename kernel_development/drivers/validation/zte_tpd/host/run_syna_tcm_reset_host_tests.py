#!/usr/bin/env python3
"""Build and run the direct-source syna_tcm_reset oracle twice in Docker."""
...
EXPECTED = (
    "PASS test_null_tcm\n"
    "PASS test_send_success\n"
    "PASS test_send_failure\n"
    "PASS test_post_reset_success\n"
    "SUMMARY tests=4 failures=0\n"
)
...
def main() -> int:
    ...
    test_source = source.with_name("syna_tcm_reset_host_test.c")
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_tcm_reset.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "syna_tcm_reset_host_test"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/syna_tcm_reset_host_test.c",
            "-o", f"/output/{binary.name}",
        ]
        compiled = run(compile_command)
        executed = run([
            "docker", "run", "--rm", "-v", f"{cycle}:/output:ro", IMAGE,
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_tcm_reset_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_reset",
        ...
        "limitations": [
            "The host oracle uses executable x86_64 callback stubs and does not exercise AArch64 code directly.",
            "SPI, IRQ, MMIO, firmware transport, concurrency and physical hardware remain outside this test.",
        ],
    }
