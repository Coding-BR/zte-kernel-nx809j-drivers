#!/usr/bin/env python3
"""Run the direct get_fake_sleep source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS get_fake_sleep: 4 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "get_fake_sleep_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "get_fake_sleep.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("get_fake_sleep source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-Wl,--build-id=none", "-frandom-seed=zte-tpd-get-fake-sleep-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/get_fake_sleep_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "get_fake_sleep", "mode": "offline_direct_source_asan_ubsan",
