#!/usr/bin/env python3
"""Run the syna_mmap host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS syna_mmap host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation/zte_tpd/host/syna_mmap_host_test.c"
    source = drivers / "reconstructed/zte_tpd/syna_mmap.c"
    ...
    report = {
        ...
        "inputs": [
            {"path": str(source.relative_to(root)).replace("\\", "/"), "sha256": sha256(source)},
            {"path": str(harness.relative_to(root)).replace("\\", "/"), "sha256": sha256(harness)},
        ],
        "expected_cases": 4,
        "repetitions": 2,
        "cycles": cycles,
        ...
        "limitations": [
            "Valid file/private_data fixtures only; stock has no null guards.",
            "MMIO and NX809J runtime behavior are unverified.",
        ],
    }
