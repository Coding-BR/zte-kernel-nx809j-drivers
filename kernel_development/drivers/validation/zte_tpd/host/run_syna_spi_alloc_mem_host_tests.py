#!/usr/bin/env python3
"""Run the syna_spi_alloc_mem contract under Docker clang ASan/UBSan twice."""
...
SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_spi_alloc_mem_host_test.c")
DRIVER = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_spi_alloc_mem.c")
IMAGE = "nubia-sm8850-kernel-builder:latest"
TOOLCHAIN = "nubia_sm8850_kernel_toolchains"
CLANG = "/toolchains/clang-r536225/bin/clang"
EXPECTED = "PASS syna_spi_alloc_mem host tests (8 cases)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-target207-alloc", "-ffile-prefix-map=/drivers=<drivers>",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_spi_alloc_mem_asan_ubsan_docker",
        "driver": "zte_tpd", "target": "syna_spi_alloc_mem", "source": str(source),
        ...
        "limitations": [
            "The harness stubs managed allocation, free, printk and the device manager.",
            "It covers xfer allocation/reuse, rx/tx allocation, allocation failures, invalid device and signed-size boundary paths.",
