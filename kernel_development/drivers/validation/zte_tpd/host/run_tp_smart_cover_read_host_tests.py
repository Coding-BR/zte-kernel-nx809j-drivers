#!/usr/bin/env python3
"""Run the tp_smart_cover_read host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_smart_cover_read host tests (2 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "tp_smart_cover_read_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "tp_smart_cover_read.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-tp-smart-cover-read",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/tp_smart_cover_read_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates the +0xe38 callback offset, +0x4 value read, partial read and EOF contract with fake memory.",
