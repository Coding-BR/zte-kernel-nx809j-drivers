#!/usr/bin/env python3
"""Run the isolated syna_dev_set_screen_on_fp_mode contract under sanitizers."""
...
HOST_SOURCE = Path(
    ...
    "syna_dev_set_screen_on_fp_mode_host_test.c"
)
DRIVER_SOURCE = Path(
...
EXPECTED = "PASS syna_dev_set_screen_on_fp_mode host tests (3 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next94-syna-dev-set-screen-on-fp-mode",
    ...
    report = {
        ...
        "mode": "offline_direct_source_next94_syna_dev_set_screen_on_fp_mode_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_set_screen_on_fp_mode",
        "covered_functions": ["syna_dev_set_screen_on_fp_mode"],
        "source": str(source),
        "source_sha256": sha256(source),
        "compiler": CLANG,
        "compiler_version": run(
            ["docker", "run", "--rm", "-v", f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE, CLANG, "--version"]
        ).stdout.strip(),
        ...
        "inputs": [
            ...
            {
                "path": str(REPO_ROOT / DRIVER_SOURCE),
                "size": (REPO_ROOT / DRIVER_SOURCE).stat().st_size,
        ...
        "limitations": [
            "The harness covers only the NULL, successful and negative-return branches of this synchronous wrapper.",
            "It does not communicate with a touch controller, firmware transport, display, or hardware.",
