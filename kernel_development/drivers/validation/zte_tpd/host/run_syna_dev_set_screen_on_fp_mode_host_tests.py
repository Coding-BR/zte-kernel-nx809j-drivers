#!/usr/bin/env python3
"""Build and run the syna_dev_set_screen_on_fp_mode host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_set_screen_on_fp_mode host tests (3 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_set_screen_on_fp_mode_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target372-screen-fp",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_set_screen_on_fp_mode_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_syna_dev_set_screen_on_fp_mode_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_set_screen_on_fp_mode",
              "covered_functions": ["syna_dev_set_screen_on_fp_mode",
                                     "syna_tcm_set_dynamic_config"],
              "source": str(source), "source_sha256": sha256(source), "expected_cases": 3,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "The dynamic-config transport is stubbed; only argument, return and logging contracts are exercised.",
