...
#include "../../../reconstructed/zte_tpd/syna_tcm_testing_check_frame_data.c"
...
{
    reset_state();
    if (syna_tcm_testing_check_frame_data(1, 8, 1, 1, NULL, 1, 8) != 0 ||
        printk_calls != 1)
        failures++;
}
...
static void test_size_mismatch(void)
{
    ...
    reset_state();
    if (syna_tcm_testing_check_frame_data(1, 1, 1, 1, callback, 1, 1) != 0 ||
...
static void test_empty_frame(void)
{
    _DWORD *callback = (_DWORD *)(uintptr_t)frame_callback;
    reset_state();
    if (syna_tcm_testing_check_frame_data(1, 0, 0, 2, callback, 1, 0) != 1 ||
...
static void test_matrix_pass(void)
{
    ...
    reset_state();
    if (syna_tcm_testing_check_frame_data((__int64)(uintptr_t)frame, 8, 2, 2,
...
static void test_callback_failure(void)
{
    ...
    reset_state();
    if (syna_tcm_testing_check_frame_data(1, 8, 1, 2, callback, 1, 0) != 0 ||
...
int main(void)
{
    ...
    if (failures != 0)
        return 1;
    puts("PASS syna_tcm_testing_check_frame_data host tests (5 cases)");
    return 0;
}
