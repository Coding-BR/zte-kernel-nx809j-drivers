#!/usr/bin/env python3
"""Run the bounded array-data aggregator in the project Docker image."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "syna_tcm_testing_check_array_data_host_test.c "
        "-o /out/array_data_host_test && /out/array_data_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "array_data_host_test"
    ...
    report = {
        "schema_version": 1,
        "target": "syna_tcm_testing_check_array_data",
        "mode": "direct_source_asan_ubsan_host_oracle",
        "stock_contract": "Ghidra Assembly 0011df64",
        "expected_cases": 11,
        "cycles": cycles,
