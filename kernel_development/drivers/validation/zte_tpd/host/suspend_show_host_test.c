...
#include "../../../reconstructed/zte_tpd/suspend_show.c"

struct fixture {
	uint8_t device[0xe58 + sizeof(void *)];
	char output[32];
};
				 ...
				 int with_callback)
{
	...
	store_u8(fixture->device, 0x15, value);
	store_ptr(fixture->device, 0xca0, (void *)wrong_callback);
	if (with_callback)
		store_ptr(fixture->device, 0xe58, (void *)refresh_suspend);
	else
		store_ptr(fixture->device, 0xe58, NULL);
...
static void test_full_read_and_correct_callback_offset(void)
{
	...
	result = suspend_show(&file, fixture.output, sizeof(fixture.output),
				     &position);
	expect(result == 17, case_name, "unexpected read length");
	expect(memcmp(fixture.output, "tp suspend is: 1\n", 17) == 0,
...
static void test_partial_read_and_eof(void)
{
	...
	result = suspend_show(&file, fixture.output, 2, &position);
	expect(result == 2, case_name, "partial read length differs");
	expect(memcmp(fixture.output, "tp", 2) == 0, case_name,
...
int main(void)
{
	...
	puts("PASS suspend_show host tests (2 cases)");
	return 0;
}
