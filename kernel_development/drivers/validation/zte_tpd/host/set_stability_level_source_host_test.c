...
#include "../../../reconstructed/zte_tpd/set_stability_level.c"
...
static void fixture_init(struct fixture *fixture, int with_callback)
{
    ...
    store_ptr(fixture->device, 0xd70, (void *)stale_callback);
    store_ptr(fixture->device, 0xd78,
...
static void test_valid_values_and_callback(void)
{
    ...
    expect(set_stability_level(&file, "0", 1, &offset) == 1, name,
    ...
    expect(set_stability_level(&file, "7\n", 2, &offset) == 2, name,
...
static void test_invalid_input_has_no_side_effect(void)
{
    ...
    fixture_init(&fixture, 1);
    expect(set_stability_level(&file, "invalid", 7, &offset) == -EINVAL, name,
           "invalid input return differs");
    expect(callback_calls == 0 && printk_calls == 0, name,
           "invalid input caused side effects");
    expect(set_stability_level(&file, "4294967296", 10, &offset) == -EINVAL,
           name, "out of range return differs");
    expect(callback_calls == 0 && printk_calls == 0, name,
...
int main(void)
{
    ...
    puts("PASS set_stability_level source host tests (3 cases)");
    return 0;
}
