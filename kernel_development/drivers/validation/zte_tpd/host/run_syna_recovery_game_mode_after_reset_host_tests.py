#!/usr/bin/env python3
"""Build and run the syna_recovery_game_mode_after_reset host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_recovery_game_mode_after_reset host tests (5 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path, default=pathlib.Path(__file__).with_name("syna_recovery_game_mode_after_reset_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v", f"{cycle_root}:/output", "-v", f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE, CLANG, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra", "-Werror", "-fno-omit-frame-pointer", "-fno-pie", "-no-pie", "-D", "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-next151-recovery-game", "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined", "-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/syna_recovery_game_mode_after_reset_host_test.c", "-o", f"/output/{binary_name}"]
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "mode": "offline_direct_source_syna_recovery_game_mode_after_reset_asan_ubsan", "driver": "zte_tpd", "target": "syna_recovery_game_mode_after_reset", "covered_functions": ["syna_recovery_game_mode_after_reset"], "source": str(source), "source_sha256": sha256(source), "expected_scenarios": 5, "repetitions": 2, "cycles": cycles, "reproducible": reproducible, "reproducible_binary": reproducible, "passed": passed, "status": "PASS" if passed else "FAIL", "limitations": ["The harness covers NULL rejection, normal mode, fallback delay selection, game-mode sequence and continuation across all operation failures.", "No smartphone, firmware transport or physical hardware is exercised."]}
