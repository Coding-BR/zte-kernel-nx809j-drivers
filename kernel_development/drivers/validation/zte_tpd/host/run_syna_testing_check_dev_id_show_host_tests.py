#!/usr/bin/env python3
"""Build and run the check-device-ID sysfs harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_testing_check_dev_id_show host tests (7 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		...
		default=pathlib.Path(__file__).with_name(
			"syna_testing_check_dev_id_show_host_test.c"
		),
	)
	...
	driver_source = (
		...
		/ "reconstructed"
		/ "zte_tpd"
		/ "syna_testing_check_dev_id_show.c"
	)
	...
	for cycle in (1, 2):
		...
		command = [
			...
			"-fno-pie",
			"-no-pie",
			"-frandom-seed=zte-tpd-next52-testing-check-dev-id",
	...
	report = {
		...
		"mode": "offline_direct_source_next52_testing_check_dev_id_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_testing_check_dev_id_show",
		"covered_functions": ["syna_testing_check_dev_id_show"],
		"source": str(source),
