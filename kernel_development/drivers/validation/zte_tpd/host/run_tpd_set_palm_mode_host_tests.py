#!/usr/bin/env python3
"""Build and run the tpd_set_palm_mode contract harness twice."""
...
EXPECTED = "PASS tpd_set_palm_mode contract test (7 checks)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        command = [
            ...
            "-include stdarg.h -fno-omit-frame-pointer -fno-pie -no-pie "
            "-fsanitize=address,undefined -Wl,--build-id=none "
            "-I/drivers/reconstructed/zte_tpd "
            "/drivers/validation/zte_tpd/host/tpd_set_palm_mode_contract_test.c "
            "-o /out/tpd_set_palm_mode_contract_test",
        ]
        compiled = subprocess.run(command, text=True, capture_output=True, check=False)
        executed = None
        binary = cycle / "tpd_set_palm_mode_contract_test"
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "--network", "none", "-v",
                 f"{cycle.resolve()}:/out:ro", IMAGE,
                 "/out/tpd_set_palm_mode_contract_test"],
    ...
    report = {"schema_version": 1, "driver": "zte_tpd", "target": "tpd_set_palm_mode",
              "mode": "offline_contract_model_asan_ubsan",
              "source": str(ROOT / "kernel_development/drivers/validation/zte_tpd/host/tpd_set_palm_mode_contract_test.c"),
              "source_sha256": sha256(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/tpd_set_palm_mode.c"),
              "expected_output": EXPECTED, "cycles": cycles,
              "reproducible_binary": passed, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": ["Stubbed printk; real hardware execution is a separate gate.", "AArch64, KCFI, Joern and canonical build are separate gates."]}
