#!/usr/bin/env python3
"""Run the ufp_report_lcd_state_work callback harness twice with sanitizers."""
...
def main() -> int:
    ...
    source = pathlib.Path(__file__).with_name("ufp_report_lcd_state_work_host_test.c").resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "ufp_report_lcd_state_work.c"
    ...
    for cycle in (1, 2):
        ...
        result = subprocess.run([
            ...
            "-frandom-seed=zte-tpd-ufp-report-lcd-state-work",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/ufp_report_lcd_state_work_host_test.c",
            "-o", "/output/host_test_asan_ubsan",
        ], text=True, capture_output=True, check=False)
        executed = subprocess.run([
            "docker", "run", "--rm", "-v", f"{output}:/output:ro", args.image,
            "/output/host_test_asan_ubsan",
        ], text=True, capture_output=True, check=False) if result.returncode == 0 else None
        passed = bool(executed and executed.returncode == 0 and
                      executed.stdout == "PASS ufp_report_lcd_state_work host tests (1 case)\n" and
                      executed.stderr == "")
        cycles.append({"cycle": cycle, "compile_returncode": result.returncode,
                       "compile_stdout": result.stdout, "compile_stderr": result.stderr,
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              ...
              "target": "ufp_report_lcd_state_work",
              "covered_functions": ["ufp_report_lcd_state_work"],
              ...
              "expected_cases": 1, "repetitions": 2, "cycles": cycles,
