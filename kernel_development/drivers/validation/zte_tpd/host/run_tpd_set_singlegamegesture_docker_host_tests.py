#!/usr/bin/env python3
"""Build and run the tpd_set_singlegamegesture contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_set_singlegamegesture contract test (8 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_set_singlegamegesture_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_set_singlegamegesture.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_set_singlegamegesture_contract_asan_ubsan"
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-set-singlegamegesture",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_set_singlegamegesture_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_set_singlegamegesture_docker_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_set_singlegamegesture",
        "function": "tpd_set_singlegamegesture",
        ...
        "limitations": [
            "The harness validates positive, zero, negative and inactive branches with fake memory.",
