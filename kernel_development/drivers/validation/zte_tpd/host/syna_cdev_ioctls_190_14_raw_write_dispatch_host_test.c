...
static int64_t raw_write_result;

static int64_t raw_write_spy(int64_t tcm_hcd, uint64_t buffer,
...
#include "../../../reconstructed/zte_tpd/routes/syna_cdev_ioctls_190_14_raw_write_dispatch.c"
...
static int test_positive_result_and_exact_arguments(void)
{
	...
	reset_observation();
	raw_write_result = INT64_C(0x123456789);
	if (expect(syna_cdev_ioctls_190_14_raw_write_dispatch(
			tcm_hcd, buffer, request, length, raw_write_spy) ==
			raw_write_result, "positive result was not forwarded"))
		return 1;
	return expect(raw_write_calls == 1 && observed_tcm_hcd == tcm_hcd &&
			      observed_buffer == buffer && observed_request == request &&
			      observed_length == length,
			      "raw-write arguments were not preserved");
}

static int test_negative_result_is_forwarded(void)
{
	reset_observation();
	raw_write_result = INT64_C(-5);
	if (expect(syna_cdev_ioctls_190_14_raw_write_dispatch(
			INT64_C(-1), UINT64_C(0x8000000000000001), INT64_C(-2),
			UINT32_C(0xffffffff), raw_write_spy) == raw_write_result,
			"negative result was not forwarded"))
		return 1;
	return expect(raw_write_calls == 1 && observed_tcm_hcd == INT64_C(-1) &&
...
int main(void)
{
	...
	if (test_negative_result_is_forwarded())
		return 1;
	puts("PASS syna_cdev_ioctls_190_14_raw_write_dispatch: 2 cases");
	return 0;
}
