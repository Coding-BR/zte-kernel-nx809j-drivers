#!/usr/bin/env python3
"""Run the direct tp_fold_state_read source contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tp_fold_state_read: 4 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_fold_state_read_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_fold_state_read.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_fold_state_read source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-fold-state-read-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_fold_state_read_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_fold_state_read", "mode": "offline_direct_source_asan_ubsan",
