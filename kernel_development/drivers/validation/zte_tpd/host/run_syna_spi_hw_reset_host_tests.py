#!/usr/bin/env python3
"""Build and run the syna_spi_hw_reset harness twice with sanitizers."""
...
EXPECTED_STDOUT = "PASS syna_spi_hw_reset host tests (4 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        type=pathlib.Path,
        default=pathlib.Path(__file__).with_name("syna_spi_hw_reset_host_test.c"),
    )
    ...
    driver_source = (
        source.parents[3] / "reconstructed" / "zte_tpd" / "syna_spi_hw_reset.c"
    )
    layout_header = (
        source.parents[3] / "reconstructed" / "zte_tpd" / "zte_tpd_syna_layout.h"
    )
    ...
    report = {
        ...
        "mode": "offline_direct_source_next22_hw_reset_asan_ubsan",
        "target": "syna_spi_hw_reset",
        "source": str(source),
        ...
        "limitations": [
            "The harness stubs GPIO, sleep and printk primitives.",
            "It proves branch, polarity, timing, call-order and log behavior.",
            "Assembly, KCFI and Ghidra equivalence remain independent gates.",
            "No smartphone or physical GPIO controller is used.",
        ],
    }
