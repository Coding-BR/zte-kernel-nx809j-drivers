#!/usr/bin/env python3
"""Build and run the tpd_set_follow_hand_level contract oracle twice in Docker."""
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_set_follow_hand_level_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_set_follow_hand_level.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_set_follow_hand_level_contract_asan_ubsan"
        compile_command = [
            ...
            "-Wl,--build-id=none", "-frandom-seed=zte-tpd-tpd-set-follow-hand-level",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_set_follow_hand_level_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_set_follow_hand_level_docker_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_set_follow_hand_level", "function": "tpd_set_follow_hand_level",
        ...
        "limitations": [
            "The harness stubs syna_dev_set_follow_hand_level and uses fake state/hardware memory.",
