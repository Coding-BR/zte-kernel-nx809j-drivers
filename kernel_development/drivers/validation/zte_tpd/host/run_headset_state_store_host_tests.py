#!/usr/bin/env python3
"""Run the headset_state_store host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS headset_state_store host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "headset_state_store_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "headset_state_store.c"
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-headset-state-store",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/headset_state_store_host_test.c",
    ...
    report = {
        ...
        "limitations": [
            "The harness validates the +0xe70 callback offset, bool normalization, nine-byte cap, -EINVAL and count-return contracts with fake memory.",
