#!/usr/bin/env python3
"""Build and run the ufp_report_gesture_uevent host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS ufp_report_gesture_uevent host tests (3 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("ufp_report_gesture_uevent_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next132-gesture", "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/ufp_report_gesture_uevent_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_ufp_report_gesture_uevent_asan_ubsan",
        "driver": "zte_tpd",
        "target": "ufp_report_gesture_uevent",
        "covered_functions": ["ufp_report_gesture_uevent"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers single tap, double tap and unknown gesture strings, event order, wakeup duration, pending state, workqueue and delay.",
