#!/usr/bin/env python3
"""Run the bounded identification-info parser in the project Docker image."""
...
def main() -> int:
    ...
    compile_command = (
        ...
        "syna_tcm_v1_parse_idinfo_host_test.c "
        "-o /out/parse_idinfo_host_test && /out/parse_idinfo_host_test"
    )
    cycles = []
    for cycle in (1, 2):
        output_dir = build_root / f"cycle{cycle}"
        output_dir.mkdir(parents=True, exist_ok=True)
        binary = output_dir / "parse_idinfo_host_test"
    ...
    report = {
        "schema_version": 1,
        "target": "syna_tcm_v1_parse_idinfo",
        "mode": "direct_source_asan_ubsan_host_oracle",
        "stock_contract": "Ghidra Assembly 0011f6d0",
        "expected_cases": 7,
        "cycles": cycles,
        "sanitizer": {
            "asan": True,
            ...
            "alignment": False,
            "alignment_reason": (
                "Stock AArch64 uses an unaligned 32-bit load at object offset 0x92; "
                "AArch64 permits it while the C alignment sanitizer rejects the "
                "decompiler-style cast."
            ),
        },
