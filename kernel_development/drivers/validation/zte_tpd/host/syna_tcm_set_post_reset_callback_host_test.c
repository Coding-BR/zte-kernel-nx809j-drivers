...
typedef void (*tcm_post_reset_callback_fn)(void);

struct tcm_dev {
	uint8_t reserved[0x23d8];
...
#include "../../../reconstructed/zte_tpd/syna_tcm_set_post_reset_callback.c"
#undef printk
...
static int test_store_and_overwrite(void)
{
	...
	if (syna_tcm_set_post_reset_callback(&tcm, callback_one,
					 &first_context) != 0 ||
	    tcm.post_reset_callback != callback_one ||
	    tcm.post_reset_context != &first_context) {
		fprintf(stderr, "store contract mismatch\n");
		return 1;
	}
	if (syna_tcm_set_post_reset_callback(&tcm, callback_two,
					 &second_context) != 0 ||
	    tcm.post_reset_callback != callback_two ||
	    tcm.post_reset_context != &second_context) {
...
static int test_null_tcm(void)
{
	...
	if (syna_tcm_set_post_reset_callback(NULL, callback_one, NULL) != -241) {
		fprintf(stderr, "null path return mismatch\n");
		return 1;
	}
	if (printk_calls != calls_before + 1 ||
	    strcmp(captured_format, expected_format) != 0 ||
	    strcmp(captured_name, "syna_tcm_set_post_reset_callback") != 0) {
...
int main(void)
{
	...
	printf("PASS syna_tcm_set_post_reset_callback host tests (2 cases)\n");
	return 0;
}
