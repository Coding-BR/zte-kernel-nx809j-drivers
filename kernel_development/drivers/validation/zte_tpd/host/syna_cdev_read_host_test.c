...
static unsigned int observed_read_length;
...
static __int64 syna_cdev_ioctl_raw_read(__int64 tcm_hcd, unsigned __int64 buffer,
                                        __int64 request, unsigned int read_length)
{
    raw_read_calls++;
    observed_tcm_hcd = tcm_hcd;
    observed_buffer = (__int64)buffer;
    observed_request = request;
    observed_read_length = read_length;
...
#include "../../../reconstructed/zte_tpd/syna_cdev_read.c"
...
static int test_zero_length_short_circuits(void)
{
    ...
    reset_environment();
    if (syna_cdev_read(&file, buffer, 0, &offset) != 0)
        return fprintf(stderr, "zero-length return mismatch\n"), 1;
    return expect(mutex_lock_calls == 0 && mutex_unlock_calls == 0 &&
                  raw_read_calls == 0 && printk_calls == 0,
                  "zero-length side effects");
}

static int test_full_read_preserves_arguments_and_unlocks(void)
{
    struct file file = {0};
    ...
    raw_read_result = (int)sizeof(buffer);
    result = syna_cdev_read(&file, buffer, sizeof(buffer), &offset);
    if (expect(result == (ssize_t)sizeof(buffer), "full-read return"))
        return 1;
    return expect(mutex_lock_calls == 1 && mutex_unlock_calls == 1 &&
                  raw_read_calls == 1 && printk_calls == 0 &&
                  observed_tcm_hcd == (__int64)expected_hcd &&
                  observed_buffer == (__int64)(uintptr_t)buffer &&
                  observed_request == (unsigned int)sizeof(buffer) &&
                  observed_read_length == sizeof(buffer),
                  "full-read arguments or lock contract");
}

static int test_partial_read_logs_requested_and_returned_counts(void)
{
    struct file file = {0};
    struct tcm_storage tcm = {0};
    char buffer[31] = {0};
    ...
    if (expect(result == 7 && mutex_lock_calls == 1 && mutex_unlock_calls == 1 &&
               raw_read_calls == 1 && printk_calls == 1,
               "partial-read call contract"))
        return 1;
    return expect(strcmp(observed_format,
                         "\0013[error] %s: Invalid read operation, request:%d, return:%d\n") == 0 &&
                  strcmp(observed_name, "syna_cdev_read") == 0 &&
                  observed_log_request == sizeof(buffer) && observed_log_result == 7,
                  "partial-read log contract");
}
...
static int test_negative_error_and_u32_request_truncation(void)
{
    ...
    g_cdev_data = (__int64)(uintptr_t)&tcm;
    raw_read_result = -52;
    result = syna_cdev_read(&file, buffer, count, &offset);
    if (expect(result == -52 && mutex_lock_calls == 1 && mutex_unlock_calls == 1 &&
               raw_read_calls == 1 && printk_calls == 1,
               "negative-error call contract"))
        return 1;
    return expect(observed_request == 4 && observed_read_length == 4 &&
...
int main(void)
{
    if (test_zero_length_short_circuits() ||
        test_full_read_preserves_arguments_and_unlocks() ||
        test_partial_read_logs_requested_and_returned_counts() ||
        test_negative_error_and_u32_request_truncation())
        return 1;
    puts("PASS syna_cdev_read host tests (4 cases)");
    return 0;
}
