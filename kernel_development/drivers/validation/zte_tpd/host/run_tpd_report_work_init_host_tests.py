#!/usr/bin/env python3
"""Build and run the tpd_report_work_init host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_report_work_init host tests (2 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("tpd_report_work_init_host_test.c"))
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "tpd_report_work_init.c"
	...
	version_command = ["docker", "run", "--rm", "-v",
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm",
			...
			"-no-pie", "-frandom-seed=zte-tpd-next104-report-work-init",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/"
			"tpd_report_work_init_host_test.c", "-o", f"/output/{binary_name}"]
		compile_result = execute(command)
		run_command = ["docker", "run", "--rm", "-v",
			f"{cycle_root}:/output:ro", IMAGE, f"/output/{binary_name}"]
	...
	report = {"schema_version": 1,
		...
		"mode": "offline_direct_source_next104_tpd_report_work_init_asan_ubsan",
		"driver": "zte_tpd", "target": "tpd_report_work_init",
		"covered_functions": ["tpd_report_work_init"], "source": str(source),
		...
		"sanitizers": ["address", "undefined"], "expected_cases": 2,
		...
		"limitations": [
			"The harness proves allocation failure, ten stock work-item layouts, callbacks, timer offsets, flags and call count.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone, workqueue scheduler or Synaptics hardware interaction is used."]}
