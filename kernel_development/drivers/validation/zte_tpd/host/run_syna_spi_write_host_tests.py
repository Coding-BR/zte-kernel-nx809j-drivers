#!/usr/bin/env python3
"""Run the syna_spi_write contract under Docker clang ASan and UBSan twice."""
...
SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_spi_write_host_test.c")
DRIVER = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_spi_write.c")
IMAGE = "nubia-sm8850-kernel-builder:latest"
TOOLCHAIN = "nubia_sm8850_kernel_toolchains"
CLANG = "/toolchains/clang-r536225/bin/clang"
EXPECTED = "PASS syna_spi_write host tests (8 cases)\n"
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-target204-write", "-ffile-prefix-map=/drivers=<drivers>",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_spi_write_asan_ubsan_docker",
        "driver": "zte_tpd", "target": "syna_spi_write", "source": str(source),
        ...
        "limitations": [
            ...
            "It covers missing context/device, invalid length, allocation and SPI failures, null input, default success and segmented success.",
