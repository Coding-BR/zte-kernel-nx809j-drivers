#!/usr/bin/env python3
"""Build and run the tpd_get_sensibility_level contract harness twice."""
...
def main() -> int:
    ...
    for number in (1, 2):
        ...
        command = [
            ...
            "-o /out/tpd_get_sensibility_level_contract_test",
        ]
        compiled = subprocess.run(command, text=True, capture_output=True, check=False)
        executed = None
        binary = cycle / "tpd_get_sensibility_level_contract_test"
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "--network", "none", "-v",
                 f"{cycle.resolve()}:/out:ro", IMAGE,
                 "/out/tpd_get_sensibility_level_contract_test"],
    ...
    report = {
        "schema_version": 1,
        "driver": "zte_tpd",
        "target": "tpd_get_sensibility_level",
        "mode": "offline_contract_model_asan_ubsan",
        "source": str(ROOT / "kernel_development/drivers/validation/zte_tpd/host/tpd_get_sensibility_level_contract_test.c"),
        "source_sha256": sha256(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/tpd_get_sensibility_level.c"),
