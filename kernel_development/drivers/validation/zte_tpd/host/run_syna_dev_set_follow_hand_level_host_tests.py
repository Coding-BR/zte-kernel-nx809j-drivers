#!/usr/bin/env python3
"""Build and run the syna_dev_set_follow_hand_level host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_set_follow_hand_level host tests (11 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_set_follow_hand_level_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-next147-follow-hand", "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_set_follow_hand_level_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_set_follow_hand_level_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_set_follow_hand_level",
        "covered_functions": ["syna_dev_set_follow_hand_level"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers NULL input, all valid levels, the 0x5fc/0x604 offset encoding, invalid levels and negative dynamic-config propagation.",
            "No smartphone, firmware transport or physical hardware is exercised.",
        ],
    }
