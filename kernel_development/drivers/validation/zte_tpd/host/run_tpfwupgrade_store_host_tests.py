#!/usr/bin/env python3
"""Run the tpfwupgrade_store host contract twice with ASan/UBSan."""
...
EXPECTED = "PASS tpfwupgrade_store host tests (5 cases)\n"
...
def main() -> int:
	...
	harness = drivers / "validation" / "zte_tpd" / "host" / "tpfwupgrade_store_host_test.c"
	source = drivers / "reconstructed" / "zte_tpd" / "tpfwupgrade_store.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-frandom-seed=zte-tpd-tpfwupgrade-store",
			"-ffile-prefix-map=/drivers=<drivers>",
			"-fsanitize=address,undefined", "-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/tpfwupgrade_store_host_test.c",
	...
	report = {
		...
		"limitations": [
			"The harness validates parse failure, mutex pairing, cleanup, allocation, zeroing, callback and ENOMEM contracts with fake memory.",
