#!/usr/bin/env python3
"""Build and run the direct-source max-RW-size harness twice in Docker."""
...
EXPECTED_STDOUT = "syna_tcm_v1_set_up_max_rw_size host oracle: PASS\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_tcm_v1_set_up_max_rw_size_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_tcm_v1_set_up_max_rw_size.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target262-direct-max-rw",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_tcm_v1_set_up_max_rw_size_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_tcm_v1_set_up_max_rw_size_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_tcm_v1_set_up_max_rw_size",
        "covered_functions": ["syna_tcm_v1_set_up_max_rw_size"],
        "stock_contract": "Ghidra Assembly 00120da8",
        "source": str(source),
        ...
        "limitations": [
            "The harness includes the reconstructed source directly and stubs printk/barrier only.",
            "The tests do not prove AArch64 opcode/relocation identity, KCFI, whole-module ABI or physical hardware behavior."
        ],
    }
