#!/usr/bin/env python3
"""Run the recovered input-release sequence twice with ASan and UBSan."""
...
def main() -> int:
    ...
    source = pathlib.Path(__file__).with_name("syna_dev_free_input_events_host_test.c").resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_dev_free_input_events.c"
    ...
    expected = "PASS syna_dev_free_input_events host tests (4 cases)\n"
    cycles = []
    for cycle in (1, 2):
        ...
        compiled = subprocess.run([
            ...
            "-frandom-seed=zte-tpd-free-input-events",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_free_input_events_host_test.c",
    ...
    report = {
        ...
        "driver": "zte_tpd",
        "target": "syna_dev_free_input_events",
        "covered_functions": ["syna_dev_free_input_events"],
        ...
        "limitations": [
            "The host path supplies zero for the AArch64 w2 transient register.",
            "The test proves event ordering, fixed offsets and flag predicates only.",
            "Input subsystem integration, Android userspace and NX809J hardware remain untested.",
