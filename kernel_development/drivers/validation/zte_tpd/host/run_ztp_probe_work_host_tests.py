...
EXPECTED_STDOUT = "PASS ztp_probe_work host tests (2 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("ztp_probe_work_host_test.c"))
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next62_ztp_probe_work_asan_ubsan", "driver": "zte_tpd",
		"target": "ztp_probe_work", "covered_functions": ["ztp_probe_work"],
		...
		"limitations": ["The harness proves the two stock calls, their order, the ignored callback argument and the ignored callee return.",
