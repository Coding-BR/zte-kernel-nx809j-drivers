#!/usr/bin/env python3
"""Build and run the tpd_enable_wakegesture contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_enable_wakegesture contract test (4 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_enable_wakegesture_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_enable_wakegesture.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_enable_wakegesture_contract_asan_ubsan"
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-enable-wakegesture",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_enable_wakegesture_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_enable_wakegesture_docker_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_enable_wakegesture",
        "function": "tpd_enable_wakegesture",
        ...
        "limitations": [
            "The harness validates both observed branches and offsets with fake memory.",
            "It does not execute the real touchscreen driver, power framework or hardware.",
        ],
    }
