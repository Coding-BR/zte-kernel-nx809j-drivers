#!/usr/bin/env python3
"""Compile and execute the tpd_id3_report_work host contract twice."""
...
def main() -> int:
    ...
    source = "/drivers/validation/zte_tpd/host/tpd_id3_report_work_host_test.c"
    cycles = []
    for number in (1, 2):
        cycle = args.build_root / f"cycle{number}"
        cycle.mkdir(parents=True, exist_ok=True)
        binary = cycle / "tpd_id3_report_work_host_test"
        command = [
            ...
            f"{source} -o /out/tpd_id3_report_work_host_test && "
            "/out/tpd_id3_report_work_host_test",
        ]
        ...
        cycles.append({
            ...
            "passed": result.returncode == 0 and result.stdout ==
                      "PASS tpd_id3_report_work host tests (2 cases)\n" and
                      not result.stderr,
        })
