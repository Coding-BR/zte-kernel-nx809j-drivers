...
EXPECTED_STDOUT = "PASS syna_dev_set_tp_report_rate host tests (4 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_set_tp_report_rate_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-target373-report-rate",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_set_tp_report_rate_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "driver": "zte_tpd", "target": "syna_dev_set_tp_report_rate",
              "covered_functions": ["syna_dev_set_tp_report_rate", "syna_tcm_set_dynamic_config"],
              "source": str(source), "source_sha256": sha256(source), "expected_cases": 4,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "The dynamic-config transport is stubbed; only mode mapping and return contracts are exercised.",
