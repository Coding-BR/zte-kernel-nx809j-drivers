...
static int refresh_glove(struct ztp_device *cdev)
{
	callback_calls++;
...
#include "../../../reconstructed/zte_tpd/tp_glove_read.c"

struct fixture {
	uint8_t device[0xe58];
	char output[32];
};
				...
				int with_callback)
{
	...
	store_u32(fixture->device, 8, value);
	store_ptr(fixture->device, 0xc90, (void *)wrong_callback);
	if (with_callback)
		store_ptr(fixture->device, 0xe48, (void *)refresh_glove);
	else
		store_ptr(fixture->device, 0xe48, NULL);
...
static void test_full_read_and_correct_callback_offset(void)
{
	...
	fixture_init(&fixture, 12, 1);
	result = tp_glove_read(&file, fixture.output, sizeof(fixture.output),
				       &position);
	expect(result == 3, case_name, "unexpected read length");
	expect(memcmp(fixture.output, "12\n", 3) == 0, case_name,
	       "unexpected formatted value");
	expect(position == 3, case_name, "position not advanced");
	...
	expect(printk_calls == 1 && last_logged_value == 12, case_name,
...
static void test_partial_read_and_eof(void)
{
	...
	result = tp_glove_read(&file, fixture.output, 1, &position);
	expect(result == 1, case_name, "partial read length differs");
	expect(fixture.output[0] == '4', case_name,
...
int main(void)
{
	...
	puts("PASS tp_glove_read host tests (2 cases)");
	return 0;
}
