...
#include "../../../reconstructed/zte_tpd/tpd_goodix_ts_resume.c"
...
static void test_forwarding(int marker, int expected_result)
{
	...
	resume_calls = 0;
	resume_result = expected_result;

	expect_int("resume result", tpd_goodix_ts_resume(&pdev), expected_result);
	expect_int("syna_dev_resume calls", (int)resume_calls, 1);
...
int main(void)
{
	...
	if (failures != 0)
		return 1;
	puts("PASS tpd_goodix_ts_resume host tests (2 cases)");
	return 0;
}
