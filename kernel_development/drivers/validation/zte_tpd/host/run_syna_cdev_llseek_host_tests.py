#!/usr/bin/env python3
"""Build and run the syna_cdev_llseek host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_cdev_llseek host tests (5 cases)\n"
...
def main() -> int:
	...
	parser.add_argument(
		"--source",
		type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("syna_cdev_llseek_host_test.c"),
	)
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_cdev_llseek.c"
	...
	for cycle in (1, 2):
		...
		compile_command = [
			...
			"-frandom-seed=zte-tpd-next81-syna-cdev-llseek",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
			"-Wl,--build-id=none",
			"/drivers/validation/zte_tpd/host/syna_cdev_llseek_host_test.c",
	...
	report = {
		...
		"mode": "offline_direct_source_next81_syna_cdev_llseek_asan_ubsan",
		"driver": "zte_tpd",
		"target": "syna_cdev_llseek",
		"covered_functions": ["syna_cdev_llseek"],
		"source": str(source),
		...
		"limitations": [
			"The harness proves an exact -EINVAL result for ignored arguments.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone, VFS core or character-device interaction is used.",
		],
	}
