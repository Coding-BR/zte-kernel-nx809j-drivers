#!/usr/bin/env python3
"""Run the ufp_mac_exit teardown contract twice with ASAN and UBSAN."""
...
def main() -> int:
    ...
    source = pathlib.Path(__file__).with_name("ufp_mac_exit_host_test.c").resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "ufp_mac_exit.c"
    ...
    expected = "PASS ufp_mac_exit host tests (2 cases)\n"
    cycles = []
    for cycle in (1, 2):
        ...
        compiled = subprocess.run([
            ...
            "-frandom-seed=zte-tpd-ufp-mac-exit",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/ufp_mac_exit_host_test.c",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              ...
              "target": "ufp_mac_exit", "covered_functions": ["ufp_mac_exit"],
              ...
              "limitations": ["The harness proves recovered teardown ordering and arguments only.",
                              "It does not model actual workqueue ownership, concurrent work, or NX809J hardware.",
