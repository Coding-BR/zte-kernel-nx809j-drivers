#!/usr/bin/env python3
"""Run the direct tp_BBAT_test_write source contract twice with ASan/UBSan."""
...
EXPECTED = "tp_BBAT_test_write host contract: PASS (4 printk calls)\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_BBAT_test_write_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_BBAT_test_write.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tp_BBAT_test_write source or harness is missing")
    ...
    for number in (1, 2):
        ...
        compiled = run(["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                        ...
                        "-frandom-seed=zte-tpd-bbat-test-write-source",
                        "-ffile-prefix-map=/drivers=<drivers>",
                        "/drivers/validation/zte_tpd/host/tp_BBAT_test_write_host_test.c",
    ...
    report = {"schema_version": 1, "driver": "zte_tpd",
              "target": "tp_BBAT_test_write", "mode": "offline_direct_source_asan_ubsan",
