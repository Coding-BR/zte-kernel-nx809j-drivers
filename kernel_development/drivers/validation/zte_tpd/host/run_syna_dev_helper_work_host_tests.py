#!/usr/bin/env python3
"""Build and run the syna_dev_helper_work harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_helper_work host tests (5 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_helper_work_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-next166-helper-work",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/"
            "syna_dev_helper_work_host_test.c", "-o", f"/output/{binary_name}",
        ]
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_helper_work_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_helper_work",
        "covered_functions": ["syna_dev_helper_work"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers inactive state, boot mode requeue, pending report, ready callback and boot failure.",
            "The indirect CFI preamble is modeled by a no-op break hook; no kernel workqueue, firmware or smartphone hardware is exercised.",
        ],
    }
