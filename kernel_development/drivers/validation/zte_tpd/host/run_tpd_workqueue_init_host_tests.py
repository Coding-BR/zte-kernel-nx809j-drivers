#!/usr/bin/env python3
"""Build and run the tpd_workqueue_init host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS tpd_workqueue_init host tests (4 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("tpd_workqueue_init_host_test.c"))
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "tpd_workqueue_init.c"
	...
	for cycle in (1, 2):
		...
		command = ["docker", "run", "--rm",
			...
			"-Werror", "-Wno-int-conversion", "-fno-omit-frame-pointer",
			"-fno-pie", "-no-pie",
			"-frandom-seed=zte-tpd-next63-tpd-workqueue-init",
			"-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
			"-Wl,--build-id=none", "/drivers/validation/zte_tpd/host/"
			"tpd_workqueue_init_host_test.c", "-o", f"/output/{binary_name}"]
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next63_tpd_workqueue_init_asan_ubsan",
		"driver": "zte_tpd", "target": "tpd_workqueue_init",
		"covered_functions": ["tpd_workqueue_init"], "source": str(source),
		...
		"limitations": [
			"The harness proves all stock-observed initialization and failure branches, offsets, callbacks, timer arguments and visible call order.",
			"The intentional stock branch calls destroy_workqueue(NULL) only after tpd_report_work_init clears the saved workqueue slot.",
