...
static long set_one_key_contract(const char *buffer, size_t count,
...
int main(void)
{
    ...
    callback_count = 0;
    callback_value = 99;
    result = set_one_key_contract("0", 1, fake_callback);
    if (check(result == 1 && callback_count == 1 && callback_value == 0,
              "zero normalizes to false"))
        return 1;

    result = set_one_key_contract("7\n", 2, fake_callback);
    if (check(result == 2 && callback_count == 2 && callback_value == 1,
              "nonzero normalizes to true"))
        return 1;

    result = set_one_key_contract("invalid", 7, fake_callback);
    if (check(result == -EINVAL && callback_count == 2,
              "invalid input returns EINVAL without callback"))
        return 1;

    result = set_one_key_contract("4294967296", 10, NULL);
    if (check(result == -EINVAL && callback_count == 2,
              "out of range input returns EINVAL"))
        return 1;

    puts("PASS set_one_key host tests (4 cases)");
    return 0;
}
