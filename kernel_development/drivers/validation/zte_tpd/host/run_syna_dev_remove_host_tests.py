#!/usr/bin/env python3
"""Build and run the syna_dev_remove host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_remove host tests (4 scenarios)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_dev_remove_host_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro",
                   ...
                   "-Wno-sign-compare", "-Wno-int-conversion", "-Wno-incompatible-pointer-types",
                   ...
                   "-Wno-error=unused-but-set-variable", "-Wno-error=uninitialized",
                   "-fno-omit-frame-pointer", "-fno-pie", "-no-pie", "-D", "ZTE_TPD_HOST_TEST",
                   "-frandom-seed=zte-tpd-target158-remove", "-ffile-prefix-map=/drivers=<drivers>",
                   "-fsanitize=address,undefined", "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_remove_host_test.c",
                   "-o", f"/output/{binary_name}"]
        compiled = execute(command)
        run = (execute(["docker", "run", "--rm", "-e", "ASAN_OPTIONS=detect_leaks=0",
                        "-v", f"{cycle_root}:/output:ro", IMAGE, f"/output/{binary_name}"])
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              "mode": "offline_direct_source_syna_dev_remove_asan_ubsan", "driver": "zte_tpd",
              "target": "syna_dev_remove", "covered_functions": ["syna_dev_remove"],
              "source": str(source), "source_sha256": sha256(source), "expected_scenarios": 4,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": ["The harness covers full cleanup, inactive panel, unavailable managed device and absent optional buffers.",
                              "The NULL context branch is intentionally not invoked because the stock path logs and then dereferences it.",
                              "Kernel scheduler, Device Tree, panel service, firmware transport and physical hardware are stubbed offline."]}
