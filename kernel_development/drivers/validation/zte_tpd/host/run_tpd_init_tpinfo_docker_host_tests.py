#!/usr/bin/env python3
"""Build and run the tpd_init_tpinfo contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_init_tpinfo contract test (6 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_init_tpinfo_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_init_tpinfo.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_init_tpinfo_contract_asan_ubsan"
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-init-tpinfo",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_init_tpinfo_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_init_tpinfo_docker_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_init_tpinfo",
        "function": "tpd_init_tpinfo",
        ...
        "limitations": [
            "The harness validates observed offsets, stores and byte-order conversion with fake memory.",
