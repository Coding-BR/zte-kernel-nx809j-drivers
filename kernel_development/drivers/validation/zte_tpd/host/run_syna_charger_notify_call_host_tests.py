#!/usr/bin/env python3
"""Build and run the syna_charger_notify_call contract harness twice."""
...
EXPECTED = "PASS syna_charger_notify_call contract test (14 checks)\n"
...
def source_body() -> tuple[str, str]:
    source_path = ROOT / "kernel_development/drivers/reconstructed/zte_tpd/syna_charger_notify_call.c"
...
def main() -> int:
    ...
    if args.prepare_source_root:
        ...
        (args.prepare_source_root / "syna_charger_notify_call_under_test.c").write_text(body, encoding="utf-8")
    cycles = []
    for number in (1, 2):
        cycle = args.build_root / f"cycle{number}"
        cycle.mkdir(parents=True, exist_ok=True)
        (cycle / "syna_charger_notify_call_under_test.c").write_text(body, encoding="utf-8")
        command = [
            ...
            "-Wl,--build-id=none -I/out -I/drivers/reconstructed/zte_tpd "
            "/drivers/validation/zte_tpd/host/syna_charger_notify_call_contract_test.c "
            "-o /out/syna_charger_notify_call_contract_test",
        ]
        compiled = subprocess.run(command, text=True, capture_output=True, check=False)
        executed = None
        binary = cycle / "syna_charger_notify_call_contract_test"
        if compiled.returncode == 0:
            executed = subprocess.run(
                ["docker", "run", "--rm", "--network", "none", "-v",
                 f"{cycle.resolve()}:/out:ro", IMAGE,
                 "/out/syna_charger_notify_call_contract_test"],
    ...
    report = {"schema_version": 1, "driver": "zte_tpd", "target": "syna_charger_notify_call",
              "mode": "offline_contract_model_asan_ubsan",
              "source": str(ROOT / "kernel_development/drivers/reconstructed/zte_tpd/syna_charger_notify_call.c"),
              "source_sha256": source_hash,
              "expected_output": EXPECTED, "cycles": cycles,
              "reproducible_binary": passed, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "coverage": {"nonzero_event_early_return": True, "usb_match": True,
                           "ac_match": True, "nonmatch_rejection": True,
                           "queue_cpu_32": True, "queue_delay_125": True,
                           "notifier_relative_offsets": True},
              "limitations": ["Uses a deterministic queue_delayed_work_on stub; hardware behavior is a separate gate.",
