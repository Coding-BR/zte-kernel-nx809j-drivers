#!/usr/bin/env python3
"""Run reproducible ASan/UBSan tests for tpd_touch_release."""
...
EXPECTED_STDOUT = "PASS tpd_touch_release host tests (4 cases)\n"
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "tpd_touch_release_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "tpd_touch_release.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("tpd_touch_release harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-touch-release",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-fno-sanitize=alignment",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/tpd_touch_release_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_touch_release_asan_ubsan",
        "driver": "zte_tpd", "target": "tpd_touch_release",
        "covered_functions": ["tpd_touch_release"],
        ...
        "limitations": [
            "Input, point-report state, jiffies, input events, gesture reporting, ghost detection and workqueue are deterministic host stubs.",
            "The harness covers inactive cleanup, normal release, single/double gesture paths, ghost accounting and edge re-press scheduling without hardware. Alignment sanitizer is disabled because the decompiled stock layout intentionally writes byte offsets 85 and 89 through wider lvalues.",
