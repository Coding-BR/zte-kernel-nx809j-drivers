#!/usr/bin/env python3
"""Build and run the syna_dev_do_reflash host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_do_reflash host tests (7 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "syna_dev_do_reflash_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "ZTE_TPD_HOST_TEST", "-frandom-seed=zte-tpd-target374-do-reflash",
                   "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
                   "-Wl,--build-id=none",
                   "/drivers/validation/zte_tpd/host/syna_dev_do_reflash_contract_test.c",
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_syna_dev_do_reflash_asan_ubsan",
              "driver": "zte_tpd", "target": "syna_dev_do_reflash",
              "covered_functions": ["syna_dev_do_reflash"], "source": str(source),
              "source_sha256": sha256(source), "expected_cases": 7,
              "repetitions": 2, "cycles": cycles, "reproducible": reproducible,
              "reproducible_binary": reproducible, "passed": passed,
              "status": "PASS" if passed else "FAIL",
              "limitations": [
                  "Firmware loading and update transport are stubbed; the harness checks the exact call and error contracts.",
