#!/usr/bin/env python3
"""Build and run the zte_touch_pdev_register host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS zte_touch_pdev_register host tests (3 cases)\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("zte_touch_pdev_register_host_test.c"))
	...
	driver_source = drivers / "reconstructed" / "zte_tpd" / "zte_touch_pdev_register.c"
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next61_pdev_register_asan_ubsan", "driver": "zte_tpd",
		"target": "zte_touch_pdev_register", "covered_functions": ["zte_touch_pdev_register"],
		...
		"limitations": ["The harness proves allocation, add-success, add-failure, cdev offsets and direct release ordering.",
			"Module assembly, KCFI, Ghidra and Joern remain independent gates.",
			"No smartphone or platform bus interaction is used."]}
