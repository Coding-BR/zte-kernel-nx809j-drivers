#!/usr/bin/env python3
"""Build and run the firmware-descriptor cleanup harness twice with sanitizers."""
...
EXPECTED_STDOUT = ("PASS test_descriptor_with_data_cleanup\n"
	"PASS test_descriptor_without_data_cleanup\n"
	"PASS test_null_descriptor_reset\n"
...
def main() -> int:
	...
	parser.add_argument("--source", type=pathlib.Path,
		default=pathlib.Path(__file__).with_name("tp_free_tp_firmware_data_host_test.c"))
	parser.add_argument("--build-root", type=pathlib.Path, required=True)
	parser.add_argument("--output", type=pathlib.Path, required=True)
	args = parser.parse_args()

	source = args.source.resolve()
	driver_source = source.parents[3] / "reconstructed" / "zte_tpd" / "tp_free_tp_firmware_data.c"
	...
	for cycle in (1, 2):
		...
		command = [args.compiler, "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra",
			...
			"-frandom-seed=zte-tpd-next61-tp-free-firmware-data",
	...
	report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
		"mode": "offline_direct_source_next61_tp_free_tp_firmware_data_asan_ubsan",
		"driver": "zte_tpd", "target": "tp_free_tp_firmware_data",
		"covered_functions": ["tp_free_tp_firmware_data"], "source": str(source),
		...
		"limitations": ["The harness directly exercises descriptor, data and null cleanup branches.",
