#!/usr/bin/env python3
"""Build and run the syna_dev_module_init host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_module_init host tests (2 cases)\n"
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name("syna_dev_module_init_host_test.c"))
    parser.add_argument("--build-root", type=pathlib.Path, required=True)
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    source = args.source.resolve()
    drivers = source.parents[3]
    driver_source = drivers / "reconstructed" / "zte_tpd" / "syna_dev_module_init.c"
    ...
    for cycle in (1, 2):
        ...
        command = ["docker", "run", "--rm", "-v", f"{drivers}:/drivers:ro", "-v",
                   ...
                   "-frandom-seed=zte-tpd-syna-dev-module-init", "-ffile-prefix-map=/drivers=<drivers>",
    ...
    report = {"schema_version": 1, "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
              ...
              "driver": "zte_tpd", "target": "syna_dev_module_init",
              "covered_functions": ["syna_dev_module_init", "syna_hw_interface_init",
                                     "msleep", "platform_driver_register"],
              "source": str(source), "source_sha256": sha256(source), "expected_cases": 2,
              ...
              "limitations": ["The harness uses deterministic stubs for hardware init, sleep and driver registration.",
                              "No smartphone, SPI, IRQ, MMIO, firmware transport or hardware is exercised."]}
