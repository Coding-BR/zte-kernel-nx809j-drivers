#!/usr/bin/env python3
"""Run the isolated partition-name lookup contract under sanitizers."""
...
HOST_SOURCE = Path("kernel_development/drivers/validation/zte_tpd/host/syna_tcm_get_partition_id_string_host_test.c")
DRIVER_SOURCE = Path("kernel_development/drivers/reconstructed/zte_tpd/syna_tcm_get_partition_id_string.c")
...
EXPECTED = "PASS syna_tcm_get_partition_id_string host tests (28 cases)\n"
...
def main() -> int:
    ...
    for cycle in range(1, 3):
        ...
        compile_command = [
            "docker", "run", "--rm", "-v", f"{REPO_ROOT / 'kernel_development/drivers'}:/drivers:ro",
            "-v", f"{cycle_root}:/output", "-v", f"{TOOLCHAIN_VOLUME}:/toolchains:ro", IMAGE, CLANG,
            "-std=gnu11", "-O1", "-g", "-Wall", "-Wextra", "-Werror", "-fno-omit-frame-pointer",
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next87-partition-id-string",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined", "-Wl,--build-id=none",
            f"/drivers/{HOST_SOURCE.relative_to('kernel_development/drivers').as_posix()}", "-o", binary,
        ]
    ...
    report = {"schema_version": 1, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
              "mode": "offline_direct_source_next87_partition_id_string_asan_ubsan", "driver": "zte_tpd",
              "target": "syna_tcm_get_partition_id_string", "covered_functions": ["syna_tcm_get_partition_id_string"],
              ...
              "container_image": IMAGE, "toolchain_volume": TOOLCHAIN_VOLUME, "sanitizers": ["address", "undefined"],
              "expected_cases": 28, "repetitions": 2, "cycles": cycles,
              "inputs": [{"path": str(source), "size": source.stat().st_size, "sha256": sha256(source)},
              ...
              "limitations": ["The harness proves table lookup and invalid-ID fallback only.", "Assembly, KCFI, Ghidra and Joern remain independent gates.", "No smartphone, touch hardware or flash interaction is used."]}
