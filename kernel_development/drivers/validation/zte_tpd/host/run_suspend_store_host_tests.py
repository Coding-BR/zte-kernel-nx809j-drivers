#!/usr/bin/env python3
"""Run a reproducible ASan/UBSan harness for suspend_store in Docker."""
...
def main() -> int:
    ...
    harness = drivers_root / "validation" / "zte_tpd" / "host" / "suspend_store_host_test.c"
    source = drivers_root / "reconstructed" / "zte_tpd" / "suspend_store.c"
    if not harness.is_file() or not source.is_file():
        raise FileNotFoundError("suspend_store harness or source is missing")
    ...
    for cycle in range(1, args.repetitions + 1):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-suspend-store",
            "-ffile-prefix-map=/drivers=<drivers>",
            "-fsanitize=address,undefined", "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/suspend_store_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_suspend_store_asan_ubsan",
        "driver": "zte_tpd",
        "target": "suspend_store",
        "covered_functions": ["suspend_store"],
