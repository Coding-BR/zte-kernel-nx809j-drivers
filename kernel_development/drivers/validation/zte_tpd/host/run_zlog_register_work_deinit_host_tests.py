#!/usr/bin/env python3
"""Build and run the direct zlog_register_work_deinit contract in Docker twice."""
...
EXPECTED_STDOUT = "PASS zlog_register_work_deinit host tests (4 cases)\n"
DRIVER_SOURCE_C = pathlib.Path(
    "kernel_development/drivers/reconstructed/zte_tpd/zlog_register_work_deinit.c"
)
...
def main() -> int:
    ...
    parser.add_argument("--source", type=pathlib.Path,
                        default=pathlib.Path(__file__).with_name(
                            "zlog_register_work_deinit_contract_test.c"))
    ...
    for cycle in (1, 2):
        ...
        passed = bool(run and run.returncode == 0
                      and run.stdout == EXPECTED_STDOUT and run.stderr == "")
    ...
    report = {"schema_version": 1,
              ...
              "mode": "offline_direct_source_zlog_register_work_deinit_asan_ubsan",
              "driver": "zte_tpd", "target": "zlog_register_work_deinit",
              "covered_functions": ["zlog_register_work_deinit"],
              ...
              "limitations": [
                  "tpd_zlog_device, cancel_delayed_work_sync, printk and vfree are contract stubs.",
                  "The harness verifies cancellation, freeing and NULL clearing on repeated teardown.",
