#!/usr/bin/env python3
"""Build and run the syna_dev_enable_lowpwr_gesture host harness twice in Docker."""
...
EXPECTED_STDOUT = "PASS syna_dev_enable_lowpwr_gesture host tests (5 cases)\n"
...
def main() -> int:
    ...
    source = root / "kernel_development" / "drivers" / "validation" / "zte_tpd" / "host" / "syna_dev_enable_lowpwr_gesture_host_test.c"
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-fno-pie", "-no-pie", "-frandom-seed=zte-tpd-next173-lowpwr-gesture",
            "-ffile-prefix-map=/drivers=<drivers>", "-fsanitize=address,undefined",
            "-Wl,--build-id=none",
            "/drivers/validation/zte_tpd/host/syna_dev_enable_lowpwr_gesture_host_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_syna_dev_enable_lowpwr_gesture_asan_ubsan",
        "driver": "zte_tpd",
        "target": "syna_dev_enable_lowpwr_gesture",
        "covered_functions": ["syna_dev_enable_lowpwr_gesture"],
        "source": str(source),
        ...
        "limitations": [
            "The harness covers inactive state, enable/disable transitions, IRQ wake, dynamic-config IDs 9/254/212 and failures at each stage.",
            "No smartphone, IRQ controller or Synaptics hardware interaction is used.",
        ],
    }
