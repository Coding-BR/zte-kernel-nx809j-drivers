#!/usr/bin/env python3
"""Run the direct tp_game_partition_read source contract twice with ASan/UBSan in Docker."""
...
EXPECTED = "PASS tp_game_partition_read: 5 cases\n"
...
def main() -> int:
    ...
    harness = drivers / "validation" / "zte_tpd" / "host" / "tp_game_partition_read_host_test.c"
    source = drivers / "reconstructed" / "zte_tpd" / "tp_game_partition_read.c"
    exact_source = drivers / "reconstructed" / "zte_tpd" / "tp_game_partition_read_exact.S"
    if not harness.is_file() or not source.is_file() or not exact_source.is_file():
        raise FileNotFoundError("tp_game_partition_read source or direct harness is missing")
    ...
    for cycle in (1, 2):
        ...
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-game-partition-read-source",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tp_game_partition_read_host_test.c",
