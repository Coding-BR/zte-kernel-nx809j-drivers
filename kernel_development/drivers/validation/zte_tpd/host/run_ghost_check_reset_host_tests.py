#!/usr/bin/env python3
"""Compile and execute the ghost_check_reset host contract twice."""
...
def main() -> int:
    ...
    source = "/drivers/validation/zte_tpd/host/ghost_check_reset_host_test.c"
    cycles = []
    for number in (1, 2):
        cycle = args.build_root / f"cycle{number}"
        cycle.mkdir(parents=True, exist_ok=True)
        binary = cycle / "ghost_check_reset_host_test"
        command = [
            ...
            f"{source} -o /out/ghost_check_reset_host_test && "
            "/out/ghost_check_reset_host_test",
        ]
        ...
        cycles.append({
            ...
            "passed": result.returncode == 0 and result.stdout ==
                      "PASS ghost_check_reset host tests (2 cases)\n" and
                      not result.stderr,
        })
