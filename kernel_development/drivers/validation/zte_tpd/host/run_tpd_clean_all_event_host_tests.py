#!/usr/bin/env python3
"""Compile and execute the tpd_clean_all_event contract twice."""
...
def main() -> int:
    ...
    source = "/drivers/validation/zte_tpd/host/tpd_clean_all_event_host_test.c"
    cycles = []
    for number in (1, 2):
        cycle = args.build_root / f"cycle{number}"
        cycle.mkdir(parents=True, exist_ok=True)
        binary = cycle / "tpd_clean_all_event_host_test"
        command = [
            ...
            f"{source} -o /out/tpd_clean_all_event_host_test && "
            "/out/tpd_clean_all_event_host_test",
        ]
