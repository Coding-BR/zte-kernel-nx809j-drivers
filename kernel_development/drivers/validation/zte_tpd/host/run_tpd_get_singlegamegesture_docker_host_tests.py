#!/usr/bin/env python3
"""Build and run the tpd_get_singlegamegesture contract oracle twice in Docker."""
...
EXPECTED = "PASS tpd_get_singlegamegesture contract test (3 checks)\n"
...
def main() -> int:
    ...
    test_source = source.with_name("tpd_get_singlegamegesture_contract_test.c")
    driver_source = repo / "kernel_development" / "drivers" / "reconstructed" / "zte_tpd" / "tpd_get_singlegamegesture.c"
    ...
    for number in (1, 2):
        ...
        cycle.mkdir()
        binary = cycle / "tpd_get_singlegamegesture_contract_asan_ubsan"
        compile_command = [
            ...
            "-frandom-seed=zte-tpd-tpd-get-singlegamegesture",
            "-ffile-prefix-map=/drivers=<drivers>",
            "/drivers/validation/zte_tpd/host/tpd_get_singlegamegesture_contract_test.c",
    ...
    report = {
        ...
        "mode": "offline_direct_source_tpd_get_singlegamegesture_docker_asan_ubsan",
        "driver": "zte_tpd",
        "target": "tpd_get_singlegamegesture",
        "function": "tpd_get_singlegamegesture",
        ...
        "limitations": [
            "The harness validates the observed state offsets and destination store with fake memory.",
